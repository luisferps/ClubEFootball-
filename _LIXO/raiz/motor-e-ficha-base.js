...
function distOtima(start,arows,orc,add,bf){
 ...
 const nA=(i,n)=>{const r=R[i];if(!r)return 0;const A=(add&&add[i])||0;const B=bf&&bf[i];
  const f=(v)=>{let w=Math.min(99,v)+A; if(B)w=aplicaBuff(w,B[0],B[1]);
...
function mtPro(){
 ...
 if(ch&&nz(ch)==="encaixepro"){PRO=true;try{localStorage.setItem("MT_PRO","1")}catch(e){};mtRender();return true;}
