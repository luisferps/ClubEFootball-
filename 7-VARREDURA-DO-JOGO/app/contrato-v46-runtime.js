...
/**
 * Camada de migração V4.6.
 *
 * Substitui a leitura dos dados básicos de Player.bin por uma leitura guiada
 * exclusivamente pelo contrato ativo. O mapa físico legado permanece apenas
 * para semântica não estrutural (nomes antigos/weights) enquanto as famílias
 * restantes são migradas.
 */
(function installV46ContractRuntime(global) {
  ...
  function parseUtfTable(data) {
    ...
    const columnCount = u16be(block, 16), rowLength = u16be(block, 18), rowCount = u32be(block, 20);
    ...
    for (let index = 0; index < columnCount; index += 1) {
      ...
      const storage = flag & 0xf0, type = flag & 0x0f;
      let constant = null;
  ...
  function decompressCriLayla(source) {
    ...
    const uncompressedSize = u32(source, 8), headerOffset = u32(source, 12);
    const output = new Uint8Array(uncompressedSize), header = source.subarray(16 + headerOffset, 16 + headerOffset + 0x100), data = source.subarray(16, 16 + headerOffset);
  ...
  function overall(attributes, position) {
    ...
    let score = weights.b;
    for (let i = 0; i < weights.w.length; i += 1) { if (attributes[i] == null) return null; score += attributes[i] * weights.w[i]; }
    return Math.round(score);
  }

  async function decodeBasicCards(bytes, plan) {
    const index = reader.requirePlan(plan);
    const keys = ['carta.id','carta.altura','carta.peso','carta.idade','carta.posicao.principal','carta.playstyle.primario','carta.playstyle.secundario','carta.pe','carta.pe.ruim_uso','carta.pe.ruim_precisao','carta.forma','carta.resistencia_lesao.alta','carta.resistencia_lesao.media','carta.nacionalidade.raw','carta.nome.roman'];
    if (keys.some((key) => !index.fields.has(key))) throw new Error('contrato ativo não contém todos os dados básicos de carta');
    const file = index.files.get(index.fields.get('carta.id').arquivo_id);
    if (!file || keys.some((key) => index.fields.get(key).arquivo_id !== file.arquivo_id)) throw new Error('dados básicos de carta não pertencem ao mesmo Player.bin do contrato');
    const cpk = extractCpk(bytes), packed = cpk[file.arquivo];
    if (!packed) throw new Error(`${file.arquivo} não encontrado no CPK atual`);
    const decoded = await reader.decodeFile(plan, file.arquivo, await unpackWesys(packed), keys);
    ...
    return decoded.records.map((record) => {
      const v = record.values, id = String(v['carta.id']), position = positions.get(Number(v['carta.posicao.principal']));
      if (!position) throw new Error(`posição principal sem catálogo no contrato: ${id}`);
      ...
      return {
        ...
        nationality: nationalityEnglish[nationalityCode] || String(nationalityCode), name: String(v['carta.nome.roman'] || '')
      };
  ...
  async function extractCardsFromCpkV46(bytes, plan, log = () => {}) {
    ...
    const [slots, attributes, relations, bodies] = await Promise.all([
      core.extractCardSlotsByContract(bytes, plan), core.extractCardAttributesByContract(bytes, plan), core.extractCardRelationsByContract(bytes, plan), core.extractCardBodiesByContract(bytes, plan)
    ]);
    // Box continua explicitamente fora desta migração; o contrato permanece bloqueado até PlayerVariationDetail entrar no selo.
    for (const card of cards) {
      card.box = null;
      const slot = slots.slots.get(card.card_id); if (!slot) throw new Error(`contrato não retornou slots para ${card.card_id}`);
      ...
      const test = card.attrs.every((value) => value === 99), base = BigInt(card.card_id) < (1n << 18n);
      card.tipo = test ? 'teste' : (base ? 'base' : 'colecionavel'); card.roda_motor = !test && !base; card.overall = overall(card.attrs, card.position);
      const rel = relations.get(card.card_id); if (!rel) throw new Error(`contrato não retornou relações para ${card.card_id}`);
    ...
    log(`V4.6 · dados básicos, slots, atributos, relações e corpo lidos pelo contrato ${plan.versao_contrato}; boxes ainda bloqueados até selagem de PlayerVariationDetail.bin`);
    return cards;
  }
