...
(function installUi(global) {
  ...
  const state = {
    ...
    readingContract: { status: 'loading', plan: null, error: null },
    sources: {},
  ...
  const SOURCE_SPECS = Object.freeze({
    dt870_updated: { label: 'DT870 da atualização', filename: 'dt870_console_win.cpk', purpose: 'Cartas, relações e dimensões atuais; técnicos e nacionalidades; ímpetos e CompetitionUnit; habilidades e overlay de playstyles.', operations: 'Cartas e catálogos' },
  ...
  function contractSeal(plan = state.readingContract.plan) {
    if (!plan || state.readingContract.status !== 'ready') throw new Error('O contrato de leitura ativo não foi confirmado. A extração permanece bloqueada.');
    ...
    const seal = Object.fromEntries(keys.map((key) => [key, plan[key]]));
    if (Object.values(seal).some((value) => typeof value !== 'string' || !value)) throw new Error('O contrato de leitura retornou selo incompleto.');
    return seal;
  }
  function assertContractResponse(response, payload = null) {
    const expected = contractSeal();
    const headers = {
      contrato_id: response.headers.get('X-Clubef-Contract-Id'),
      versao_contrato: response.headers.get('X-Clubef-Contract-Version'),
      fingerprint_contrato_sha256: response.headers.get('X-Clubef-Contract-Fingerprint'),
      fingerprint_fontes_sha256: response.headers.get('X-Clubef-Sources-Fingerprint'),
      fingerprint_catalogos_sha256: response.headers.get('X-Clubef-Catalogs-Fingerprint')
    };
    if (headers.contrato_id !== expected.contrato_id || headers.versao_contrato !== expected.versao_contrato || headers.fingerprint_contrato_sha256 !== expected.fingerprint_contrato_sha256 || headers.fingerprint_fontes_sha256 !== expected.fingerprint_fontes_sha256) {
      throw new Error('A resposta não está selada pelo mesmo contrato/fingerprint ativo.');
    }
    if (payload && payload.leitura_contrato && Object.entries(expected).some(([key, value]) => payload.leitura_contrato[key] !== value)) {
      throw new Error('O corpo da resposta diverge do selo de contrato ativo.');
    }
  }
  async function fetchWithContract(url, init = undefined) {
    const response = await global.fetch(url, init);
    if (String(url).startsWith('/api/')) assertContractResponse(response);
    return response;
  }
  async function refreshReadingContract() {
    const response = await global.fetch('/api/reading-contract/current', { cache: 'no-store' });
    const payload = await response.json().catch(() => ({ error: `HTTP ${response.status}` }));
    if (!response.ok) throw new Error(payload.error || `HTTP ${response.status}`);
    state.readingContract = { status: 'ready', plan: payload, error: null };
    contractSeal();
    return payload;
  }
  ...
  async function refreshExecutorStatus() {
    try {
      ...
      try {
        await refreshReadingContract();
      } catch (contractError) {
        state.readingContract = { status: 'blocked', plan: null, error: contractError.message };
      }
      $('executor-dot').className = `dot ${state.executor.manual_apply_available ? 'ok' : 'neutral'}`;
      $('executor-title').textContent = state.readingContract.status === 'ready' ? (state.executor.manual_apply_available ? 'Banco conectado · envio manual' : 'Executor local · modo protegido') : 'Contrato de leitura bloqueado';
      $('executor-detail').textContent = state.readingContract.status === 'ready' ? (state.executor.manual_apply_available ? 'destino único: clube_novo; nenhuma escrita automática' : 'banco sem conexão; extração continua disponível') : state.readingContract.error;
      ...
      state.readingContract = { status: 'blocked', plan: null, error: error.message };
  ...
  async function fetchReferenceStatus() {
    const response = await fetchWithContract('/api/card-reference/status', { cache: 'no-store' });
    const payload = await response.json().catch(() => ({ error: `HTTP ${response.status}` }));
  ...
  function updateRunAvailability() {
    const contractReady = state.readingContract.status === 'ready';
    const incrementalReady = Boolean(contractReady && state.executor.online && state.executor.database_configured && operationIsReady(CARD_SOURCE_ROLES));
    ...
    if (!contractReady) {
      setOperationStatus('incremental-status', 'error', 'Comparação de cartas bloqueada', state.readingContract.error || 'O contrato de leitura ainda não foi confirmado.');
    } else if (!state.executor.online || !state.executor.database_configured) {
    ...
    $('run-metadata').disabled = state.metadataRunning || !contractReady || !operationIsReady(METADATA_SOURCE_ROLES);
  ...
  async function registerSource(role, bytes, details) {
    ...
    renderSources();
    const validation = await core.validateSourceByContract(bytes, state.readingContract.plan, role);
    state.sources[role] = {
      status: 'ready',
      bytes,
  ...
  async function ensureSourceBytes(role) {
    const source = state.sources[role];
    if (source && source.bytes) return source.bytes;
    if (!source || source.status !== 'ready') throw new Error(`A fonte ${SOURCE_SPECS[role].label} não está pronta para leitura física.`);
    const response = await fetchWithContract(`/api/sources/file?role=${encodeURIComponent(role)}`, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Não foi possível abrir ${SOURCE_SPECS[role].label} para a releitura de Dimensões.`);
    await registerSource(role, new Uint8Array(await response.arrayBuffer()), {
      name: source.name, modifiedAt: source.modifiedAt, origin: source.origin || 'automatic'
    });
    return state.sources[role].bytes;
  }
    ...
    try {
      ...
      for (const role of Object.keys(SOURCE_SPECS)) {
        ...
        if (role === 'dt870_updated' || role === 'dt261_bra') {
          try {
            const fileResponse = await fetchWithContract(`/api/sources/file?role=${encodeURIComponent(role)}`, { cache: 'no-store' });
  ...
  async function fetchCurrentCardBaseline() {
    const response = await fetchWithContract('/api/card-baseline/current.csv', { cache: 'no-store' });
    if (!response.ok) {
      const payload = await response.json().catch(() => ({ error: `HTTP ${response.status}` }));
  ...
  async function runIncrementalComparison(trigger) {
    ...
    try {
      ...
      const cards = await core.extractCardsFromCpk(cpkBytes, state.readingContract.plan, (message) => log('log-incremental', message));
      ...
      const relationsReadback = await postJson('/api/card-relations/validate', { card_csv: core.rowsToCsv(currentRows) }, 600000);
      ...
      const manifest = await core.sealManifest({
        ...
        validation,
        card_relations_validation: relationsReadback,
        changes: {
      ...
      state.incremental = { baseline, cards, currentRows, relationsReadback, diff, manifest, selection: defaultCardSelection(diff), allItems: applicableItems };
  ...
  async function refreshMetadataAutomatically(trigger = 'abertura automática') {
    ...
    try {
      ...
      for (const role of METADATA_SOURCE_ROLES) await ensureSourceBytes(role);
      const response = await fetchWithContract('/api/metadata-reference/status', { cache: 'no-store' });
      ...
      const familySourceBytes = Object.fromEntries(METADATA_SOURCE_ROLES.map((role) => [role, state.sources[role].bytes]));
      const familySourceDescriptors = Object.fromEntries(METADATA_SOURCE_ROLES.map((role) => [role, sourceDescriptor(role)]));
      log('log-metadata', 'Relendo cumulativamente Técnicos, Textos e Ímpetos nas quatro fontes oficiais…');
      const physical = await core.extractMetadataByFamily(familySourceBytes, familySourceDescriptors, (message) => log('log-metadata', message));
      ...
      summary.textos = {
        status: 'validado_banco', reason: baseline.schema_ready ? null : 'contrato estrutural ainda não instalado; comparação disponível e aplicação bloqueada',
      ...
      log('log-metadata', 'Ímpetos aprovados: 440 tipos catalogados, 2.072 efeitos, 407 condições, 696 faixas e 35 membros de liga; consumidor permanece bloqueado.');
      log('log-metadata', 'Relendo país, clube, liga e tipo para todas as cartas…');
      const dimensionSnapshot = await core.extractCardDimensionsByFamily(
        familySourceBytes,
      ...
      const dimensionReadback = await postJson('/api/card-dimensions/validate', { snapshot: dimensionSnapshot }, 600000);
      if (!dimensionReadback.passed || !dimensionReadback.transaction_read_only || dimensionReadback.database_write !== false) {
        throw new Error('A comparação integral de Dimensões com clube_novo não foi aprovada.');
      }
      diff.dimensoes_cartas = {
        status: 'validado_banco', reason: null,
        current: dimensionStructure.cards, baseline_active: dimensionReadback.comparisons.cards.database,
        new_entries: [], changed_entries: [], absent_entries: [], without_previous_fingerprint: 0, duplicate_ids: []
      };
      summary.dimensoes_cartas = {
        status: 'validado_banco', reason: null,
        current: dimensionStructure.cards, baseline_active: dimensionReadback.comparisons.cards.database,
        new: 0, changed: 0, absent: 0, without_previous_fingerprint: 0, duplicate_ids: 0,
      ...
      const manifest = await core.sealManifest({
        ...
        text_validation: textDiff.validation, text_source: sourceDescriptor('dt261_bra'), text_baseline: { records: baseline.records, sha256: baseline.sha256, transaction_read_only: true, schema_ready: baseline.schema_ready, catalog_references_checked: baseline.catalog_references_checked, validated_foreign_keys: baseline.validated_foreign_keys },
      ...
      state.metadata = { reference, baseline, physical, textCatalog, textItems, technicianReadback, impetusReadback, dimensionReadback, diff, summary, manifest, selection: new Set(textItems.map((item) => item.key)) };
      ...
      $('metadata-support').innerHTML = Object.entries(summary).map(([name, item]) => item.status === 'nao_suportado_nesta_atualizacao'
        ...
        : item.status === 'bloqueado_fonte_alterada'
          ...
          : `<div class="support-item"><b>${escapeHtml(name)}</b> — ${escapeHtml(name === 'textos' ? (item.new || item.changed ? `${item.new} novas chaves · ${item.changed} atualizadas · 0 remoções sem substituição` : `Tudo atualizado; ${Number(item.current || 0).toLocaleString('pt-BR')} chaves oficiais e 166 referências conferidas.`) : item.status === 'validado_banco' ? 'Tudo atualizado; releitura física igual a clube_novo em transação somente leitura.' : item.status === 'comparado' ? 'Tudo atualizado; fonte física igual à referência interna vigente.' : item.reason || item.status)}</div>`).join('');
      ...
      renderAutomaticSummary();
      log('log-metadata', `Concluído: ${textDiff.current} chaves oficiais, 1.478 técnicos, 2.072 efeitos de ímpeto e ${dimensionStructure.cards} cartas com dimensões conferidos; nenhuma escrita foi feita.`);
    } catch (error) {
  ...
  $('run-full').addEventListener('click', async () => {
    ...
    try {
      ...
      const baselineResponse = await fetchWithContract('/api/card-reference/current.csv', { cache: 'no-store' });
      ...
      const cards = await core.extractCardsFromCpk(source.bytes, state.readingContract.plan, (message) => log('log-full', message));
      const validation = core.validateCards(cards);
  ...
  async function postJson(url, body, timeoutMs = 0) {
    ...
    let response;
    try {
      response = await fetchWithContract(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...body, leitura_contrato: contractSeal() }), signal: controller ? controller.signal : undefined });
    } catch (error) {
  ...
  async function fetchApplyStatus(executionId) {
    const response = await fetchWithContract(`/api/apply/status?execution_id=${encodeURIComponent(executionId)}`, { cache: 'no-store' });
    const payload = await response.json().catch(() => ({ error: `HTTP ${response.status}` }));
    if (!response.ok) throw new Error(payload.error || `HTTP ${response.status}`);
    return payload;
  }
