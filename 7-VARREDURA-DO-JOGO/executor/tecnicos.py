...
Regra V4.6: endereço físico não é autoridade deste módulo. Arquivo, registro,
bits e larguras são recebidos na fotografia produzida pelo Extrator a partir
das referências canônicas do clube_novo.
"""
...
def _compare(source: set[tuple[Any, ...]], database: set[tuple[Any, ...]]) -> dict[str, Any]:
    ...
    missing = sorted(source - database, key=key)
    extra = sorted(database - source, key=key)
    return {
        "source": len(source),
        "database": len(database),
        "missing_in_database": len(missing),
        "extra_in_database": len(extra),
        "source_sha256": _sha256(sorted(source, key=key)),
        "database_sha256": _sha256(sorted(database, key=key)),
...
def _physical(row: dict[str, Any], label: str) -> tuple[int, int]:
    ...
    if not isinstance(bit, int) or not isinstance(width, int) or bit < 0 or width <= 0:
        raise ValueError(f"{label} sem endereço/largura fornecidos pela fotografia canônica")
    return bit, width


def _source_file(row: dict[str, Any], label: str) -> str:
    value = row.get("arquivo")
    if not isinstance(value, str) or not value:
        raise ValueError(f"{label} sem arquivo fornecido pela fotografia canônica")
    return value
...
def validate_tecnicos(snapshot: dict[str, Any], connection: Any) -> dict[str, Any]:
    ...
    if len(records) != 1478 or len({str(r.get("id")) for r in records}) != 1478:
    ...
    source_technicians = {
        (
            int(r["id"]), r.get("nome_en") or None, r.get("nome_jp") or None, r.get("nome_cn") or None,
            int(r["idade"]), int(r["nacionalidade_codigo"]), int(r["afinidade_codigo"]),
            int(r["record_index"]), str(r["source_file_sha256"]),
        )
        for r in records
    }
    source_nationalities = {
        (
            int(r["codigo_jogo"]), r.get("nome_pt_br"), r.get("sigla"), int(r["record_index"]),
            int(r["record_size"]), int(r["codigo_bit"]), int(r["codigo_largura"]), int(r["nome_offset"]),
            int(r["nome_largura"]), int(r["sigla_offset"]), int(r["sigla_largura"]), str(r["source_file_sha256"]),
        )
        for r in nationalities
    }
    source_affinities = {
        (
            int(r["codigo_jogo"]), r.get("nome_pt"), r.get("nome_tela"), bool(r.get("ausencia_legitima")),
            bool(r.get("rotulo_confirmado")), int(r["bit"]), int(r["largura"]), str(r["source_file_sha256"]),
            bool(r.get("pode_rodar")), r.get("falta_o_que"),
        )
        for r in affinities
    }
    ...
    for r in records:
        record_file = _source_file(r, f"técnico {r.get('id')}")
        for code, value in (r.get("proficiencias") or {}).items():
            evidence = (r.get("proficiencias_fisico") or {}).get(code) or {}
            bit, width = _physical(evidence, f"proficiência {code}")
            evidence_file = evidence.get("arquivo") or record_file
            source_styles.add(
                (int(r["id"]), code, int(value), str(evidence_file), int(r["record_index"]), bit, width, str(r["source_file_sha256"]), True)
            )
        for boost in r.get("boosts") or []:
            bit, width = _physical(boost, "boost de técnico")
            boost_file = boost.get("arquivo") or record_file
            pending.append(
                (
                    int(r["id"]), int(boost["ordem"]), int(boost["atributo_idx_canonico"]), int(boost["delta"]),
                    str(boost_file), int(r["record_index"]), bit, width, str(r["source_file_sha256"]), True,
                )
            )
    ...
    checks = {
        "technicians": _compare(source_technicians, dbtech),
        "nationalities": _compare(source_nationalities, dbnat),
        "affinities": _compare(source_affinities, dbaff),
        "proficiencies_and_overload": _compare(source_styles, dbstyles),
        "boosts": _compare(source_boosts, dbboosts),
        "foreign_key_orphans": orphans,
    }
    exact = all(checks[name]["exact"] for name in ("technicians", "nationalities", "affinities", "proficiencies_and_overload", "boosts"))
