...
navegador padrão; ele nunca abre o JSON técnico no Bloco de Notas. O HTML
mostra estados e contagens por família, filtros simples e uma amostra limitada
por verificação/tipo, com chave canônica, classificação, detalhe e procedência.
As contagens vêm da varredura integral do resultado salvo; a limitação é só de
apresentação, explicitada na página.
