...
(function installCore(global) {
  ...
  async function extractMetadataByFamily(sourceBytes, sourceDescriptors, log = () => {}) {
    ...
    for (let index = 0; index < competitionUnitRaw.length / 2472; index += 1) {
      ...
      if (anterior && anterior !== 0xffff) impetos.liga_membros.push({ codigo_liga_alvo_base: codigo, codigo_liga_membro: anterior, ordem_fisica: 1, papel_fisico: 'vinculo_anterior', record_index: index, bit_inicial: 32, largura: 16 });
      impetos.liga_membros.push({ codigo_liga_alvo_base: codigo, codigo_liga_membro: codigo, ordem_fisica: anterior && anterior !== 0xffff ? 2 : 1, papel_fisico: 'alvo_base', record_index: index, bit_inicial: 80, largura: 16 });
      if (posterior && posterior !== 0xffff) impetos.liga_membros.push({ codigo_liga_alvo_base: codigo, codigo_liga_membro: posterior, ordem_fisica: (anterior && anterior !== 0xffff ? 3 : 2), papel_fisico: 'vinculo_posterior', record_index: index, bit_inicial: 48, largura: 16 });
    }
    ...
    impetos.field_contract = {
      ...
      liga_membros: { arquivo: 'CompetitionUnit.bin', tamanho_registro: 2472, anterior: 'bit32/w16', posterior: 'bit48/w16', rotina: '0x144A52D40' }
    };
