...
async function runOnce(index) {
  ...
  assert(baseline.headers.get('x-clubef-slot-projection') === 'none; baseline is stored database state', 'baseline não declarou que não mascara slots');
  ...
  for (const row of projectedRows) {
    ...
    for (const field of ['impeto_s1', 'impeto_s2_cond', 'vaga_s1', 'vaga_s2']) {
      if (String(row[field] ?? '') !== String(physicalRow[field] ?? '')) storedSlotDifferences += 1;
    }
  }
  ...
  assert(cardImpetus.physical.filled === 2378 && cardImpetus.physical.vacancies === 1369 && cardImpetus.physical.empty === 82397, 'contagem física w8 de slots não confere');
  assert(cardImpetus.database_relation.differences_from_physical > 0, 'a divergência real de carta_impeto_jogo foi mascarada');
  const sampleIds = [
    ...
    '56160992464525'   // registro 33281: prova pública w8=45, não w10=301
  ];
  ...
  assert(allSlots.every(item => /^Player\.bin bit(308|288)\/w8$/.test(item.slot.origem_fisica)), 'readback não preservou a origem física w8');
  assert(impetusReadback.database_relation_slot_mismatches > 0, 'readback não revelou a divergência da relação de banco');
  const ronwen = impetusReadback.cards.find(card => card.card_id === '56160992464525');
  assert(ronwen && ronwen.slots[0].codigo_impeto === 45, 'registro 33281 não confirmou w8=45');
...
(async () => {
  ...
  console.log(JSON.stringify({
    result: 'APROVADO_FISICO_RELACAO_DIVERGENTE', database_write: false, runs: 2, fingerprint: first.fingerprint,
