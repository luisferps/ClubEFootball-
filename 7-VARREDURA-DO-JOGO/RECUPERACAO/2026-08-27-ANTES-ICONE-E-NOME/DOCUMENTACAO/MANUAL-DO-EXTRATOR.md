...
2. Dê dois cliques em `ABRIR-EXTRATOR-CLUBEFOOTBALL.exe`.
...
O `.cmd`, o PowerShell e o HTML não são opções concorrentes de uso comum; ficam apenas como componentes internos ou recuperação. O aplicativo abre o executor oculto, mantém a escrita desligada e apresenta somente a interface.
...
3. Use `VERSOES-ANTERIORES\2026-08-27-ANTES-BUSCA-POR-FAMILIA` para restaurar a versão imediatamente anterior, ou `VERSOES-ANTERIORES\2026-08-26` para o legado original.
...
```text
7-VARREDURA-DO-JOGO\
├── ABRIR-EXTRATOR-CLUBEFOOTBALL.exe
├── Extrator-ClubEfootball.html
├── app\
│   ├── mapeamento-fisico.js
│   ├── catalog-source-map.js
│   ├── extrator-core.js
│   └── extrator-ui.js
├── executor\
├── DOCUMENTACAO\
