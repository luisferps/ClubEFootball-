...
def _identity(spec: RelationSpec, row: tuple[Any, ...]) -> tuple[Any, ...]:
    # Relações com valor/ordem têm identidade composta por carta e FK; a
    # última coluna é o conteúdo comparável. Estilo de IA é uma relação pura.
    return row if len(spec.columns) == 2 else row[:-1]


def _classified_relation(kind: str, spec: RelationSpec, source: tuple[Any, ...] | None, database: tuple[Any, ...] | None) -> dict[str, Any]:
    ...
    return {
        "classificacao": kind,
        "escopo": spec.name,
        "chave_canonica": dict(zip(spec.columns[:len(identity)], identity, strict=True)),
        "fonte_fisica": None if source is None else {"fotografia": "cartas-fisicas.csv", "card_id": source[0]},
        "vinculo_banco": None if database is None else dict(zip(spec.columns[:len(identity)], identity, strict=True)),
        "valor_fisico": None if source is None else list(source),
        "valor_banco": None if database is None else list(database),
    }


def compare_relation(spec: RelationSpec, expected: Iterable[tuple[Any, ...]], actual: Iterable[tuple[Any, ...]]) -> dict[str, Any]:
    ...
    def keyed(rows: list[tuple[Any, ...]], origin: str) -> tuple[dict[tuple[Any, ...], tuple[Any, ...]], list[dict[str, Any]]]:
        ...
        for row in rows:
            ...
            if key in out:
                repeated.append(_classified_relation("repetido", spec, row if origin == "fisico" else None, row if origin == "banco" else None))
            else:
                out[key] = row
        return out, repeated
    ...
    for key in sorted(set(source_by_key) | set(database_by_key), key=lambda value: json.dumps(value, default=str)):
        ...
        if right is None:
            classification["new"].append(_classified_relation("novo", spec, left, None))
        elif left is None:
            classification["removed"].append(_classified_relation("removido", spec, None, right))
        elif left != right:
            classification["altered"].append(_classified_relation("alterado", spec, left, right))
...
def validate_card_relations(csv_text: str, connection: Any, schema: str, sql: Any) -> dict[str, Any]:
    ...
    if len(required_positions) != 12:
        raise ValueError(
            f"a fotografia física contém {len(required_positions)} aptidões distintas; esperado 12"
        )
