...
from card_impetus import readback_card_slots, validate_physical_slot_projection
...
def current_card_impetus_readback(card_ids: list[str], csv_text: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    with psycopg.connect(dsn, connect_timeout=20) as connection:
        connection.read_only = True
        return readback_card_slots(connection, card_ids, csv_text)
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/card-baseline/current.csv":
            try:
                ...
                self.send_header("X-Clubef-Slot-Projection", "none; baseline is stored database state")
                self.end_headers()
                self.wfile.write(data)
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path == "/api/card-impetus/readback":
                ...
                self.send_json(HTTPStatus.OK, current_card_impetus_readback(card_ids, csv_text, config))
                return
