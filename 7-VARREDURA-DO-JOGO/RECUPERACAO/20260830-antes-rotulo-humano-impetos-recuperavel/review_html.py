...
SCHEMA = "clubef-result-review-html-v2"
...
TYPE_LABELS = {
    ...
    "altered": "Alterado",
    "repeated": "Repetido",
    "invalid": "Para investigar",
}
FAMILY_LABELS = {
    "cartas": "Cartas e jogadores",
    "relacoes": "Habilidades, posições e atributos dos jogadores",
    "dimensoes": "Dados físicos dos jogadores",
    "impetos": "Ímpetos",
    "tecnicos": "Técnicos",
    "textos": "Textos do jogo",
    "catalogos": "Informações técnicas para investigar",
    "metadados": "Informações técnicas para investigar",
}
STATE_LABELS = {
...
def _presentation_context(run_dir: Path, samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if plan_path.is_file():
        try:
            ...
            catalogs = plan.get("catalogos") if isinstance(plan, dict) else None
            ...
            pass
    return {"cards": cards, "catalogs": catalog_rows}
...
def _human_entity_title(sample: dict[str, Any], context: dict[str, Any]) -> str:
    ...
    impulse = _first_human_label(_presentation_row(context, "impeto_jogo", impulse_code))
    ...
    if impulse_code is not None:
        return f"Ímpeto “{impulse}”" if impulse else "Ímpeto sem rótulo disponível no catálogo"
    if isinstance(catalog, str) and catalog:
...
def _family_summaries(header: dict[str, Any], report_counts: dict[str, dict[str, int]], complete: bool, real_totals: dict[str, dict[str, int]]) -> list[dict[str, Any]]:
    ...
    for family_key in order:
        ...
        if not complete:
            detail = "A página foi gerada parcialmente; as contagens podem estar incompletas."
        elif total == 0 and state != "technical_issue":
            detail = "Nenhuma diferença encontrada nesta família nesta execução."
        elif state == "technical_issue":
            detail = "Há uma informação para investigar antes de aplicar esta parte."
        else:
            detail = f"{total} diferença(s) encontrada(s) nesta família."
        summaries.append({
            "family_key": family_key,
            "family": label,
            "state": state,
            "state_label": STATE_LABELS.get(state, "Em revisão"),
            "counts": totals,
            "reports": reports,
...
def _render_divergence_block(sample: dict[str, Any]) -> str:
    ...
    change = presentation.get("change") or "Há uma diferença para revisar antes de qualquer aplicação."
    return (
        "<article class=\"divergence\">"
        "<h5>" + esc(title) + "</h5>"
        "<p class=\"human-detail\">" + esc(change) + "</p>"
        "<details class=\"technical-details\"><summary>Detalhes técnicos</summary>"
        "<dl><dt>Identidade canônica</dt><dd><code>" + esc(sample.get("identity")) + "</code></dd>"
        "<dt>Comparação registrada</dt><dd><code>" + esc(sample.get("detail")) + "</code></dd>"
        "<dt>Origem física</dt><dd><code>" + esc(sample.get("provenance")) + "</code></dd>"
        "<dt>Verificação</dt><dd><code>" + esc(sample.get("report")) + "</code></dd>"
        "</dl></details></article>"
    )
...
def _render_grouped_divergences(model: dict[str, Any]) -> str:
    ...
    def family_total_text(summary: dict[str, Any]) -> str:
        ...
        classified = sum(int(value or 0) for value in (summary.get("counts") or {}).values())
        return f"Total real: {quantity(classified)} diferenças classificadas."
    ...
    for family, types in _grouped_samples(model):
        ...
        for type_key, type_label, samples in types:
            ...
            if remaining:
                more = (
                    ...
                    + str(min(PRESENTATION_PAGE_SIZE, len(remaining)))
                    + " exemplos desta página</button>"
                )
            type_total = int((summary.get("counts") or {}).get(type_key, 0))
            type_real = f"Total real neste tipo: {quantity(type_total)} registro(s) afetado(s)."
            ...
            type_html.append(
                ...
                "<span>" + str(len(samples)) + " exemplos disponíveis nesta página (não é o total)</span></summary>"
        ...
        groups.append(
            ...
            "<span>" + str(sum(len(items) for _, _, items in types))
            + " exemplos disponíveis nesta página (não é o total)</span></summary>" + "".join(type_html) + "</details>"
        )
    return "".join(groups) or "<p id=\"empty-state\">Não há diferenças para exibir nesta execução.</p>"
...
def _render_html(model: dict[str, Any]) -> str:
    ...
    def count_text(counts: dict[str, Any]) -> str:
        parts = [f"{TYPE_LABELS[key]}: {int(counts.get(key, 0))}" for key in DIVERGENCE_KINDS if int(counts.get(key, 0))]
        return " · ".join(parts) if parts else "Sem diferenças"

    family_rows = "\n".join(
        "<tr><td>" + esc(row["family"]) + "</td><td><span class=\"state " + _tag(row["state"]) + "\">"
        + esc(row.get("state_label")) + "</span></td><td>" + esc(count_text(row["counts"]))
        + "</td><td>" + esc(row["detail"]) + "</td></tr>"
        for row in model["family_summary"]
    )
    family_options = "<option value=\"all\">Todas</option>" + "".join(
        "<option value=\"" + esc(row["family"]) + "\">" + esc(row["family"]) + "</option>"
...
<style>
:root{{color-scheme:light;font-family:Segoe UI,Arial,sans-serif;color:#16202a;background:#f4f7fa}}body{{margin:0;background:#f4f7fa}}main{{max-width:1180px;margin:auto;padding:28px}}section{{background:#fff;border:1px solid #d8e1ea;border-radius:10px;padding:20px;margin:16px 0;box-shadow:0 1px 3px #16202a12}}h1{{margin:0 0 6px}}h2{{margin-top:0}}.lead,.muted{{color:#526372}}.notice{{border-left:4px solid #2563eb;padding:10px 12px;background:#eff6ff}}table{{width:100%;border-collapse:collapse}}th,td{{padding:10px;border-bottom:1px solid #e5eaf0;text-align:left;vertical-align:top}}th{{background:#f8fafc}}.state,.pill{{display:inline-block;border-radius:999px;padding:3px 8px;font-weight:600;font-size:.86em;background:#e5e7eb}}.review,.altered{{background:#fef3c7;color:#854d0e}}.observed,.ready{{background:#dcfce7;color:#166534}}.technical_issue,.invalid,.repeated{{background:#fee2e2;color:#991b1b}}.new{{background:#dbeafe;color:#1d4ed8}}.removed{{background:#fce7f3;color:#9d174d}}.filters{{display:flex;gap:12px;flex-wrap:wrap;margin:12px 0}}label{{font-weight:600}}select{{margin-left:6px;padding:6px}}.family-group,.type-group,.technical-details{{border:1px solid #d8e1ea;border-radius:8px;background:#fff;margin:10px 0}}.family-group>summary,.type-group>summary{{cursor:pointer;padding:13px;display:flex;justify-content:space-between;gap:12px;align-items:center}}.family-group>summary{{background:#f8fafc;font-size:1.04em}}.type-group{{margin:10px 12px}}.type-group>summary{{background:#fcfdff}}.sample-list{{padding:0 12px}}.divergence{{border-left:4px solid #d8e1ea;padding:12px;margin:10px 0;background:#fff}}.divergence h5{{margin:0 0 7px;font-size:1em}}.human-detail{{margin:0;line-height:1.45}}.technical-details{{margin:12px 0 0;background:#f8fafc}}.technical-details summary{{cursor:pointer;padding:9px 11px;font-weight:600}}.technical-details dl{{padding:0 11px 11px}}dl{{display:grid;grid-template-columns:180px 1fr;gap:8px;margin:0}}dt{{font-weight:700}}dd{{margin:0;overflow-wrap:anywhere}}code{{font-family:Consolas,monospace;font-size:.9em}}.show-more{{margin:0 12px 14px;padding:8px 11px;border:1px solid #2563eb;border-radius:6px;background:#fff;color:#1d4ed8;font-weight:600;cursor:pointer}}.hidden{{display:none!important}}ul{{padding-left:22px;overflow-wrap:anywhere}}@media(max-width:700px){{main{{padding:14px}}table{{font-size:.9em}}dl{{grid-template-columns:1fr}}.family-group>summary,.type-group>summary{{align-items:flex-start;flex-direction:column}}}}
</style></head><body><main>
<h1>Resultado da varredura</h1><p class=\"lead\">Veja primeiro o que mudou no jogo. Os códigos e demais dados de diagnóstico ficam recolhidos em cada item.</p>
<section><h2>Resumo</h2><p class=\"notice\"><b>Modo somente leitura:</b> esta execução não aplicou dados. O relatório foi montado a partir do resultado já salvo.</p>
<table><thead><tr><th>Dados do jogo</th><th>Situação</th><th>O que mudou</th><th>Leitura</th></tr></thead><tbody>{family_rows}</tbody></table></section>
<section><h2>Diferenças para revisar</h2><p class=\"muted\">Cada grupo informa primeiro o total real de entidades e diferenças. Os exemplos abaixo são apenas uma amostra paginada para a página abrir rápido; o resultado completo permanece preservado no arquivo técnico desta execução.</p>
<div class=\"filters\"><label>Dados do jogo <select id=\"family-filter\">{family_options}</select></label><label>Tipo de mudança <select id=\"type-filter\">{type_options}</select></label></div><p id=\"visible-count\"></p><div id=\"divergence-list\">{_render_grouped_divergences(model)}</div></section>
<section><details><summary><b>Rastreabilidade técnica da execução</b></summary><p>Execução: <code>{esc(model['execution']['execution_id'])}</code> · resultado salvo: <code>{esc(model['source_result']['name'])}</code>.</p><p>Contrato: <code>{esc(contract.get('contrato_id'))}</code> · versão do jogo: <code>{esc(contract.get('versao_jogo'))}</code> · fingerprint de contrato: <code>{esc(contract.get('fingerprint_contrato_sha256'))}</code> · fingerprint de fontes: <code>{esc(contract.get('fingerprint_fontes_sha256'))}</code>.</p><p>Gate de revisão: <code>{esc(review_gate.get('state'))}</code> · aplicação habilitada: <code>{esc(review_gate.get('application_enabled'))}</code>.</p><h3>Fontes declaradas</h3><ul>{source_rows}</ul><h3>Artefatos desta execução</h3><ul>{artifacts}</ul><p class=\"muted\">Manifesto: <code>{esc(model['execution']['manifest'])}</code>.</p></details></section>
</main><script>
const PAGE_SIZE={PRESENTATION_PAGE_SIZE};
function filterDivergences(){{const family=document.getElementById('family-filter').value;const type=document.getElementById('type-filter').value;let groups=0;document.querySelectorAll('.family-group').forEach(function(group){{const familyMatch=family==='all'||group.dataset.family===family;let typeMatch=false;group.querySelectorAll('.type-group').forEach(function(typeGroup){{const visible=type==='all'||typeGroup.dataset.type===type;typeGroup.classList.toggle('hidden',!visible);if(visible)typeMatch=true;}});const visible=familyMatch&&typeMatch;group.classList.toggle('hidden',!visible);if(visible)groups++;}});document.getElementById('visible-count').textContent=groups+' grupo(s) compatível(eis) com o filtro.';}}
function revealMore(button){{const group=button.closest('.type-group');const template=group.querySelector('template.more-items');const list=group.querySelector('.sample-list');const items=Array.from(template.content.querySelectorAll('.divergence')).slice(0,PAGE_SIZE);items.forEach(function(item){{list.appendChild(item);}});const left=template.content.querySelectorAll('.divergence').length;if(!left){{button.remove();}}else{{button.textContent='Ver mais '+Math.min(PAGE_SIZE,left)+' de '+left+' exemplo(s)';}}}}
