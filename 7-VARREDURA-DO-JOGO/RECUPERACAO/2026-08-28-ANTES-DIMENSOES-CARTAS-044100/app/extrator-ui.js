...
(function installUi(global) {
  ...
  async function refreshMetadataAutomatically(trigger = 'abertura automática') {
    ...
    try {
      ...
      const dimensionSourceBytes = Object.fromEntries(METADATA_SOURCE_ROLES.map((role) => [role, state.sources[role].bytes]));
      const dimensionSourceDescriptors = Object.fromEntries(METADATA_SOURCE_ROLES.map((role) => [role, sourceDescriptor(role)]));
      const dimensionSnapshot = await core.extractCardDimensionsByFamily(
        dimensionSourceBytes,
        dimensionSourceDescriptors,
      ...
      const manifest = await core.sealManifest({
        ...
        text_validation: textDiff.validation, text_source: sourceDescriptor('dt261_bra'), text_baseline: { records: baseline.records, sha256: baseline.sha256, transaction_read_only: true, schema_ready: baseline.schema_ready },
        card_dimensions_validation: dimensionReadback, summary
      });
      state.metadata = { reference, baseline, textCatalog, textItems, dimensionReadback, diff, summary, manifest, selection: new Set(textItems.map((item) => item.key)) };
      ...
      renderAutomaticSummary();
      log('log-metadata', `Concluído: ${textDiff.current} chaves oficiais e ${dimensionStructure.cards} cartas com país, clube, liga e tipo conferidos; nenhuma escrita foi feita.`);
    } catch (error) {
