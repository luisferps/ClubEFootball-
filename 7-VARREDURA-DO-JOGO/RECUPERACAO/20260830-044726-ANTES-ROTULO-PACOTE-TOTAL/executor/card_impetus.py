...
def resolve_impetus_presentation_label(reading_contract: dict[str, Any], code: int | None) -> dict[str, Any]:
    """Resolve um rótulo somente por FK explícita para o ``all.str`` físico.
...
def slot_sources_from_contract(reading_contract: dict[str, Any]) -> dict[int, str]:
    """Resolve endereços publicados exclusivamente do pedido versionado."""
    if not isinstance(reading_contract, dict):
        raise ValueError("pedido de leitura ausente para o readback de Ímpetos")
    files = {item.get("arquivo_id"): item for item in reading_contract.get("arquivos", []) if isinstance(item, dict)}
    fields = {item.get("chave_campo"): item for item in reading_contract.get("campos", []) if isinstance(item, dict)}
    sources: dict[int, str] = {}
    for slot, key in ((1, "carta.impeto.slot1"), (2, "carta.impeto.slot2")):
        field = fields.get(key)
        file = files.get(field.get("arquivo_id")) if isinstance(field, dict) else None
        if not isinstance(field, dict) or not isinstance(file, dict):
            raise ValueError(f"o pedido de leitura não contém {key}")
        bit, width, name = field.get("bit_inicio"), field.get("largura_bits"), file.get("arquivo")
        if not isinstance(bit, int) or not isinstance(width, int) or not isinstance(name, str) or not name:
            raise ValueError(f"o contrato de {key} não está tipado")
        sources[slot] = f"{name} bit{bit}/w{width}"
    return sources
...
def apply_canonical_slot_projection(rows: dict[str, dict[str, str]], connection: Any, reading_contract: dict[str, Any]) -> dict[str, Any]:
    """Publica slots apenas da relação detalhada já reconciliada com Player.bin w10.

    Isto não participa da validação física: ``validate_physical_slot_projection``
    continua recebendo o CSV do jogo como origem independente.
    """
    slot_sources_from_contract(reading_contract)
    projection, relation = _relation_projection(connection)
    changed_cards = 0
    changed_fields = {field: 0 for field in SLOT_COLUMNS}
    changes: list[str] = []
    for card_id, row in rows.items():
        canonical = projection.get(card_id, EMPTY)
        changed = False
        for field in SLOT_COLUMNS:
            old, new = _text(row.get(field)).lower(), canonical[field]
            if old != new:
                changed = True
                changed_fields[field] += 1
                changes.append(f"{card_id}|{field}|{old}|{new}")
            row[field] = new
        changed_cards += int(changed)
    return {
        "source": "clube_novo.carta_impeto_jogo (reconciliada com Player.bin w10)",
        "relation_rows": len(relation), "changed_cards_from_stored_summary": changed_cards,
        "changed_fields_from_stored_summary": changed_fields,
        "stored_difference_sha256": hashlib.sha256("\n".join(sorted(changes)).encode("utf-8")).hexdigest(),
        "projection_difference": 0,
    }


def validate_physical_slot_projection(csv_text: str, connection: Any, reading_contract: dict[str, Any]) -> dict[str, Any]:
    """Compara fonte física w10 ao banco, nunca usa banco como entrada."""
    slot_sources = slot_sources_from_contract(reading_contract)
...
def readback_card_slots(connection: Any, card_ids: list[str], csv_text: str, reading_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"contract": CONTRACT, "transaction_read_only": True, "database_write": False, "consumer_enabled": False, "slot_source": "Player.bin w10 via fotografia física", "cards": results, "missing_card_ids": missing_database, "database_relation_slot_mismatches": mismatches}
