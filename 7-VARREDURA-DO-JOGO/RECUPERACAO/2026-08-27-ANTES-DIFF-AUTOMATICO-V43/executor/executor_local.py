...
def prepare_card_package(payload: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not selection["items"]:
        return {"no_changes": True, "package_type": package_type, "summary": summary, "preflight": preflight, "target": target, "write_enabled": False}
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/status":
            ...
            self.send_json(HTTPStatus.OK, {
                ...
                "mode": "manual-confirmation" if manual_card_apply_allowed(config) else "dry-run",
                "write_enabled": False,
                "manual_apply_available": manual_card_apply_allowed(config),
                "database_configured": bool(connection_string()),
                "connection_source": connection_source(),
                "target": "clube_novo.carta_jogo",
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path == "/api/prepare":
                ...
                return
            if path == "/api/apply":
                if not manual_card_apply_allowed(config):
                ...
                if approved.get("selection", {}).get("kind") != "cards":
                    raise PermissionError("somente o pacote validado de cartas pode usar o envio manual")
                ...
                application_manifest = {
                    ...
                    "target": {"schema": "clube_novo", "table": "carta_jogo", "preserved_schema": "clube"},
