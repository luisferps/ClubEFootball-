...
(function installUi(global) {
  ...
  function updateMetadataApplySummary() {
    ...
    $('apply-summary-metadata').textContent = selected.length
      ? `${selected.length} item(ns) em conferência; aplicação permanece bloqueada sem adaptador canônico.`
      : 'Nenhum catálogo será aplicado automaticamente.';
  ...
  async function refreshMetadataAutomatically(trigger = 'abertura automática') {
    ...
    renderAutomaticSummary();
    log('log-metadata', 'Verificando referência interna e identidades das fontes…', true);
    try {
      ...
      const summary = Object.fromEntries(Object.entries(reference.summary || {}).map(([catalog, item]) => {
        const affected = (item.source_roles || []).some((role) => changedRoles.has(role));
      ...
      const manifest = await core.sealManifest({ contract: core.CONTRACT_VERSION, mode: 'metadata_diff', execution_id: core.makeExecutionId(), generated_at: new Date().toISOString(), expires_at: core.expirationFromNow(60), database_write: false, source_authority: 'referência interna versionada + identidades físicas separadas por família', reference_id: reference.reference_id, source_matches_reference: reference.source_matches_current_reference, changed_roles: [...changedRoles], counts: { new: totals.new, changed: totals.changed, possibly_inactive: totals.absent }, selection_contract: { algorithm: 'sha256/canonical-json', items: [] }, summary });
      state.metadata = { reference, diff, summary, manifest, selection: new Set() };
      ...
      $('metadata-support').innerHTML = Object.entries(summary).map(([name, item]) => item.status === 'nao_suportado_nesta_atualizacao'
        ...
        : item.status === 'bloqueado_fonte_alterada'
          ...
          : `<div class="support-item"><b>${escapeHtml(name)}</b> — ${escapeHtml(item.status === 'comparado' ? 'Tudo atualizado; fonte física igual à referência interna vigente.' : item.reason || item.status)}</div>`).join('');
      renderMetadataReview(); $('result-metadata').hidden = false;
      const blocked = [...changedRoles].length > 0;
      const changes = totals.new + totals.changed + totals.absent;
      if (blocked) {
        setOperationStatus('metadata-status', 'error', 'Metadados bloqueados com segurança', 'Uma fonte mudou; a referência anterior foi preservada e nenhum catálogo foi promovido.');
        state.automatic.metadata = { state: 'error', pending: false, message: `Fonte alterada: ${[...changedRoles].join(', ')}. Famílias afetadas permanecem bloqueadas.` };
      } else {
      ...
      renderAutomaticSummary();
      log('log-metadata', 'Concluído: comparação automática encerrada; nenhum catálogo foi aplicado.');
    } catch (error) {
