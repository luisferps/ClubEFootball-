...
(function installCore(global) {
  ...
  function fixedUtf8(bytes, offset, length) {
    let end = offset;
    const limit = Math.min(bytes.length, offset + length);
    while (end < limit && bytes[end] !== 0) end += 1;
    return TD.decode(bytes.subarray(offset, end));
  }
  ...
  async function extractMetadataByFamily(sourceBytes, sourceDescriptors, log = () => {}) {
    ...
    const coachPacked = cpks.dt870_updated['Coach.bin'];
    if (!coachPacked) throw new Error('Coach.bin não foi encontrado no DT870 da atualização.');
    const coachRaw = await unpackWesys(coachPacked);
    if (!coachRaw.length || coachRaw.length % 176 !== 0) throw new Error('Coach.bin atual não respeita registros de 176 bytes.');
    const coachStyles = [
      ['possessionGame', 206],
      ['longBallCounter', 238],
      ['quickCounter', 224],
    ...
    const coachRecords = [];
    for (let offset = 0; offset < coachRaw.length; offset += 176) {
      const proficiencias = Object.fromEntries(coachStyles.map(([codigo, bit]) => [codigo, readBits(coachRaw, offset, bit, 7)]));
      const boosts = [160, 148].map((bit, index) => {
        const encoded = readBits(coachRaw, offset, bit, 5);
        return encoded === 0 ? null : { ordem: index + 1, atributo_idx_canonico: encoded - 1, delta: 1, bit, largura: 5 };
      }).filter(Boolean);
      const record = {
        id: u64String(coachRaw, offset),
        nome_jp: fixedUtf8(coachRaw, offset + 32, 46),
        nome_en: fixedUtf8(coachRaw, offset + 78, 46),
        nome_cn: fixedUtf8(coachRaw, offset + 124, 52),
        proficiencias,
        boosts,
        source_role: 'dt870_updated',
        record_index: offset / 176
      };
      coachRecords.push({ ...record, fingerprint: stableJson(record) });
    }
    const tecnicos = {
      supported: true,
      file: 'Coach.bin',
      source_policy: 'DT870 da atualização é autoritativo para identidade atual, cinco proficiências e até dois boosts',
      record_size: 176,
      id_contract: 'u64 little-endian no bit 0',
      physical_contract: {
        names: 'UTF-8 nos bytes 32/78/124',
        proficiencies: Object.fromEntries(coachStyles.map(([codigo, bit]) => [codigo, { bit, width: 7 }])),
        boost_slots: [{ order: 1, bit: 160, width: 5 }, { order: 2, bit: 148, width: 5 }],
        boost_decode: 'zero=ausente; valor físico-1=índice canônico; delta comprovado +1'
      },
      records: coachRecords,
      duplicate_ids: duplicateIds(coachRecords, 'id'),
      blocked_fields: {
        sobreposicao: 'catálogo textual existe, mas o campo de proficiência ainda não foi localizado no Coach.bin',
        link_up: 'CoachLink.bin localizado, porém sem semântica e cardinalidade integralmente comprovadas'
      }
    };
    if (tecnicos.duplicate_ids.length) throw new Error('Coach.bin atual contém IDs de técnico duplicados.');
    log(`técnicos · DT870 da atualização: ${tecnicos.records.length}; cinco proficiências e boosts normalizados`);
  ...
  async function extractMetadataFromCpk(bytes, source, log = () => {}) {
    ...
    const specs = {
      ...
      tecnicos: { file: 'Coach.bin', record_size: 176, id: (buffer, offset) => u64String(buffer, offset), id_contract: 'u64 little-endian no bit 0' }
    };
    const catalogs = {};
