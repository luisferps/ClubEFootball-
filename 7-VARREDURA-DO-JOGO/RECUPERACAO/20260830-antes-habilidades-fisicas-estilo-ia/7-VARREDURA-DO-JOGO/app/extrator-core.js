...
(function installCore(global) {
  ...
  async function extractCardRelationsByContract(bytes, readingContract) {
    ...
    return new Map(decoded.records.map((record) => {
      ...
      const habilidades_fisicas = skillFields.map((field, ordem) => ({
        campo_id: field.campo_id, skill_id: Number(field.transformacao?.skill_id),
        bit: field.bit_inicio, largura: field.largura_bits, ativo: true, ordem,
        registro: record.index, arquivo: file.arquivo, hash: decoded.sha256 || null,
        procedencia: field.proveniencia_mapa_assunto || null
      }));
  ...
  async function extractCardsFromCpk(bytes, readingContract, log = () => {}) {
    ...
    for (const card of cards) {
      ...
      if (String(card.card_id) === '84') log(`diag habilidade 84 relations=${Array.isArray(relations.habilidades_fisicas) ? relations.habilidades_fisicas.length : 'ausente'}`);
      card.skills = relations.skills; card.habilidades_fisicas = relations.habilidades_fisicas; card.ai_styles = relations.ai; card.aptitudes = relations.aptitudes;
      if (!card._diag_relacoes && cards.length) card._diag_relacoes = { relations_chamada: true, tem_chave: Object.prototype.hasOwnProperty.call(relations, 'habilidades_fisicas'), tamanho_relations: Array.isArray(relations.habilidades_fisicas) ? relations.habilidades_fisicas.length : null, tamanho_card: Array.isArray(card.habilidades_fisicas) ? card.habilidades_fisicas.length : null };
      if (String(card.card_id) === '84') log(`diag habilidade 84 card=${Array.isArray(card.habilidades_fisicas) ? card.habilidades_fisicas.length : 'ausente'}`);
    ...
    cards.diagnostico_habilidades_fisicas = { processadas: cards.length, relations_array: cards.filter((card) => Array.isArray(card.habilidades_fisicas)).length, nao_vazias: cards.filter((card) => Array.isArray(card.habilidades_fisicas) && card.habilidades_fisicas.length).length, primeira: cards[0] ? { card_id: String(cards[0].card_id), tamanho: Array.isArray(cards[0].habilidades_fisicas) ? cards[0].habilidades_fisicas.length : null } : null };
    return cards;
  }
