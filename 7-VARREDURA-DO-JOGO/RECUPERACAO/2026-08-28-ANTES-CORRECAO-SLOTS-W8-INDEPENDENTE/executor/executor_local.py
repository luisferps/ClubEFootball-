...
from card_impetus import apply_normalized_slot_projection, readback_card_slots, validate_physical_slot_projection
...
def current_card_baseline(config: dict[str, Any]) -> tuple[bytes, dict[str, Any]]:
    ...
    with psycopg.connect(dsn, connect_timeout=20) as connection:
        ...
        slot_projection = apply_normalized_slot_projection(rows, connection)
    ...
    return data, {
        "source": "clube_novo.carta_jogo + clube_novo.carta_impeto_jogo",
        "records": len(rows),
...
def current_card_impetus_validation(csv_text: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    with psycopg.connect(dsn, connect_timeout=20) as connection:
        connection.read_only = True
        return validate_physical_slot_projection(csv_text, connection)


def current_card_impetus_readback(card_ids: list[str], config: dict[str, Any]) -> dict[str, Any]:
    ...
    with psycopg.connect(dsn, connect_timeout=20) as connection:
        connection.read_only = True
        return readback_card_slots(connection, card_ids)
...
def prepare_card_package(payload: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    if package_type == "incremental":
        ...
        with psycopg.connect(dsn, connect_timeout=20) as connection:
            ...
            apply_normalized_slot_projection(target_rows, connection)
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/card-baseline/current.csv":
            try:
                ...
                self.send_header("X-Clubef-Slot-Projection", "clube_novo.carta_impeto_jogo")
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path == "/api/card-impetus/validate":
                ...
                result = current_card_impetus_validation(csv_text, config)
                self.send_json(HTTPStatus.OK if result.get("passed") else HTTPStatus.CONFLICT, result)
                return
            if path == "/api/card-impetus/readback":
                ...
                self.send_json(HTTPStatus.OK, current_card_impetus_readback(card_ids, config))
                return
