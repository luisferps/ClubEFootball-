...
async function runOnce(index) {
  ...
  assert(baseline.headers.get('x-clubef-slot-projection') === 'clube_novo.carta_impeto_jogo', 'baseline não declarou a relação canônica');
  assert(baseline.headers.get('x-clubef-slot-projection-differences') === '0', 'projeção resumida ainda diverge da relação normalizada');
  assert(baseline.headers.get('x-clubef-stored-slot-differences') === '270', 'diagnóstico das 270 projeções antigas mudou');
  ...
  let projectedSlotDifferences = 0;
  for (const row of projectedRows) {
    ...
    for (const field of ['impeto_s1', 'impeto_s2_cond', 'vaga_s1', 'vaga_s2']) {
      if (String(row[field] ?? '') !== String(physicalRow[field] ?? '')) projectedSlotDifferences += 1;
    }
  }
  assert(projectedSlotDifferences === 0, `baseline projetado diverge do arquivo físico em ${projectedSlotDifferences} campos`);
  const cardImpetus = await postJson(`${baseUrl}/api/card-impetus/validate`, { card_csv: CLUBEF_CORE.rowsToCsv(rows) });
  assert(cardImpetus.passed && cardImpetus.differences === 0 && cardImpetus.relation_rows === 3748, 'slots físicos não bateram com carta_impeto_jogo');
  const sampleIds = [
  ...
  const impetusReadback = await postJson(`${baseUrl}/api/card-impetus/readback`, { card_ids: sampleIds });
  ...
  const typeSet = new Set(allSlots.filter(item => item.slot.receita).map(item => Number(item.slot.receita.tipo_raw)));
  for (const type of [0, 1, 2, 3, 5]) assert(typeSet.has(type), `classe/tipo de condição ${type} sem amostra funcional`);
  const criteriaSet = new Set(allSlots.filter(item => item.slot.receita).map(item => item.slot.receita.criterio));
  for (const criterion of ['sempre_ativo', 'avaliacao_ao_vivo', 'quantidade_jogadores_nacionalidade_regiao', 'quantidade_jogadores_liga_categoria', 'quantidade_jogadores_classe_impeto']) {
    assert(criteriaSet.has(criterion), `classe funcional ${criterion} sem amostra`);
  }
  const messi = impetusReadback.cards.find(card => card.card_id === '106781476920663');
  const messiArgentina = messi.slots.find(slot => slot.codigo_impeto === 334);
  assert(messiArgentina.ativacao === 'condicional' && messiArgentina.receita.alvos.nacionalidades.some(item => item.codigo_nacionalidade === 144), 'Messi/Argentina sem receita correta');
  assert(messi.slots.some(slot => slot.ativacao === 'sempre_ativo'), 'Messi sem amostra de slot sempre ativo');
  const neymar = impetusReadback.cards.find(card => card.card_id === '106755438714272');
  const neymarLeague = neymar.slots.find(slot => slot.codigo_impeto === 170);
  assert(neymarLeague.ativacao === 'condicional' && neymarLeague.receita.alvos.membros_liga.length === 2, 'Neymar sem as duas ligas brasileiras');
  assert(neymarLeague.receita.faixas.length > 0 && neymarLeague.receita.efeitos.length > 0, 'Neymar sem faixas/efeitos da receita');
...
(async () => {
  ...
  console.log(JSON.stringify({
    ...
    impetus: {
      ...
      slot_projection_differences: first.evidence.cardImpetus.differences,
