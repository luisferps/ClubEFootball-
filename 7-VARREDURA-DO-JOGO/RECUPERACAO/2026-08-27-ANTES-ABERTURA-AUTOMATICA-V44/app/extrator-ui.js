...
(function installUi(global) {
  ...
  function updateRunAvailability() {
    ...
    if (!state.executor.online || !state.executor.database_configured) {
      $('incremental-status').textContent = 'A comparação aguarda a conexão segura de leitura com clube_novo.';
    } else if (!operationIsReady(CARD_SOURCE_ROLES)) {
      $('incremental-status').textContent = 'A comparação aguarda o DT870 da atualização.';
    } else if (!state.incremental && !state.incrementalRunning) {
      $('incremental-status').textContent = 'Tudo pronto. A comparação será iniciada automaticamente.';
    }
  ...
  async function refreshSources() {
    ...
    $('incremental-status').textContent = 'Atualizando as fontes e preparando uma nova comparação…';
  ...
  async function maybeStartIncrementalComparison(trigger) {
    ...
    if (!$('pane-incremental').classList.contains('active')) return;
  ...
  async function runIncrementalComparison(trigger) {
    if (state.incrementalRunning) return;
    if (!(state.executor.online && state.executor.database_configured)) {
      $('incremental-status').textContent = 'COMPARAÇÃO BLOQUEADA — não foi possível ler a base atual de clube_novo.';
      return;
    }
    if (!operationIsReady(CARD_SOURCE_ROLES)) {
      $('incremental-status').textContent = 'COMPARAÇÃO BLOQUEADA — o DT870 da atualização ainda não está disponível.';
      return;
    }
    ...
    $('incremental-status').textContent = `Comparação iniciada automaticamente (${trigger}). Aguarde o resumo aparecer abaixo.`;
    $('result-incremental').hidden = true;
    $('apply-card-package').hidden = true;
    ...
    try {
      ...
      $('incremental-status').textContent = changes
        ? 'Comparação concluída. Confira os números e use o botão seguinte para preparar o envio.'
        : 'Comparação concluída sem mudanças. Nenhum envio é necessário.';
      log('log-incremental', 'Concluído: somente o diff foi preparado; nenhum dado foi aplicado.');
      $('result-incremental').scrollIntoView({ behavior: 'smooth', block: 'start' });
    } catch (error) {
      ...
      $('result-incremental').hidden = false;
      $('incremental-status').textContent = 'A comparação terminou bloqueada; nenhum dado foi enviado.';
      $('result-incremental').scrollIntoView({ behavior: 'smooth', block: 'start' });
    } finally {
  ...
  $('prepare-card-package').addEventListener('click', async () => {
    ...
    try {
      ...
      $('apply-button-card-package').disabled = true;
    } finally { button.disabled = !state.cardPackage || !state.executor.online || state.cardPackage.no_changes; }
  });
