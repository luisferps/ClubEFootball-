...
RELATIONS = (
    ...
    RelationSpec("habilidades", "carta_habilidade_jogo", ("card_id", "skill_id", "ordem"), ("str", "int", "int"), "cartas-fisicas-canonicas.json"),
...
def _canonical_skill_rows(cards: Sequence[dict[str, Any]], reading_contract: Mapping[str, Any]) -> tuple[list[tuple[Any, ...]], dict[tuple[Any, ...], dict[str, Any]]]:
    ...
    for card in cards:
        ...
        for sequence, (physical_order, skill_id, bit, width, member, mapping) in enumerate(sorted(active)):
            row = (card_id, skill_id, sequence)
            result.append(row)
