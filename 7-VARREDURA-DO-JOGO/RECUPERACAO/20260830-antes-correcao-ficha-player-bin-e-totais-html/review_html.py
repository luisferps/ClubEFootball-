        ...
        for type_key, type_label, samples in types:
            ...
            if remaining:
                more = (
                    ...
                    + " de " + str(len(remaining)) + " exemplo(s)</button>"
                )
            type_html.append(
                "<details class=\"type-group\" data-type=\"" + esc(type_key) + "\">"
                "<summary><span class=\"pill " + esc(_tag(type_key)) + "\">" + esc(type_label) + "</span>"
                "<span>" + str(len(samples)) + " exemplo(s) na página</span></summary>"
                "<div class=\"sample-list\">" + visible_blocks + "</div>" + more + "</details>"
            )
        groups.append(
            "<details class=\"family-group\" data-family=\"" + esc(family) + "\">"
            "<summary><strong>" + esc(family) + "</strong><span>" + str(sum(len(items) for _, _, items in types))
            + " exemplo(s) nesta página</span></summary>" + "".join(type_html) + "</details>"
        )
...
<section><h2>Diferenças para revisar</h2><p class=\"muted\">Os grupos começam fechados. Cada tipo mostra 10 exemplos e libera os próximos sob demanda; o resultado completo permanece preservado no arquivo técnico desta execução.</p>
