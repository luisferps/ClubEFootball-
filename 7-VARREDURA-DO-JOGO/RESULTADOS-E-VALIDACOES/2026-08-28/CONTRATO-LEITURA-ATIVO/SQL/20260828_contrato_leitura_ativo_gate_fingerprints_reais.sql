-- Um fingerprint sentinela é permitido apenas no rascunho, nunca na execução ativa.
...
begin
  if new.estado = 'ativo' then
    if new.fingerprint_contrato_sha256=repeat('0',64) or new.fingerprint_fontes_sha256=repeat('0',64) then
      raise exception 'contrato não pode ativar: fingerprint sentinela não é aceito';
    end if;
    if not new.cobertura_total then raise exception 'contrato não pode ativar: cobertura integral não confirmada'; end if;
    if exists (select 1 from clube_novo.contrato_leitura_campo f where f.contrato_id=new.contrato_id and f.ativo and f.status_prova not in ('comprovado','convencao_aprovada')) then
      raise exception 'contrato não pode ativar: há campo ativo sem prova física nem convenção aprovada rastreável';
    end if;
    if exists (select 1 from clube_novo.contrato_leitura_cadeia c where c.contrato_id=new.contrato_id and c.requer_selo_contrato and c.estado <> 'conforme') then
...
set search_path = clube_novo, pg_temp
as $$
declare contract_row clube_novo.contrato_leitura_jogo%rowtype; result jsonb;
begin
  select * into contract_row from clube_novo.contrato_leitura_jogo where estado='ativo' and cobertura_total order by ativado_em desc nulls last,criado_em desc limit 1;
  if not found then raise exception 'contrato de leitura ativo e integral não encontrado'; end if;
  if contract_row.fingerprint_contrato_sha256=repeat('0',64) or contract_row.fingerprint_fontes_sha256=repeat('0',64) then raise exception 'contrato ativo inválido: fingerprint sentinela'; end if;
  if exists (select 1 from clube_novo.contrato_leitura_campo f where f.contrato_id=contract_row.contrato_id and f.ativo and f.status_prova not in ('comprovado','convencao_aprovada')) then raise exception 'contrato ativo inválido: campo sem prova física nem convenção aprovada'; end if;
  if exists (select 1 from clube_novo.contrato_leitura_cadeia c where c.contrato_id=contract_row.contrato_id and c.requer_selo_contrato and c.estado <> 'conforme') then raise exception 'contrato ativo inválido: cadeia satélite não conforme'; end if;
  select jsonb_build_object(
    'contrato_id',contract_row.contrato_id,'versao_jogo',contract_row.versao_jogo,'versao_contrato',contract_row.versao_contrato,
    'fingerprint_contrato_sha256',contract_row.fingerprint_contrato_sha256,'fingerprint_fontes_sha256',contract_row.fingerprint_fontes_sha256,
