-- Gate específico do cálculo: estado físico e efeitos comprovados, não tradução inglesa.
create or replace function build_editor.adicionais_v1(p_pos integer) returns table(codigo integer)
language sql stable set search_path='' as $$
 ...
 having count(*) between 1 and 4 and bool_and(a.delta=1 and a.status_validacao='comprovado_biblioteca_dt870')
   and (p_pos=0 or not bool_or(o.indice_otimizador>=21));
$$;

create or replace function build_editor.avaliar_v1(p_entrada jsonb) returns jsonb
