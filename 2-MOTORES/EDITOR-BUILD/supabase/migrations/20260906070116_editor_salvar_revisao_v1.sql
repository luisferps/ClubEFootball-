...
begin
 ...
 if p_id is null then
   ...
   update build_editor.build_pessoal set nome=btrim(p_nome),pedido_id=p_pedido_id,entrada=p_entrada,resultado=salvar_v1.resultado,
     motor_versao=salvar_v1.resultado->>'motor',revisao=revisao+1,atualizado_em=now()
...
create policy somente_dono on build_editor.recibo for select to authenticated using(usuario_id=(select auth.uid()));
