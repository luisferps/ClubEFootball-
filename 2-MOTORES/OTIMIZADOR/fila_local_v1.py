...
def _identidade_estado_local(lote_id: str) -> str:
    """Identidade estável da máquina para separar estados portáteis copiados.

    A fotografia da fila pode viajar entre computadores; uma reserva ou um
    arquivo de envio nunca deve acompanhá-la como se pertencesse ao destino.
    O valor é derivado localmente e não é escrito no pacote de entrada.
    """
    material = "|".join((str(lote_id), platform.node(), str(uuid.getnode()), os.environ.get("COMPUTERNAME", "")))
...
class PacoteLocalV1:
    ...
    def _resolver_pasta_estado(self) -> Path:
        ...
        if self.pasta.parent.name == PASTA_PACOTE_PORTATIL:
            # Mesmo que alguém copie a pasta OTIMIZADOR inteira por pendrive,
            # a máquina de destino recebe outro diretório de estado. Assim não
            # herda uma reserva, spool ou envio pendente do computador origem.
            return (self.pasta.parent.parent / "runtime" / "fila-local" /
                    self.lote_id / _identidade_estado_local(self.lote_id))
        return self.pasta
...
class WorkerFilaLocalV1:
    ...
    def _identidade_da_maquina(self) -> str:
        ...
        return _identidade_estado_local(self.lote_id)
