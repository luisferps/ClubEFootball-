...
def recompor(runner, snapshot_carta, linha, anterior, habilidades):
    ...
    vetadas = {int(h) for h, v in M.HAB.items() if v.get('vetada')}
    ...
    if abs(nota-float(anterior['pontuacao']))>1e-7:
        raise ValueError('Pontuacao mudou: encaminhar para busca integral')
    ...
    try:
        ...
        result = runner.trabalha({'n':linha['linha_id'],'card_id':card_id,'funcao_id':fid,
          ...
          'origem':'correcao_pontual_habilidades_v12'})
    finally:
    ...
    result['correcao_pontual']={'contrato':'correcao_habilidades_sem_nova_busca_v12',
