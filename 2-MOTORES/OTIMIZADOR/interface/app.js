...
async function api(c,tempoLimite=15000){const controlador=new AbortController(),temporizador=window.setTimeout(()=>controlador.abort(),tempoLimite);try{const r=await fetch(c,{cache:'no-store',signal:controlador.signal});let d;try{d=await r.json();}catch(_){throw new Error('O servidor local devolveu uma resposta inválida.');}if(!r.ok)throw new Error(d.erro||'Consulta local indisponível');return d;}catch(e){if(e&&e.name==='AbortError')throw new Error('A consulta local demorou mais de '+Math.ceil(tempoLimite/1000)+' s.');throw e;}finally{window.clearTimeout(temporizador);}}
...
let temporizadorTemposFila=null;
let temporizadorFila=null;
let atualizandoFila=false;
let carregandoLinhasFila=false;
let linhasFilaCarregando=false;
let erroLinhasFila='';
let carregandoApoiosFila=false;
let proximaLeituraApoiosFila=0;
...
function resumoSaudeLocal(s){if(!s||s.ok!==true)return 'Servidor local indisponível';let resumo=s.worker_ativo===true?'Worker ativo':'Servidor local ativo';const segundos=Number(s.worker_decorrido_segundos);if(s.worker_ativo===true&&Number.isFinite(segundos))resumo+=' · há '+duracaoLegivel(segundos*1000);return resumo;}
function pintarSaudeLocal(s){estadoLocal=s||null;const ligado=!!(s&&s.ok===true);const titulo=ligado?(s.worker_ativo===true?'Worker ativo':'Painel conectado'):'Servidor indisponível';$('#estado').textContent=titulo;$('#estado').title=ligado?((s.contrato||'')+(s.versao_interface?' · interface '+s.versao_interface:'')):'Não foi possível falar com o servidor local';$('#estado').className='selo '+(ligado?'ok':'erro');const detalhe=ligado?resumoSaudeLocal(s)+' — fechar esta janela só esconde o painel; use o ícone do Otimizador perto do relógio para reabrir.':'Não abra outra cópia nem inicie a fila até o servidor local voltar a responder.';$('#estado-local').textContent=detalhe;}
...
function renderizarLinhasFila(){
  if(erroLinhasFila&&linhasFilaAtuais.length===0){
    $('#resumo-linhas').textContent=erroLinhasFila;
    $('#pagina-anterior-fila').disabled=true;
    $('#proxima-pagina-fila').disabled=true;
    linhaVazia('#linhas-fila',erroLinhasFila,10);
    return;
  }
  if(linhasFilaCarregando&&linhasFilaAtuais.length===0){
    $('#resumo-linhas').textContent='Carregando linhas da fila em segundo plano…';
    $('#pagina-anterior-fila').disabled=true;
    $('#proxima-pagina-fila').disabled=true;
    linhaVazia('#linhas-fila','Carregando as linhas. O estado, os controles e a linha ativa já estão atualizados.',10);
    return;
  }
  ...
  $('#linhas-fila').innerHTML=linhas.map(x=>{
    return '<tr data-linha-id="'+seguro(x.linha_id)+'" class="'+classeLinhaFila(x.estado)+'"><td class="indice-fila" title="Posição atual da fila">'+seguro(x._ordem_visual)+'</td><td>'+seguro(x.carta_rotulo||x.carta_nome||'Carta sem nome no catálogo')+'</td><td>'+seguro(x.funcao_rotulo||'Função sem catálogo')+'</td><td>'+seguro(x.posicao_rotulo||'Posição sem catálogo')+'</td><td>'+seguro(x.impeto_condicional_rotulo||'Sem ímpeto condicional')+'</td><td>'+seguro(rotuloEstado(x.estado))+'</td><td>'+tempoNaFila(x)+'</td><td>'+buildsComparadasCompactas(x)+'</td><td>'+buildsPossiveisCompactas(x)+'</td><td>'+seguro(texto(x.motivo||x.erro))+'</td></tr>';
...
function pintarFila(d){
  ...
  $('#fila-aviso').textContent=!bancoDisponivel?(d.mensagem||'Reconectando ao banco sem enviar comandos.'):workerLocalAusente?(d.acoes&&d.acoes.recuperar===true?'Uma linha ficou reservada sem worker local. Clique em Recuperar fila para devolvê-la a pendente e deixar a rodada pausada; as concluídas não serão tocadas.':'A fila tem uma linha reservada, mas este computador não tem worker local ativo. Nenhuma nova linha será calculada até a recuperação segura.'):reassumirLocal?'A rodada continua marcada como Rodando, mas este computador não possui worker ativo. Clique em Retomar worker local para continuar somente as pendências sem duplicar uma linha.':(d.mensagem||mensagemDaRodada(estado,t));
  ...
  if(!d.disponivel){linhasFilaCarregando=false;linhasFilaAtuais=[];renderizarLinhasFila();return;}
  linhasFilaCarregando=d.linhas_carregando===true&&linhasFilaAtuais.length===0;
  if(!linhasFilaCarregando){
  ...
  }else if(linhasFilaAtuais.length===0){
    paginacaoFila=d.paginacao||{total:0,offset:0,limite:TAMANHO_PAGINA};
  }
...
function abrirBuildCampea(linhaId){const x=buildsCampeasPorLinha.get(String(linhaId));if(!x)return;const barras=barrasCompactas(x),habilidades=(x.habilidades_adicionais_rotulo||[]);$('#build-campea-aviso').textContent=(x.carta_rotulo||x.carta_nome||'Carta sem nome no catálogo')+' · '+(x.funcao_rotulo||'Função sem catálogo')+' · '+(x.posicao_rotulo||'Posição sem catálogo');preencher('#build-campea-detalhe',[['Ímpeto condicional / nível',x.impeto_condicional_rotulo||'Sem ímpeto condicional'],['Barras vencedoras',barras.detalhe||'Não registradas'],['Técnico vencedor',x.tecnico_rotulo||'Técnico sem catálogo'],['Habilidades adicionais vencedoras',habilidades.length?habilidades.join(' · '):'Nenhuma registrada'],['Ímpeto adicional',x.impeto_adicional_rotulo||'Não registrado na saída persistida deste lote'],['Pontuação final',pontuacaoFinal(x)],['Builds comparadas',buildsComparadas(x)],['Builds possíveis na linha',buildsPossiveis(x)]]);const dialog=$('#dialog-build-campea');if(typeof dialog.showModal==='function')dialog.showModal();else dialog.setAttribute('open','');}
...
async function carregarApoiosFila(forcar=false){
  if(!abaAtiva('resultados')||carregandoApoiosFila||(!forcar&&Date.now()<proximaLeituraApoiosFila))return;
  carregandoApoiosFila=true;
  try{
    pintarResultados(await api('/api/resultados?offset='+(paginaResultados*TAMANHO_PAGINA)+'&limite='+TAMANHO_PAGINA));
  }catch(e){
    $('#resultados-aviso').textContent='Não foi possível atualizar Resultados: '+texto(e&&e.message)+'. A lista será tentada novamente.';
  }finally{
    carregandoApoiosFila=false;
    proximaLeituraApoiosFila=Date.now()+15000;
  }
}
async function carregarDetalhesFila(){
  if(carregandoLinhasFila)return;
  carregandoLinhasFila=true;
  linhasFilaCarregando=linhasFilaAtuais.length===0;
  erroLinhasFila='';
  renderizarLinhasFila();
  try{
    const d=await api('/api/fila/linhas?offset='+(paginaFila*TAMANHO_PAGINA)+'&limite='+TAMANHO_PAGINA);
    if(d.conexao_banco===false)throw new Error(d.mensagem||'O banco ainda não respondeu à lista da fila.');
    erroLinhasFila='';
    pintarFila({...d,linhas_carregando:false});
    window.setTimeout(()=>carregarApoiosFila(abaAtiva('resultados')),0);
  }catch(e){
    // A tela precisa mostrar a falha desta leitura; escondê-la fazia parecer
    // que a fila estava vazia embora os totais continuassem chegando.
    linhasFilaCarregando=false;
    const mensagem='Não foi possível carregar a lista da fila: '+texto(e&&e.message)+'. A tentativa será repetida automaticamente.';
    erroLinhasFila=mensagem;
    $('#fila-aviso').textContent=mensagem;
    renderizarLinhasFila();
  }finally{carregandoLinhasFila=false;}
}
async function atualizarFila(){
  if(atualizandoFila)return;
  atualizandoFila=true;
  const botao=$('#atualizar-fila'),textoOriginal=botao.textContent;
  botao.disabled=true;botao.textContent='Atualizando…';
  let atrasoForcado=null;
  try{
    await atualizarSaudeLocal();
    // Primeiro, somente o resumo V6. Ele não consulta a página nem catálogos,
    // portanto os botões e a linha ativa não ficam presos atrás da lista.
    const s=await api('/api/fila/status');
    pintarFila({...s,linhas_carregando:s.disponivel===true&&linhasFilaAtuais.length===0&&!erroLinhasFila});
    if(s.conexao_banco===false){
      falhasConsultaFila+=1;
      atrasoForcado=atrasoConsultaIndisponivel();
      const proxima='Tentando novamente em '+Math.ceil(atrasoForcado/1000)+' s.';
      $('#fila-aviso').textContent=(s.mensagem||'Reconectando ao banco.')+' '+proxima;
      pintarResultados({disponivel:false,mensagem:'Resultados aguardando a reconexão segura com o banco.',itens:[]});
      return;
    }
    falhasConsultaFila=0;
    // A lista é secundária e segue em segundo plano. Uma chamada já aberta não
    // é duplicada pelos refreshes automáticos de três segundos.
    window.setTimeout(carregarDetalhesFila,0);
  }catch(e){
    falhasConsultaFila+=1;
    atrasoForcado=atrasoConsultaIndisponivel();
    const local=resumoSaudeLocal(estadoLocal);
    $('#fila-aviso').textContent='A conexão local está se recuperando. '+(estadoLocal?' — '+local:'')+' · nova tentativa em '+Math.ceil(atrasoForcado/1000)+' s.';
    ultimoEstadoFila=estadoLocal&&estadoLocal.worker_ativo===true?'rodando':'consulta_indisponivel';
  }finally{
    atualizandoFila=false;
    botao.disabled=false;botao.textContent=textoOriginal;
    agendarAtualizacaoFila(ultimoEstadoFila,atrasoForcado);
  }
}
async function acaoFila(acao,corpo){const botao=acao==='iniciar'?$('#iniciar-fila'):acao==='recuperar'?$('#recuperar-fila'):acao==='pausar'?$('#pausar-fila'):acao==='parar'?$('#parar-fila'):null;const original=botao&&botao.textContent;if(botao){botao.disabled=true;botao.textContent=acao==='iniciar'?'Ligando esteira…':acao==='recuperar'?'Recuperando…':acao==='pausar'?'Pausando…':'Parando…';}const rotulo=acao==='recuperar'?'recuperação segura da reserva órfã':acao;$('#fila-aviso').textContent='Enviando '+rotulo+' à esteira integral V6…';try{const r=await fetch('/api/fila/'+acao,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify(corpo||{})}),d=await r.json();if(!r.ok)throw new Error(d.erro||'Ação de fila indisponível');$('#fila-aviso').textContent=d.mensagem||'Ação aceita; atualizando o acompanhamento.';window.setTimeout(()=>atualizarFila(),0);}catch(e){$('#fila-aviso').textContent=e.message;}finally{if(botao){botao.disabled=false;botao.textContent=original;}}}
...
async function carregarCatalogos(){if(catalogosCarregados||carregandoCatalogos)return;carregandoCatalogos=true;try{const c=await api('/api/catalogos');$('#funcao').innerHTML=c.funcoes.map(x=>'<option value="'+x.funcao_id+'">'+seguro(x.nome)+'</option>').join('');$('#tecnico').innerHTML=c.tecnicos.map(x=>'<option value="'+x.tecnico_id+'">'+seguro(x.nome)+' · proficiência '+seguro(x.proficiencia)+'</option>').join('');catalogosCarregados=true;}catch(e){/* A fila já está visível; a próxima leitura saudável tenta os catálogos de novo. */}finally{carregandoCatalogos=false;}}
async function carregar(){await atualizarSaudeLocal();atualizarFila();if(temporizadorSaude===null)temporizadorSaude=window.setInterval(atualizarSaudeLocal,2000);if(temporizadorTemposFila===null)temporizadorTemposFila=window.setInterval(atualizarTemposEmProcessamento,1000);}
document.querySelectorAll('[data-aba]').forEach(x=>x.addEventListener('click',()=>{trocarAba(x.dataset.aba);if(x.dataset.aba==='resultados')atualizarFila();if(x.dataset.aba==='individual')carregarCatalogos();}));
