# -*- coding: utf-8 -*-
"""
MOTOR DOS BONUS — v7 (26/08/2026)

O QUE MUDOU NESTA VERSAO
  1. LE DO BANCO, NAO DE ARQUIVO. Acabaram insumos_bonus.json, base_unica.json,
     pe_ruim.json e linhas.jsonl. Tudo vem de quatro portas do Supabase.
...
AS PORTAS DO BANCO
    public.regua_bonus()          a receita: casa dos estilos, molde do fisico,
                                  parametros, bloqueios, slot por posicao
    public.carta_do_motor(id)     a carta: atributos, corpo, pe ruim, os dois
                                  slots de estilo, estilos de IA
    clube.build                   os pares card x funcao ja rodados
...
MOTOR_BONUS = 'v7-2608-por-funcao-dois-slots'
LOTE = 200
NAOSEI = 'NAO-SEI.txt'
...
def le_tabela(caminho, timeout=180):
    req = urllib.request.Request('%s/rest/v1/%s' % (URL, caminho),
                                 headers=CAB, method='GET')
    with urllib.request.urlopen(req, timeout=timeout) as r:
...
def bonus_do_estilo(rb, est1, est2, funcao, posicao):
    """
    ...
    slot_manda = (rb.get('posicao_slot') or {}).get(posicao) or 'ofensivo'
    ...
    b = 0.0
    if dono and (casa.get(dono) or {}).get(posicao) == funcao:
        b += pri
    if outro and posicao in (liga.get(outro) or []):
        b += sec
...
# ================================================================== RODA
print('=' * 70)
print('  O MOTOR DOS BONUS v7  —  corpo · pe ruim · estilo (por funcao) · IA')
print('=' * 70)
...
rb = rpc('regua_bonus')
if not rb:
    print('  PAREI: a public.regua_bonus() nao devolveu nada.')
    pausa(); sys.exit(1)
par = rb.get('parametro') or {}
...
while True:
    lote = le_tabela('clube.build?select=card_id,funcao_codigo&limit=%d&offset=%d' % (passo, de))
    if not lote:
        break
...
CARTA = {}
saida = []
sem_corpo = sem_pe = sem_ia = 0
for i, cid in enumerate(cards):
    if i % 200 == 0:
        print('   %d/%d cards...' % (i, len(cards)), end='\r')
    try:
        CARTA[cid] = rpc('carta_do_motor', {'p_card_id': cid}) or {}
    except Exception:
        CARTA[cid] = {}
...
for cid, fun in pares:
    ...
    b_pe = bonus_do_pe_ruim(par, c.get('pe_ruim_uso'), c.get('pe_ruim_precisao'))
    if b_pe is None:
        sem_pe += 1

    b_est = bonus_do_estilo(rb, c.get('slot1_nome'), c.get('slot2_nome'), fun, posicao)
...
try:
    with open(NAOSEI, 'w', encoding='utf-8') as f:
        ...
        f.write('\n')
        f.write('DE ONDE CADA UM VEM AGORA (tudo do banco, 26/08):\n')
        f.write('  corpo .......... clube.carta.corpo        (ainda nao vem do arquivo do jogo)\n')
        f.write('  pe ruim ........ carta_jogo.pe_ruim_uso / pe_ruim_precisao (bits 478 e 578)\n')
        f.write('  estilo da IA ... carta_jogo.estilos_ia    (7 bits do Player.bin)\n')
...
enviados = 0
for i in range(0, len(saida), LOTE):
    lote = [{'card_id': x['card_id'], 'funcao_codigo': x['funcao_codigo'],
            ...
            for x in saida[i:i + LOTE]]
    try:
        n = rpc('gravar_bonus', {'p_linhas': lote})
        enviados += int(n or 0)
        print('   %d/%d gravados...' % (enviados, len(saida)), end='\r')
