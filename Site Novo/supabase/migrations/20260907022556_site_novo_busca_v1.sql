-- Busca publica do Site Novo. Substitui os artefatos inertes da busca aposentada.
create or replace function clube_novo.site_novo_busca_normalizar_v1(p_texto text)
returns text
language sql
immutable
parallel safe
...
create index carta_jogo_site_novo_busca_v1_fts_idx
...
revoke all on function public.site_novo_busca_v1(text, integer, integer) from public;
grant execute on function public.site_novo_busca_v1(text, integer, integer)
to anon, authenticated, service_role;

comment on function public.site_novo_busca_v1(text, integer, integer)
is 'Busca publica do Site Novo sobre carta_jogo, com prefixos, normalizacao propria, posicao em portugues e paginacao.';
