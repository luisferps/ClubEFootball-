...
create or replace function public.site_novo_boxes_em_andamento_v1(p_box text default null,p_busca text default '',p_limite integer default 24,p_offset integer default 0)
...
begin
 ...
 select b.card_id,b.linha_id,b.funcao_id,b.funcao_nome,b.posicao_codigo,b.percentual_topo,b.overall_final,max(b.overall_final) over(partition by b.card_id) pontuacao_maxima,
 row_number() over(partition by b.card_id,b.funcao_id order by b.percentual_topo desc,b.overall_final desc,b.linha_id) rn
 ...
 select c.card_id,c.nome,x.box_nome as box,c.overall,c.posicao,c.foto_url_cloudinary,(select max(v.pontuacao_maxima) from avaliadas v where v.card_id=c.card_id) pontuacao_maxima,
 ...
 select box,count(*)::integer total_cards,max((analises#>>'{0,percentual_topo}')::numeric) melhor_percentual from base
 where strpos(lower(box),lower(btrim(p_busca)))>0 group by box
 ), pagina as (select * from grupos order by melhor_percentual desc nulls last,box collate "C" limit p_limite offset p_offset),
 itens as (
 select p.box,p.total_cards,p.melhor_percentual,coalesce((select jsonb_agg(to_jsonb(t) order by (t.analises#>>'{0,percentual_topo}')::numeric desc nulls last,(t.analises#>>'{0,pontuacao}')::numeric desc nulls last,t.card_id collate "C")
 from (select b.card_id,b.nome,b.posicao,b.overall,b.foto_url_cloudinary as foto_url,b.analises,b.pontuacao_maxima from base b where b.box=p.box order by (b.analises#>>'{0,percentual_topo}')::numeric desc nulls last,(b.analises#>>'{0,pontuacao}')::numeric desc nulls last,b.card_id collate "C" limit 3)t),'[]'::jsonb) cards from pagina p
 )
 select jsonb_build_object('contrato','site-novo-boxes-v1','versao',1,'modo','boxes','box',null,'busca',p_busca,'total',(select count(*) from grupos),'total_cards',(select coalesce(sum(total_cards),0) from grupos),'limite',p_limite,'offset',p_offset,'itens',coalesce((select jsonb_agg(to_jsonb(i) order by i.melhor_percentual desc nulls last,i.box collate "C") from itens i),'[]'::jsonb)) into resultado;
 else
 with avaliadas as materialized (
 select b.card_id,b.linha_id,b.funcao_id,b.funcao_nome,b.posicao_codigo,b.percentual_topo,b.overall_final,max(b.overall_final) over(partition by b.card_id) pontuacao_maxima,
 row_number() over(partition by b.card_id,b.funcao_id order by b.percentual_topo desc,b.overall_final desc,b.linha_id) rn
 ...
 from clube_novo.box_contexto_contratacao_v1 x join clube_novo.box_card_em_andamento_v1 m on m.box_id=x.box_id join clube_novo.carta_jogo c on c.card_id=m.card_id
 where x.estado_box='em_andamento'
 ), selecionadas as (select card_id,nome,posicao,overall,foto_url_cloudinary as foto_url,analises,pontuacao_maxima from base where box=p_box), pagina as (select * from selecionadas order by (analises#>>'{0,percentual_topo}')::numeric desc nulls last,(analises#>>'{0,pontuacao}')::numeric desc nulls last,card_id collate "C" limit p_limite offset p_offset)
 select jsonb_build_object('contrato','site-novo-boxes-v1','versao',1,'modo','cards','box',p_box,'busca',p_busca,'total',(select count(*) from selecionadas),'total_cards',(select count(*) from selecionadas),'limite',p_limite,'offset',p_offset,'itens',coalesce((select jsonb_agg(to_jsonb(p) order by (p.analises#>>'{0,percentual_topo}')::numeric desc nulls last,(p.analises#>>'{0,pontuacao}')::numeric desc nulls last,p.card_id collate "C") from pagina p),'[]'::jsonb)) into resultado;
 end if;
 return resultado || jsonb_build_object('regua',(
 select coalesce(jsonb_agg(jsonb_build_object('codigo',f.codigo,'rotulo',f.rotulo,'percentual_minimo',f.percentual_minimo) order by f.ordem),'[]'::jsonb)
 ...
 ),'tem_mais',(resultado->>'total')::bigint>p_offset+p_limite,'status',case when jsonb_array_length(resultado->'itens')=0 then 'vazio' else 'pronto' end);
end $$;
