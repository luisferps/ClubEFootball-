-- A coleção comercial France 1998 foi confirmada pelo título e pelo trio de
-- cartas. Ela é complementar ao acervo legado: os rótulos físicos de versão
-- continuam fora da relação pública de boxes.
create extension if not exists pg_trgm with schema extensions;

create or replace function clube_novo.site_novo_texto_corresponde_v1(
  p_texto text,
  p_busca text
)
returns boolean
...
as $function$
  with normalizado as (
    select
      lower(extensions.unaccent(coalesce(p_texto,''))) as texto,
      lower(extensions.unaccent(btrim(coalesce(p_busca,'')))) as busca
  )
  select busca=''
    or strpos(texto,busca)>0
    or (
      length(busca)>=5
      and extensions.word_similarity(busca,texto)>=0.72
    )
  from normalizado;
$function$;

revoke all on function clube_novo.site_novo_texto_corresponde_v1(text,text) from public,anon,authenticated;
grant execute on function clube_novo.site_novo_texto_corresponde_v1(text,text) to service_role;

do $seed$
declare
  v_titulo constant text := 'Big Time & Epic: France 1998';
  v_fonte constant text := 'correcao editorial confirmada: France 1998; 2026-09-07';
  v_fingerprint constant text := encode(
    extensions.digest(
      convert_to('Big Time & Epic: France 1998|89138288270047|88044145351392|88044145348029','UTF8'),
      'sha256'
    ),
    'hex'
  );
  ...
  v_total integer;
begin
  perform pg_advisory_xact_lock(hashtextextended('clube_novo.boxes.france-1998',0));

  if exists (
    select 1
    from (values
      ('89138288270047'::text,'Marcel Desailly'::text),
      ('88044145351392'::text,'Lilian Thuram'::text),
      ('88044145348029'::text,'Patrick Vieira'::text)
    ) esperado(card_id,nome)
    left join clube_novo.carta_jogo c
      on c.card_id=esperado.card_id and c.nome=esperado.nome
    where c.card_id is null
  ) then
    raise exception 'France 1998 recusada: trio de cartas nao confere com carta_jogo';
  end if;

  select box_id into v_box_id
  from clube_novo.box_contexto_contratacao_v1
  where box_nome=v_titulo and oferta_fonte=v_fonte
  order by box_id
  limit 1;
  ...
  if v_box_id is null then
    ...
    values
      (v_box_id,v_titulo,'finalizada','anterior',v_fingerprint,v_fonte,null);
  else
    update clube_novo.box_contexto_contratacao_v1
    set estado_box='finalizada',
        status_origem='anterior',
        origem_fingerprint=v_fingerprint,
        data_oferta=null
  ...
  values
    (v_box_id,'89138288270047'),
    (v_box_id,'88044145351392'),
    (v_box_id,'88044145348029')
  on conflict do nothing;

  select count(*) into v_total
  from clube_novo.box_card_em_andamento_v1
  where box_id=v_box_id;

  if v_total<>3 then
    raise exception 'France 1998 recusada: esperado trio, encontrados % vinculos',v_total;
  end if;
end
$seed$;
...
create or replace function clube_novo.site_novo_boxes_calculo_v1(
...
comment on function clube_novo.site_novo_boxes_calculo_v1(text,text,integer,integer,text,integer)
is 'Catalogo de boxes com busca sem acento e tolerancia a um erro curto de digitacao.';
