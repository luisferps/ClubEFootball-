...
begin
 ...
 select jsonb_build_object(
  ...
  'habilidades',(select coalesce(jsonb_agg(jsonb_build_object('id',h.skill_id,'nome',h.nome_pt,'gemeas',h.gemeas,'bloqueada_funcoes',(select coalesce(jsonb_agg(b.funcao_id order by b.funcao_id),'[]'::jsonb) from clube_novo.habilidade_funcao_bloqueio_otimizador b where b.skill_id=h.skill_id),'avaliavel',h.efeito_por_codigo is not null and not h.efeito_desconhecido) order by h.nome_pt),'[]')
...
begin
 ...
 if exists(select 1 from clube_novo.habilidade_funcao_bloqueio_otimizador b
           where b.funcao_id=fid and b.skill_id=any(added)) then
   raise exception 'Habilidade adicional não permitida nesta especialidade. Remova ou substitua a habilidade.';
 end if;
 ...
 if exists(select 1 from clube_novo.bonificador_regra_playstyle where playstyle_id=main_style and posicao_id=pos and da_bonus) then style_bonus:=(par->>'estilo_ativo')::numeric; end if;
