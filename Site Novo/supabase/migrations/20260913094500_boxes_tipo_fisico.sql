...
declare
  ...
  v_closed integer := 0;
begin
  if p->>'participantes_completos' is distinct from 'true' then
    raise exception 'Captura contem apenas destaques: abra os detalhes e capture todos os participantes antes de publicar' using errcode='22023';
  end if;
  if p is null or p->>'schema' is distinct from 'clubef-boxes-jogo-runtime-v1'
     or p->>'leitor_versao' is distinct from 'boxes-cmd-get-myclub-agentlist-detalhes-v2'
     or p->>'fonte' is distinct from 'jogo:CmdGetMyclubAgentlist'
     or p->>'cobertura' is distinct from 'participantes_completos_do_agente_carregado'
  ...
  if exists(
    ...
    where jsonb_typeof(b)<>'object'
       ...
       or coalesce((b->>'total_jogo')::integer,0)<>jsonb_array_length(b->'cartas')
  ) then
  ...
  for v_box in select value from jsonb_array_elements(p->'boxes') loop
    ...
    select v_box_id,c.value,v_when
    from jsonb_array_elements_text(v_box->'cartas') c
    join clube_novo.carta_jogo j on j.card_id=c.value
    where j.codigo_tipo_carta_fisico>0;
  end loop;
...
-- Recompor vinculos das capturas completas usando o tipo fisico vigente.
INSERT INTO clube_novo.box_card_em_andamento_v1(box_id,card_id,capturado_em)
SELECT b.box_id,c.card_id,a.capturado_em
FROM clube_novo.box_captura_jogo_v1 a
JOIN clube_novo.box_agente_card_captura_jogo_v1 c USING(captura_id)
JOIN clube_novo.box_contexto_contratacao_v1 b ON b.agente_jogo_id=c.agente_jogo_id
 AND b.capturado_em=a.capturado_em AND b.oferta_fonte='jogo:CmdGetMyclubAgentlist'
JOIN clube_novo.carta_jogo j ON j.card_id=c.card_id
WHERE a.leitor_versao='boxes-cmd-get-myclub-agentlist-detalhes-v2'
 AND a.payload->>'participantes_completos'='true' AND j.codigo_tipo_carta_fisico>0
