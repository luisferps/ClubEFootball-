-- Evita encadear a view pública a outra view security_invoker. O usuário anon
-- continua sem acesso às tabelas internas; somente o dono isolado do contrato
-- público recebe SELECT nos dois relacionamentos comerciais necessários.

grant select on clube_novo.box_contexto_contratacao_v1,
...
comment on view public.frontend_boxes_v1 is
  'Contrato SELECT-only da tela Boxes cadastradas. O dono isolado lê diretamente os relacionamentos oficiais; anon não acessa tabelas internas.';

revoke select on clube_novo.carta_box_oferta_v1 from clube_frontend_view_owner;
