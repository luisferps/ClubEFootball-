...
function card(c,index){
 ...
 const details=[['Estilo de jogo',(c.estilos||[]).map(s=>s.nome).join(' / ')||'Sem estilo ativo'],['Especialidade',best?.funcao||'Ainda não avaliada']].map(([label,value])=>'<span class="nb-identity-row"><span class="nb-identity-label">'+label+'</span><span class="nb-identity-value">'+esc(value)+'</span></span>').join('');
 const main='<a class="nb-rated-player" href="'+href+'" target="_blank" rel="noopener"><span class="nb-rank">'+(index+1+(state.p_box?state.p_offset:0))+'</span>'+picture(c)+'<span class="nb-player-info"><strong>'+esc(c.nome)+'</strong><span class="nb-position">'+esc((c.posicao_nome||best?.posicao||c.posicao||'Não informada')+((best?.posicao||c.posicao)?' — '+(best?.posicao||c.posicao):''))+'</span><span class="nb-identity">'+details+'</span>'+(c.regua_vigente===false?'<span class="nb-regua-antiga" title="Nota da regua anterior: esta linha ainda nao passou pelo Otimizador novo">Regua antiga</span>':'')+'</span><span class="nb-rating">'+score+'</span></a>';
 return '<article class="nb-entry">'+main+'</article>';
}
function sortControl(){return !active&&!state.p_box?'<div class="nb-sort"><label>Ordenar por <select data-nb-sort aria-label="Ordenar boxes"><option value="recentes" '+(state.p_ordem==='recentes'?'selected':'')+'>Mais recentes</option><option value="pontuacao" '+(state.p_ordem==='pontuacao'?'selected':'')+'>Maior pontuação</option></select></label><small>Esta lista não inclui as boxes em andamento.</small></div>':'';}
