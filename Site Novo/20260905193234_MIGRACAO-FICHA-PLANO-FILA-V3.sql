-- Ficha V3: reduzir o custo de inicializacao do plano sem mudar o contrato.
-- Extrai a verificacao das filas do Otimizador para uma funcao interna PL/pgSQL.
-- Nao aumenta statement_timeout e nao altera assinatura, JSON, selecao ou publicacao.
...
begin
  ...
  if v_md5 <> 'db59855c7150b57e7c88398e543aaff6' then
    raise exception
      'Ficha base divergiu antes da V3: esperado %, encontrado %',
      'db59855c7150b57e7c88398e543aaff6', v_md5;
  end if;

  if to_regprocedure(
    'clube_novo.site_novo_fila_atributos_confirmada_v1(text)'
  ) is not null then
    raise exception 'Helper V1 da Ficha ja existe; abortando para auditar drift';
  end if;
end
$preflight$;

create function clube_novo.site_novo_fila_atributos_confirmada_v1(
  p_card_id text
)
returns boolean
language plpgsql
...
begin
  return (
    exists (
      select 1
      from clube_novo.otimizador_lote_producao_candidata_v5 fc
      join clube_novo.otimizador_lote_producao_v3 fl
        on fl.id=fc.lote_id
      where fc.card_id=p_card_id
        and fl.estado not in ('concluido','cancelado','falhou')
    )
    or exists (
      select 1
      from clube_novo.otimizador_lote_producao_carta_v3 fc
      join clube_novo.otimizador_lote_producao_v3 fl
        on fl.id=fc.lote_id
      where fc.card_id=p_card_id
        and fl.estado not in ('concluido','cancelado','falhou')
    ...
    or exists (
      select 1
      from clube_novo.otimizador_lote_producao_linha_v3 fq
      join clube_novo.otimizador_lote_producao_v3 fl
        on fl.id=fq.lote_id
      where fq.card_id=p_card_id
        and fl.estado not in ('concluido','cancelado','falhou')
        and fq.finalizada_em is null
    )
  );
end
$helper$;

comment on function clube_novo.site_novo_fila_atributos_confirmada_v1(text) is
  'Helper interno da Ficha: informa se o card possui fila viva de atributos no Otimizador. Nao publica nem escreve.';

revoke all on function clube_novo.site_novo_fila_atributos_confirmada_v1(text)
...
comment on function public.site_novo_ficha_base_publicada_v1(text,bigint) is
  'Base privada da Ficha v1. Sem linha explicita seleciona a maior nota_final publica; com linha explicita retorna exatamente a linha publicada pedida. A verificacao de fila usa helper interno para reduzir o custo de planejamento. Nao recalcula pontuacao.';

revoke all on function public.site_novo_ficha_base_publicada_v1(text,bigint)
  from public, anon, authenticated;
grant execute on function public.site_novo_ficha_base_publicada_v1(text,bigint)
  to service_role;
