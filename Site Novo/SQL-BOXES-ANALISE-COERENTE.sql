...
select case when o.analises is null then null else coalesce((
 select jsonb_agg(case when f.id is not null then jsonb_set(j.item,'{funcao}',to_jsonb(f.rotulo)) else j.item end order by clube_novo.contratacao_estrelas_v1(j.item->>'codigo') desc nulls last,(j.item->>'pontuacao')::numeric desc nulls last,j.ordem)
  ...
  if p_box is null and p_ordem='recentes' then
         ...
         from (
           select bp.card_id,bp.nome,bp.posicao,bp.overall,bp.foto_url,bp.pontuacao_maxima,bp.regua_vigente,
             ...
             row_number() over(order by
               ...
               (clube_novo.site_novo_box_card_analise_snapshot_v1(bp.box_id,bp.card_id,p_degrau::smallint)#>>'{0,pontuacao}')::numeric desc nulls last,bp.card_id collate "C") ordem_card
    ...
    cards_box as materialized (
      select v.card_id,v.box_id,c.nome,
        ...
        row_number() over(order by clube_novo.contratacao_estrelas_v1(clube_novo.site_novo_box_card_analise_snapshot_v1(v.box_id,v.card_id,p_degrau::smallint)#>>'{0,codigo}') desc nulls last,(clube_novo.site_novo_box_card_analise_snapshot_v1(v.box_id,v.card_id,p_degrau::smallint)#>>'{0,pontuacao}')::numeric desc nulls last,v.card_id collate "C") ordem_card
      from vinculos v
      join clube_novo.carta_jogo c on c.card_id=v.card_id
      left join notas n on n.card_id=v.card_id
    ),
         ...
         from (
           select b.card_id,b.box_id,b.nome,b.posicao,b.overall,b.foto_url,b.pontuacao_maxima,
             ...
             row_number() over(order by
               ...
               (clube_novo.site_novo_box_card_analise_snapshot_v1(b.box_id,b.card_id,p_degrau::smallint)#>>'{0,pontuacao}')::numeric desc nulls last,b.card_id collate "C") ordem_card
    ...
    base as materialized (
      select c.card_id,c.nome,x.box_nome as box,c.overall,
        ...
        coalesce((
          ...
          ) order by clube_novo.contratacao_estrelas_v1(et.item->>'etiqueta_codigo') desc nulls last,v.overall_final desc,v.linha_id)
          from avaliadas v
