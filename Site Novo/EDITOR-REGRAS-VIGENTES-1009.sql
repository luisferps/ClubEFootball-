...
declare
 ...
 raw_motor double precision:=0; raw_delta numeric; raw_step integer; raw_inc double precision; ceiling_motor numeric:=0; normalized_motor numeric; style_effective jsonb; style_result jsonb;
 ...
 body_detail jsonb:='{}'; body_scaled jsonb:='{}'; body_acc numeric:=0; body_part numeric; body_last text; height_result jsonb;
...
begin
 ...
 where ca.card_id=cid order by o.indice_otimizador loop
   ...
   -- Mesma soma incremental e arredondamento ties-to-even de regua.py/notaDe.
   raw_delta:=(v->>'sistema')::numeric-r.alvo;
   if r.peso<>0 then
     if raw_delta>=0 then
       for raw_step in 1..least(9,floor(raw_delta)::integer) loop
         raw_motor:=raw_motor+(array[1,.88,.76,.64,.52,.40,.28,.16,.04]::double precision[])[raw_step]*r.peso::double precision;
       end loop;
     elsif r.peso<>1 then
       raw_inc:=0.25::double precision*r.peso::double precision/12;
       for raw_step in 1..least(9,floor(-raw_delta)::integer) loop
         raw_motor:=raw_motor-(1+(raw_step-1)*raw_inc)*r.peso::double precision;
       end loop;
     end if;
   end if; ceiling_motor:=ceiling_motor+r.peso*4.68;
 ...
 raw_motor:=round(raw_motor*10)/10; ceiling_motor:=round(ceiling_motor,1);
 normalized_motor:=clube_novo.normalizar_motor_v3(fid,raw_motor::numeric,ceiling_motor);
 total:=normalized_motor+round(body_bonus+foot_bonus+ia_bonus+style_bonus,4);
 if total is null then raise exception 'Parcela da nota não confirmada.'; end if;
 v:=jsonb_build_object('motor','editor-independente-v1','regras_versao','editor-regras-v13-20260910-v1','normalizacao_versao','normalizacao-amplitude-20260910-v1','pontuacao_motor_bruta',raw_motor,'teto_motor',ceiling_motor,'pontuacao_motor_normalizada',normalized_motor,'bonus_total',round(body_bonus+foot_bonus+ia_bonus+style_bonus,4),'bonus_fisico_detalhe',height_result->'detalhe','bonus_componentes',jsonb_build_object('corpo',body_bonus,'pe',foot_bonus,'ia',ia_bonus,'estilos',style_bonus),'card_id',cid,'nota_final',total,'atributos',attrs,
...
begin
 if b.resultado->>'normalizacao_versao' is distinct from 'normalizacao-amplitude-20260910-v1' or b.resultado->>'regras_versao' is distinct from 'editor-regras-v13-20260910-v1' then
