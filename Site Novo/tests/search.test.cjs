...
runWithCommon(fs.readFileSync(path.join(base,'search-api.js'),'utf8'),{window,fetch,URL,Number,Set,setTimeout});
...
(async()=>{
 ...
 const messi=await read('Messi');assert.equal(messi.degrau,3);assert.ok(Number.isFinite(messi.itens[0].pontuacao_total),'a primeira carta tem pontuação publicada');
 ...
 assert.ok(plain.total>0);assert.equal(plain.total,accented.total);assert.deepEqual(plain.itens.map(i=>i.card_id),accented.itens.map(i=>i.card_id));
