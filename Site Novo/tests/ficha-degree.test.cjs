...
ctx.renderImpulses([{tipo:'nativo',nome:'Física',condicao_nivel:level,delta_uniforme:3,efeitos:[]}],{});
const badge=nodes[0].children[0].children[1];assert.equal(badge.tag,'span');assert.equal(badge.text,'+'+level);
}
...
assert.doesNotMatch(html,/Ficha â€” visÃ£o do usuÃ¡rio|id="status-badge"|id="page-subtitle"/);
...
(async()=>{
  ...
  console.log('Ficha: controles sincronizados, mapa pÃºblico 1/2/3 e Ã­mpetos fixos preservados.');
