...
data.degrau=3; data.itens.forEach(item=>{item.estilos=[];});
runWithCommon(fs.readFileSync(path.join(base,'ranking-api.js'),'utf8'),{window,URL,Number,Set,fetch:async(url,options)=>{sent.push(JSON.parse(options.body));return {ok:true,json:async()=>data};}});
(async()=>{
