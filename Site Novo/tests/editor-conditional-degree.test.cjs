...
const target=node('div'),session={saving:false,input:{impetos:{},condicoes:{2:3}},catalog:{adicionais:[],slots:[{slot:2,codigo:99,nome:'FÃ­sica',vaga:false,condicional:true,maximo:3,cor:2,efeitos:[{nome:'EquilÃ­brio',delta:3}]}]}};
let changes=0;
const context={window:{SiteNovoDegrau:{get:()=>3}},session,colors:{2:'amarelo'},field:()=>target,el:node,button:(text,fn,css)=>{const b=node('button',css,text);b.type='button';b.onclick=fn;return b},changed:()=>{changes++},Number};
...
console.log('OK: botÃ£o condicional alterna +3, +1, +2 e nunca oferece zero.');
