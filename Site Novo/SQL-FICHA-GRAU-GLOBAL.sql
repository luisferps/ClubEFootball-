DROP FUNCTION IF EXISTS public.site_novo_ficha_base_publicada_v1(text,bigint);
DROP FUNCTION IF EXISTS public.site_novo_ficha_sem_complemento_v14(text,bigint);
DROP FUNCTION IF EXISTS public.site_novo_ficha_v1(text,bigint);
DROP FUNCTION IF EXISTS public.site_novo_ficha_v2(text,bigint);
CREATE OR REPLACE FUNCTION public.site_novo_ficha_base_publicada_v1(p_card_id text DEFAULT NULL::text, p_linha_id bigint DEFAULT NULL::bigint, p_degrau integer DEFAULT 3)
 RETURNS jsonb
 LANGUAGE sql
...
selecionada as (
  select p.*
  from publicadas p
  where (p_linha_id is null and (p.impeto_condicional_codigo is null or p.impeto_condicional_nivel = p_degrau)) or p.id = p_linha_id
  order by
...
visiveis as (
 ...
 where p.impeto_condicional_codigo is null
    or p.impeto_condicional_nivel = p_degrau
),
contexto as (
  select
          ...
          select coalesce(
            jsonb_agg(
              jsonb_build_object(
                ...
                'regua_vigente', coalesce((select v.regua_vigente from clube_novo.build_publicacao_exibivel_v3 v where v.linha_id = p.id limit 1), false),
...
$function$
;
REVOKE ALL ON FUNCTION public.site_novo_ficha_base_publicada_v1(text,bigint,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.site_novo_ficha_base_publicada_v1(text,bigint,integer) TO service_role;
CREATE OR REPLACE FUNCTION public.site_novo_ficha_sem_complemento_v14(p_card_id text DEFAULT NULL::text, p_linha_id bigint DEFAULT NULL::bigint, p_degrau integer DEFAULT 3)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
declare
  ...
  degraus jsonb;
begin
  ficha := public.site_novo_ficha_v1(p_card_id,p_linha_id,p_degrau)
...
$function$
;
REVOKE ALL ON FUNCTION public.site_novo_ficha_sem_complemento_v14(text,bigint,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.site_novo_ficha_sem_complemento_v14(text,bigint,integer) TO anon,authenticated,service_role;
CREATE OR REPLACE FUNCTION public.site_novo_ficha_v1(p_card_id text DEFAULT NULL::text, p_linha_id bigint DEFAULT NULL::bigint, p_degrau integer DEFAULT 3)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
begin
  v_base:=public.site_novo_ficha_base_publicada_v1(p_card_id,p_linha_id,p_degrau);
...
$function$
;
REVOKE ALL ON FUNCTION public.site_novo_ficha_v1(text,bigint,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.site_novo_ficha_v1(text,bigint,integer) TO anon,authenticated,service_role;
CREATE OR REPLACE FUNCTION public.site_novo_ficha_v2(p_card_id text, p_linha_id bigint DEFAULT NULL::bigint, p_degrau integer DEFAULT 3)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
begin
  d := public.site_novo_ficha_sem_complemento_v14(p_card_id,p_linha_id,p_degrau);
...
;
REVOKE ALL ON FUNCTION public.site_novo_ficha_v2(text,bigint,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.site_novo_ficha_v2(text,bigint,integer) TO anon,authenticated,service_role;
