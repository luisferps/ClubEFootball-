-- Ficha V2: autoridade de selecao por maior nota_final.
-- Regra: sem linha explicita, abrir a build publica de maior nota.
-- Com linha explicita, manter exatamente a linha pedida.
-- Nao altera assinatura, chaves JSON, RLS nem publicacao incremental.
...
begin
  ...
  if v_md5 <> '5f221e8afd3d2e4a8ce10c5ba0d1b0e8' then
    raise exception
      'site_novo_ficha_base_publicada_v1 divergiu antes da V2: esperado %, encontrado %',
      '5f221e8afd3d2e4a8ce10c5ba0d1b0e8', v_md5;
  end if;
end
$preflight$;
...
comment on function public.site_novo_ficha_base_publicada_v1(text,bigint) is
  'Base privada da Ficha v1. Sem linha explicita seleciona a maior nota_final publica; com linha explicita retorna exatamente a linha publicada pedida. Nao recalcula pontuacao.';

revoke all on function public.site_novo_ficha_base_publicada_v1(text,bigint) from public;
revoke all on function public.site_novo_ficha_base_publicada_v1(text,bigint) from anon, authenticated;
grant execute on function public.site_novo_ficha_base_publicada_v1(text,bigint) to service_role;
