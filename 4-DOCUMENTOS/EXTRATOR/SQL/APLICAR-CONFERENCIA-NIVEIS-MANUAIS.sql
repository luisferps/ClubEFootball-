...
begin
 ...
 if v_n<>v_rows or exists(select 1 from clube_novo.carta_nivel_evidencia_v1 e join clube_novo.carta_jogo c using(card_id)
   cross join lateral (
     select jsonb_build_object('level_cap',e.nivel_maximo,'orcamento',e.orcamento_real,'cap_estimado',false)
   ...
   where e.captura_id=v_id and (
     c.level_cap is distinct from (m.esperado->>'level_cap')::integer
     or c.orcamento is distinct from (m.esperado->>'orcamento')::integer
     or c.cap_estimado is distinct from (m.esperado->>'cap_estimado')::boolean)) then
