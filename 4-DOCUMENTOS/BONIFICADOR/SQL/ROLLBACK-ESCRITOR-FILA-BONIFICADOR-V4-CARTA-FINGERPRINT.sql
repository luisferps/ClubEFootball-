-- RECUPERAÇÃO EXPLÍCITA — não executar durante uma rodada.
-- Reverte somente a revisão de 31/08/2026 do escritor V4: volta a persistir o
-- fingerprint da projeção build_linha_card. Não altera fórmula, parcelas, pesos,
-- moldes, ordem ou dados de jogo.
begin;
...
begin
  ...
  if l.build_bonificador_id is not null or l.lote_estado<>'concluido'
  ...
  values(nextval('clube_novo.build_bonificador_id_seq'::regclass),
    ...
    (p_resultado->>'bonus_total')::numeric,'{}',l.carta_fingerprint) returning id into resultado_id;
