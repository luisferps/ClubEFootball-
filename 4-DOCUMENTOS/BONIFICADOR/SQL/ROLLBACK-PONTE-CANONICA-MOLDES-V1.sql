...
begin
  if not (regua -> 'molde_corpo' ? 'Centroavante fixo')
     or (regua -> 'molde_corpo' ? 'centroavante_fixo') then
    raise exception 'rollback: formato anterior do molde nao foi restaurado';
  end if;
end
$readback$;
