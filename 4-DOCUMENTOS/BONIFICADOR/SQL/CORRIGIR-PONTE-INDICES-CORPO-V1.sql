...
begin
  ...
  if atual is distinct from '48f04884cab34110950833453f468b94' then
    raise exception 'preflight: ponte de funcoes mudou antes dos indices: %', atual;
  end if;
end
$preflight$;
...
declare
  ...
  regras integer;
  funcoes_com_12 integer;
begin
  select count(*) into regras
  from jsonb_each(regua -> 'molde_corpo') f
  cross join lateral jsonb_each(f.value) m
  where m.value ? 'idx' and (m.value ->> 'idx') is not null;

  select count(*) into funcoes_com_12 from (
    select f.key
    from jsonb_each(regua -> 'molde_corpo') f
    cross join lateral jsonb_each(f.value) m
    group by f.key
    having count(*) = 12
       and count((m.value ->> 'idx')) = 12
       and count(distinct (m.value ->> 'idx')) = 12
  ) x;

  if coalesce((regua ->> 'pode_rodar')::boolean, false) is not true
     or regras <> 228 or funcoes_com_12 <> 19
     or (regua #>> '{molde_corpo,centroavante_fixo,altura,idx}')::integer <> 0 then
    raise exception 'readback: indices canonicos dos moldes nao ficaram aptos';
  end if;
  if coalesce((casillas ->> 'pode_rodar')::boolean, true) is not false then
