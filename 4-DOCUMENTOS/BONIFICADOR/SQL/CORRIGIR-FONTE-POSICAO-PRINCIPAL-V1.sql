...
base as (
  select c.card_id, c.nome, c.posicao as posicao_raw,
         ...
         c.slot_ofensivo_id as slot1_raw,
         c.slot_defensivo_id as slot2_raw,
  ...
  left join clube_novo.playstyle p1
    on p1.indice = case
      when c.slot_ofensivo_id is not null and c.slot_ofensivo_id % 4 = 0
        then c.slot_ofensivo_id / 4
      else null
    end
  left join clube_novo.playstyle p2
    on p2.indice = c.slot_defensivo_id
),
gated as (
  select b.*,
         array_remove(array[
...
select jsonb_build_object(
  ...
  'proveniencia', jsonb_build_object(
    ...
    'slot1', 'carta_jogo.slot_ofensivo_id / 4 -> playstyle.indice -> id_jogo',
    'slot2', 'carta_jogo.slot_defensivo_id -> playstyle.indice -> id_jogo',
...
begin
  if coalesce((ok ->> 'pode_rodar')::boolean, false) is not true
     or (ok ->> 'posicao_relacao_cardinalidade')::integer <> 1 then
    raise exception 'readback: posicao principal normalizada da carta controle nao apta: %', ok;
  end if;
