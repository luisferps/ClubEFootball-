...
declare
  atual text;
begin
  select md5(pg_get_functiondef('public.bonificador_regua_v1()'::regprocedure)) into atual;
  if atual is distinct from '43d03e9995d41d46e2518d9cffde2012' then
    raise exception 'preflight: bonificador_regua_v1 mudou desde o snapshot: %', atual;
  end if;
end
$preflight$;
...
gate_values as (
  select
    ...
    (select count(*) from canonical_body_rules where medida is not null) as body_canonical_rules,
    (select count(*) from (
      select funcao_id from canonical_body_rules
      group by funcao_id having count(medida) = 12
...
gate as (
  select g.*,
         array_remove(array[
           ...
           case when body_canonical_rules <> 228 then 'molde_corpo: ponte canonica diferente de 19 x 12' end,
           case when body_functions_with_12 <> 19 then 'molde_corpo: funcao canonica sem 12 regras' end,
...
do $readback$
declare
  regua jsonb := public.bonificador_regua_v1();
  casillas jsonb := public.bonificador_carta_v1('88045755827028');
  n_funcoes integer;
begin
  select count(*) into n_funcoes from jsonb_object_keys(regua -> 'molde_corpo');
  if coalesce((regua ->> 'pode_rodar')::boolean, false) is not true
     or n_funcoes <> 19
     or not (regua -> 'molde_corpo' ? 'centroavante_fixo')
     or (regua -> 'molde_corpo' ? 'Centroavante fixo')
     or (regua #>> '{funcao_molde,centroavante_fixo,id}')::integer <> 1 then
    raise exception 'readback: ponte canonica do molde nao ficou apta: %', regua -> 'falta_o_que';
  end if;
  if coalesce((casillas ->> 'pode_rodar')::boolean, true) is not false
     or not (casillas -> 'falta_o_que' @> '["playstyle 291: regra Goleiro adiantado aguarda decisao"]'::jsonb) then
  ...
  if not has_function_privilege('service_role','public.bonificador_regua_v1()','execute')
    ...
    raise exception 'readback: grants incorretos';
  end if;
end
$readback$;
