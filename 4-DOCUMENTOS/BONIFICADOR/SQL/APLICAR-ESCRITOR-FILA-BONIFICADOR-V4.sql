-- Escritor transacional V4. Aceita exclusivamente a mesma fila canônica V4.
-- Fórmula, parcelas, pesos, moldes e fingerprint matemático permanecem idênticos ao V3.
begin;
create or replace function public.gravar_build_bonificador_v4(p_resultado jsonb)
...
begin
  if jsonb_typeof(p_resultado) <> 'object' or not (p_resultado ?& array[
    ...
    raise exception 'writer V4: payload incompleto';
  end if;
  ...
  if l.id is null or l.card_id<>p_resultado->>'card_id'
    ...
    raise exception 'writer V4: identidade canônica divergente';
  end if;
  if l.build_bonificador_id is not null
     or l.lote_estado<>'concluido' or l.estado<>'pendente'
     or l.estado_otimizador<>'concluido'
     or not (l.pendencias @> array['bonificador_nao_executado']::text[]) then
    raise exception 'writer V4: linha fora da fila operacional do Bonificador';
  end if;
  ...
  if not coalesce((r->>'pode_rodar')::boolean,false)
    ...
    raise exception 'writer V4: selo ou gate divergente';
  end if;
  if round(((p_resultado->>'bonus_pe')::numeric + (p_resultado->>'bonus_fisico_total')::numeric
    ...
    raise exception 'writer V4: total diverge das parcelas';
  end if;
  ...
  insert into clube_novo.build_bonificador(
    bonus_pe,bonus_fisico_total,bonus_posicao,bonus_playstyle_1,bonus_playstyle_2,bonus_ia,
  ...
  values((p_resultado->>'bonus_pe')::numeric,(p_resultado->>'bonus_fisico_total')::numeric,
    (p_resultado->>'bonus_posicao')::numeric,(p_resultado->>'bonus_playstyle_1')::numeric,
    ...
    r->>'contrato',r->>'contrato_fingerprint',l.carta_versao,c->>'carta_fingerprint',esperado,
  ...
  return jsonb_build_object('readback','ok','gravado',true,'idempotente',false,
    ...
    'carta_versao',l.carta_versao,'carta_fingerprint',c->>'carta_fingerprint','resultado_fingerprint',resultado_fp);
end $function$;
revoke all on function public.gravar_build_bonificador_v4(jsonb) from public,anon,authenticated;
grant execute on function public.gravar_build_bonificador_v4(jsonb) to service_role;
grant execute on function public.gravar_build_bonificador_v4(jsonb) to bonificador_runtime;
commit;
