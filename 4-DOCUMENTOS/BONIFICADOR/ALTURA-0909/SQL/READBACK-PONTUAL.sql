...
begin
  ...
  begin
    ...
    if not exists(select 1 from clube_novo.build_pontuacao_final_v2_delta_v1 p
      join clube_novo.carta_jogo cj on cj.card_id=p.card_id
      join clube_novo.build_linha_card bl on bl.id=p.linha_id
      join clube_novo.funcao_sistema fs on fs.id=p.funcao_id
      join clube_novo.posicao_jogo pj on pj.id=p.posicao_id
