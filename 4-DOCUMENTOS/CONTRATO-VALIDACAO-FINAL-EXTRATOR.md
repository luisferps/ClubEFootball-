...
## Catálogo de endereços por view e escritores declarativos — 29/08/2026

- Snapshot recuperável anterior: `7-VARREDURA-DO-JOGO\RECUPERACAO\20260829-antes-envelopes-escritores-declarativos`. Rollbacks cirúrgicos: `ROLLBACK-ENVELOPES-ESCRITORES-DECLARATIVOS-V1.sql` e `ROLLBACK-CATALOGO-ENDERECOS-VIEW-V1.sql`.
- Substituição: `executor\executor_local.py` e `executor\desktop_physical_worker.js` deixaram de montar papéis/fontes a partir de listas locais. Eles consomem `catalogo_enderecos` do pedido, derivado exclusivamente da view read-only `clube_novo.catalogo_endereco_leitura_extrator_v1`; a view mantém as FKs de família/campo e somente projeta localização e procedência das tabelas canônicas.
- Cobertura comprovada da view: 602 linhas de localização, 214 `campo_id` distintos e os quatro papéis físicos `dt200`, `dt261_bra`, `dt870_original` e `dt870_updated`. `arquivos`, `campos` e `localizadores_fontes` devolvidos pela função do pedido são projeções da mesma view. A primeira execução expôs que `all.str` recebia incorretamente o papel expandido; a correção passou a usar `papel_fonte_arquivo_canonico`. Não houve fallback nem escrita de domínio.
- Escritores: `contrato_leitura_escritor_dominio`/`contrato_leitura_escritor_destino` declaram sete escritores e 29 destinos (1 Cartas, 2 Catálogos, 3 Dimensões, 11 Ímpetos, 7 Relações, 4 Técnicos e 1 Textos). `desktop_worker.py` substitui a seleção hard-coded de destino pela validação de envelopes tipados, FKs/chaves, colunas permitidas, tipos, procedência e SQL parametrizado formado do pedido.
- Prova V5/read-only: `artefatos\desktop\smoke-catalogo-view-20260829-223810` concluiu todas as conferências, `database_write=false`, e produziu o pacote `2eea6148608e9184869960d98fc8cdb7985e089f6ae5a56421d05419508cd156`. O aceite interno e `--apply-review --test-rollback` completaram estágio, auditoria/readback e rollback para os 29 destinos, com zero envelopes/dados físicos escritos; o aceite de teste foi restaurado e a política final permaneceu fechada. `PRODUCTIVE_WRITES_LOCKED=true` continua obrigatório até uma autorização posterior para aplicar envelopes reais.

### Retificação de comparação canônica de Cartas — 29/08/2026

`desktop_physical_worker.js` deixou de gerar `cartas-divergencias.json` com
`compareCardRows`/CSV de apresentação, que classificava rótulos e ordem de
listas como mudança. `desktop_worker.py` também deixou de encaminhar a baseline
CSV. A única comparação de Cartas da V5 passa a ser
`classify_canonical_cards`, após Dimensões, usando `projecoes_cartas` do
pedido, `card_id`, FKs/códigos físicos e baseline canônica read-only. A troca
não altera fonte, banco, jogo nem domínio.
### Fronteira de destino — 29/08/2026

O catálogo/view e suas tabelas de origem são exclusivamente de leitura. A
fotografia física retorna envelopes ao estágio/pacote de revisão, a V5 liga o
aceite ao hash e somente o aplicador transacional pode atingir tabelas canônicas
de domínio declaradas pelo contrato. Nenhuma escrita aponta para mapa,
