...
alter table clube_novo.build_linha_card
  add column atributos_snapshot jsonb,
  add column atributos_snapshot_fingerprint text,
  add column snapshot_otimizador_fingerprint text,
  add column snapshot_bonificador_fingerprint text,
  add constraint build_linha_card_atributos_snapshot_check
    check (atributos_snapshot is null or jsonb_typeof(atributos_snapshot) = 'object');

comment on column clube_novo.build_linha_card.atributos_snapshot is
  'Snapshot por codigo canonico de atributo: valor_inicial, etapas reais exibidas e valor_final. Captura saida; nao recalcula formula.';
...
begin
  ...
  loop
    if not exists (
      select 1 from clube_novo.atributo_jogo a where a.codigo = v_item.key
    ) then
