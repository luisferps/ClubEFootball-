CREATE OR REPLACE FUNCTION clube_novo.finalizar_publicar_linha_antes_normalizacao_0909(p_linha_id bigint, p_origem text DEFAULT 'desconhecida'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
revoke all on function clube_novo.finalizar_publicar_linha_antes_normalizacao_0909(bigint,text) from public,anon,authenticated;
-- A finalização mantém todos os gates existentes. A nova composição ocorre
-- na mesma transação, antes que o resultado fique visível para leitores.
create or replace function clube_novo.finalizar_publicar_linha_v1(p_linha_id bigint,p_origem text default 'desconhecida')
...
begin
 begin
   ...
   if (r->>'ok')::boolean is true and r->>'estado' in ('publicada','ja_publicada') then
     ...
     r:=r||jsonb_build_object('nota_final',n->'nota_final','normalizacao_versao','normalizacao-bonus-integral-20260909-v1');
   end if;
   return r;
 exception when others then
   get stacked diagnostics v_erro=message_text;
...
update clube_novo.normalizacao_regra_v3 set estado='vigente' where versao='normalizacao-bonus-integral-20260909-v1';
...
begin
 ...
 if tg_op='UPDATE' and old.build_otimizador_id is not distinct from new.build_otimizador_id
    ...
    and new.proveniencia#>>'{normalizacao,versao}'='normalizacao-bonus-integral-20260909-v1' then return new;end if;
