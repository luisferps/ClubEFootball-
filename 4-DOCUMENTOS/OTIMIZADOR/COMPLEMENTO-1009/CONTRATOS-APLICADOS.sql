CREATE OR REPLACE FUNCTION clube_novo.registrar_complemento_v14(p_build bigint, p_resultado jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
declare hs integer[];
begin
 if not p_resultado ? 'complemento_contexto_fingerprint' then return; end if;
 select coalesce(array_agg(value::integer),'{}') into hs from jsonb_array_elements_text(p_resultado->'habilidades_complementares');
 insert into clube_novo.build_complemento_v14 values(p_build,p_resultado->>'complemento_contexto_fingerprint',hs,p_resultado->'complemento_habilidades') on conflict do nothing;
end $function$
;
...
begin
 ...
 o.builds_comparadas,o.builds_possiveis,fis,internos,o.arows_snapshot) returning id into novo;
...
CREATE OR REPLACE FUNCTION public.complemento_contexto_v14()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
declare d jsonb; fp text; inc jsonb;
begin
 inc:=clube_novo.habilidade_incidencia_snapshot_v14();
 select jsonb_build_object(
 'versao', 'complemento-nativo-10pct-20260910-v2',
 'ranking',inc->'ranking',
 'catalogo',(select jsonb_agg(jsonb_build_object('skill_id',h.skill_id,'tipo',h.tipo,'fabricavel',h.fabricavel,'pode_rodar',h.pode_rodar,'vetada',h.vetada,'efeito_por_codigo',h.efeito_por_codigo) order by h.skill_id) from clube_novo.habilidade_jogo h),
 'bloqueios',(select coalesce(jsonb_agg(jsonb_build_object('funcao_id',b.funcao_id,'skill_id',b.skill_id) order by b.funcao_id,b.skill_id),'[]'::jsonb) from clube_novo.habilidade_funcao_bloqueio_otimizador b),
 'pesos',(select jsonb_agg(jsonb_build_object('funcao_id',m.funcao_id,'codigo_atributo',m.codigo_atributo,'peso',m.peso) order by m.funcao_id,m.codigo_atributo) from clube_novo.otimizador_molde m where m.versao=5)
 ) into d;
 fp:=encode(extensions.digest(convert_to(d::text,'UTF8'),'sha256'),'hex');
 insert into clube_novo.complemento_contexto_v14(fingerprint,dados) values(fp,d) on conflict do nothing;
 return d||jsonb_build_object('fingerprint',fp);
end $function$
;

CREATE OR REPLACE FUNCTION public.complemento_importar_lote_v14(p_contexto text, p_resultados jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
declare item jsonb; saida jsonb:='[]'; r jsonb;
begin
 if jsonb_typeof(p_resultados)<>'array' or jsonb_array_length(p_resultados)>100 then raise exception 'Lote deve conter ate 100 resultados'; end if;
 for item in select value from jsonb_array_elements(p_resultados) loop
  begin
   r:=public.complemento_importar_v14((item->>'n')::bigint,p_contexto,item);
   saida:=saida||jsonb_build_array(r);
  exception when others then
   saida:=saida||jsonb_build_array(jsonb_build_object('ok',false,'linha_id',item->'n','erro',sqlerrm));
  end;
 end loop;
 return saida;
end $function$
...
CREATE OR REPLACE FUNCTION public.complemento_exportar_v14(p_depois_id bigint DEFAULT 0, p_limite integer DEFAULT 100)
 RETURNS jsonb
 LANGUAGE sql
...
select coalesce(jsonb_agg(jsonb_build_object(
 'linha',to_jsonb(t.l)||jsonb_build_object('linha_id',(t.l).id),
 'anterior',to_jsonb(t.o),'carta',public.otimizador_carta_v3((t.l).card_id)
) order by (t.l).id),'[]'::jsonb)
from (
 select l,o from clube_novo.build_linha_card l
 join clube_novo.build_otimizador o on o.id=l.build_otimizador_id
 join clube_novo.carta_jogo c on c.card_id=l.card_id
 where l.estado<>'invalida' and l.id>p_depois_id and l.execucao_tipo='producao' and l.lote_teste_id is null
 and l.estado_otimizador='concluido' and cardinality(o.habilidades_adicionais)<5
 and c.orcamento>0 and c.sem_evolucao=false
 and c.codigo_tipo_carta_fisico in(0,1,5,6)
 and o.atributos_finais is not null and o.atributos_internos is not null and o.arows_snapshot is not null
 and o.carta_fingerprint is not distinct from l.carta_fingerprint
 and not exists(select 1 from clube_novo.otimizador_lote_producao_linha_v3 q where q.linha_id=l.id and q.finalizada_em is null)
 and not exists(select 1 from clube_novo.orcamento_revisao_linha_v1 q where q.linha_anterior_id=l.id)
 and not exists(select 1 from clube_novo.complemento_recibo_v14 r where r.build_novo_id=o.id)
 order by l.id limit greatest(1,least(p_limite,100))
) t
$function$
;
