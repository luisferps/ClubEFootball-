-- Rollback da operação local em JSON V68.
--
-- Restaura integralmente a V67, inclusive a conferência do fingerprint agregado
-- no histórico de eventos preparo_integral_concluido. Não apaga resultado.
--
-- A V67 corrigia somente a porta usada por ENVIAR-RESULTADOS.bat. O lote produtivo
-- principal é incremental: seu fingerprint agregado muda quando novas cartas
-- são preparadas. Resultados já calculados no disco, porém, pertencem a linhas
-- que guardam seus próprios selos esperados. Comparar o JSON com o estado
-- agregado atual do lote recusava uma linha válida depois de uma ampliação não
-- relacionada da fila e também quebrava a repetição idempotente.
--
-- A V67 mantém a proteção fail-closed por identidade, fotografia da entrada,
-- fórmula, contrato, motor, Ímpeto condicional e resultado. Fórmula, contrato e
-- motor passam a ser conferidos e persistidos pelos campos esperados da própria
-- linha. O fingerprint agregado recebido continua obrigatório e precisa existir
-- no histórico de preparos concluídos do próprio lote, mas não é comparado
-- somente ao valor mutável atual.
--
-- Não reserva linha, não inicia motor, não publica Build e não toca legado.
...
begin
  if to_regclass('clube_novo.otimizador_lote_producao_v3') is null
     ...
     or to_regprocedure('clube_novo.conferir_impeto_x_posicao_v1(integer,integer)') is null
     or to_regprocedure('clube_novo.arows_da_cadeia_v1(jsonb)') is null
     or to_regprocedure('public.otimizador_producao_importar_json_local_v1(uuid,bigint,jsonb,timestamptz)') is null then
    raise exception 'V67 recusada: contratos atuais da importação local ausentes';
  end if;

  if not exists (
       select 1 from information_schema.columns
       where table_schema = 'clube_novo' and table_name = 'build_linha_card'
         and column_name = 'otimizador_formula_fingerprint_esperado'
     )
     or not exists (
       select 1 from information_schema.columns
       where table_schema = 'clube_novo' and table_name = 'build_linha_card'
         and column_name = 'otimizador_contrato_fingerprint_esperado'
     )
     or not exists (
       select 1 from information_schema.columns
       where table_schema = 'clube_novo' and table_name = 'build_linha_card'
         and column_name = 'otimizador_motor_versao_esperada'
     ) then
    raise exception 'V67 recusada: selos esperados por linha ausentes';
  end if;
end
$$;
  ...
  if not exists (
    select 1
     ...
     where e.lote_id = p_lote_id
       and e.evento = 'preparo_integral_concluido'
       and e.detalhe->>'fingerprint' = v_lote_fingerprint_recebido
  ) then
    raise exception 'importação JSON recusada: fingerprint do lote não pertence ao histórico confirmado';
  end if;

  -- O fingerprint acima registra uma fotografia realmente concluída do lote e
  -- fica no fingerprint integral do resultado. Não pode ser igualado somente a
  -- v_lote.fingerprint: esse agregado muda quando a fila recebe cartas novas,
  -- sem mudar esta linha.
...
comment on function public.otimizador_producao_importar_json_local_v1(
  ...
  'V67: importa resultado local pelos selos imutáveis esperados da linha; o fingerprint agregado do lote é auditado, não usado para invalidar linha estável.';
