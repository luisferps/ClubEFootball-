-- Clonagem seletiva por orçamento, sem iniciar cálculo e sem alterar a linha anterior.
begin;
create table if not exists clube_novo.orcamento_revisao_linha_v1(
 linha_anterior_id bigint primary key references clube_novo.build_linha_card(id) on delete restrict,
 linha_nova_id bigint unique references clube_novo.build_linha_card(id) on delete restrict,
 lote_corretivo_id uuid not null references clube_novo.otimizador_lote_producao_v3(id) on delete restrict,
 orcamento_anterior integer not null, orcamento_real integer not null,
 captura_id uuid not null, revisada_em timestamptz not null default clock_timestamp(),
 motivo text not null, estado text not null default 'preparada',
 linha_anterior_snapshot jsonb not null, publicacao_anterior_snapshot jsonb,
 entrada_anterior_snapshot jsonb not null,
 check(orcamento_anterior<>orcamento_real)
);
alter table clube_novo.orcamento_revisao_linha_v1 enable row level security;
revoke all on clube_novo.orcamento_revisao_linha_v1 from public,anon,authenticated;
grant select,insert,update on clube_novo.orcamento_revisao_linha_v1 to service_role;

create or replace function public.otimizador_preparar_revisao_orcamento_v1(
 p_lote_modelo uuid,p_lote_corretivo uuid,p_linhas bigint[]
...
begin
 ...
 foreach v_id in array p_linhas loop
   ...
   if not found or v_e.nivel_maximo<1 or v_e.orcamento_real<>2*(v_e.nivel_maximo-1)
     or v_e.orcamento_real is not distinct from (v_s.entrada_otimizador#>>'{escalares,orcamento}')::integer
   then raise exception 'Linha % sem divergência comprovada',v_id; end if;
   ...
   v_entrada:=jsonb_set(v_s.entrada_otimizador,'{escalares}',(v_s.entrada_otimizador->'escalares')||
     jsonb_build_object('level_cap',v_e.nivel_maximo,'orcamento',v_e.orcamento_real,'cap_estimado',false),false)||
   ...
   values(v_id,p_lote_corretivo,(v_s.entrada_otimizador#>>'{escalares,orcamento}')::integer,
      v_e.orcamento_real,v_e.captura_id,'orcamento_divergente_da_evidencia_fisica',
...
revoke all on function public.otimizador_preparar_revisao_orcamento_v1(uuid,uuid,bigint[]) from public,anon,authenticated;
grant execute on function public.otimizador_preparar_revisao_orcamento_v1(uuid,uuid,bigint[]) to service_role;
commit;
