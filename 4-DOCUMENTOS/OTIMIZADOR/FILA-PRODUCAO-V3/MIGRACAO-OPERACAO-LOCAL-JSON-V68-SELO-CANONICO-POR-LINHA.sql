-- Operação local em JSON V68 — selo canônico por linha.
--
-- Corrige somente a porta usada pelo MAQUINA2-ENVIAR-DO-DISCO.bat. O lote produtivo
...
-- linha. O fingerprint agregado recebido continua obrigatório, bem formado e
-- auditado no resultado, mas não é selo canônico da linha: ele não foi
-- preservado por linha nem integralmente no histórico do lote.
--
-- A V67 tentou conferir esse agregado no evento preparo_integral_concluido. O
-- teste real da Máquina 2 provou que o histórico tem só seis snapshots e não
-- contém o fingerprint do pacote que antes já havia importado mais de 50 mil
-- linhas. A V68 retira somente essa condição sem prova física.
--
...
comment on function public.otimizador_producao_importar_json_local_v1(
  ...
  'V68: importa resultado local pelos selos imutáveis esperados da linha; o fingerprint agregado do pacote é obrigatório e auditado, não usado como selo canônico ausente da linha.';
