-- V3.1 — piloto realmente limitado, sem alteração de fórmula, pesos, regras
...
-- de aplicar p_limite_cards. Com 20 mil candidatas, nem um piloto de 3 cartas
-- cabia no timeout do contrato. Para p_limite_cards > 0, a janela determinística
-- de candidatas é limitada antes das projeções. Para p_limite_cards = 0, o
-- universo e a ordem de produção permanecem os mesmos da V3 original.
...
begin
  ...
  -- Produção integral preserva a janela original. O piloto lê uma janela
  -- determinística de 20 candidatas por carta pedida, sem inferir resultado.
  v_janela_consulta:=case when coalesce(p_limite_cards,0)=0 then 50000
                           else least(50000,greatest(p_limite_cards,p_limite_cards*20)) end;
