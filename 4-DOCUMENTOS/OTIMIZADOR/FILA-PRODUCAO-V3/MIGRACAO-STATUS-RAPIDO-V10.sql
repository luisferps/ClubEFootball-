-- V10 — status da fila integral sem varrer a linhagem duas vezes.
--
-- Escopo: somente as leituras privadas de status V5 (painel integral) e V3
-- (controle interno do worker).
-- Não altera fórmula, pesos, moldes, snapshots, linhas, estados, publicação ou
-- worker. O contrato continua V5 e só service_role pode executá-lo.
--
-- Causa: no lote integral de 184.702 linhas a versão original contava e juntava
-- toda a tabela de linhagem com build_linha_card, inclusive COUNT(DISTINCT), a
-- cada atualização visual. A chamada REST passou a expirar com SQLSTATE 57014.
-- A tabela do lote já mantém cards/linhas/preparo de forma transacional; os
-- estados das linhas são contados diretamente por lote_producao_id, cujo índice
-- parcial V5 já é a rota canônica de leitura.

begin;

do $$
begin
  if to_regprocedure('public.otimizador_producao_status_v5(uuid)') is null then
    raise exception 'V10 recusada: contrato V5 de status não existe';
  end if;
end
$$;
...
begin
  ...
  -- Os dois valores são incrementados no mesmo commit que sela cada candidata.
  -- Evita COUNT(DISTINCT card_id) e a junção integral de 184 mil linhas.
  v_cards:=v_lote.cards;
  v_linhas:=v_lote.linhas;

  -- A V5 já possui build_linha_card_lote_producao_v5_idx em
  -- (lote_producao_id, estado_otimizador). Esta leitura não passa pela
  -- tabela de linhagem salvo para as poucas linhas ativas/detalhes abaixo.
  select
  ...
  if v_lote.tipo_lote='integral' then
    ...
    v_preparo_pendentes:=greatest(0,v_preparo_total-v_preparo_concluido);
    v_preparo_incompletas:=v_lote.excluidas_incompletas;
    v_preparo_sem_linha:=v_lote.excluidas_sem_linha;
    -- Preparo falha fechado na primeira candidata divergente. Só nesse caso a
    -- leitura pontual do catálogo de candidatas é necessária para o detalhe.
  ...
  where l.lote_producao_id=v_lote.id and l.estado_otimizador='processando';

  -- Motivos detalhados pertencem à leitura paginada. Para manter a resposta de
  -- status limitada depois de um encerramento em massa, preserva-se o detalhe
  -- completo apenas enquanto houver até 100 finais com motivo.
  if v_bloqueadas+v_interrompidas<=100 then
    select coalesce(jsonb_agg(jsonb_build_object(
      'linha_id',l.id,'card_id',l.card_id,'funcao_id',l.funcao_id,'posicao_id',l.posicao_id,
      'estado',l.estado_otimizador,'motivo',l.erro_otimizador
    ) order by q.ordem_fila),'[]'::jsonb)
...
-- O worker consulta V3 entre linhas para observar pausa/encerramento. Ele tinha
-- a mesma varredura integral da V5; mantê-la lenta faria o painel voltar mas
-- deixaria a esteira travar depois da primeira conclusão.
...
begin
  ...
  -- Contadores de identidade são selados no lote. Estados são lidos pelo
  -- índice parcial V5 diretamente da linha, sem juntar toda a linhagem.
  v_cards:=v_lote.cards;
  v_linhas:=v_lote.linhas;
  select
    count(*) filter(where l.estado_otimizador='pendente'),
  ...
  -- A página de fila contém os motivos completos. O status é limitado para
  -- não materializar centenas de milhares de itens após encerramento em massa.
  if v_bloqueadas+v_interrompidas<=100 then
    select coalesce(jsonb_agg(jsonb_build_object(
      'linha_id',l.id,'card_id',l.card_id,'funcao_id',l.funcao_id,'posicao_id',l.posicao_id,
      'estado',l.estado_otimizador,'motivo',l.erro_otimizador
