...
MIGRACAO_PILOTO = RAIZ / "4-DOCUMENTOS" / "OTIMIZADOR" / "FILA-PRODUCAO-V3" / "MIGRACAO-FILA-PRODUCAO-V3-PILOTO-LOTE-ID-V3.sql"
ROLLBACK_PILOTO = RAIZ / "4-DOCUMENTOS" / "OTIMIZADOR" / "FILA-PRODUCAO-V3" / "ROLLBACK-FILA-PRODUCAO-V3-PILOTO-LOTE-ID-V3.sql"
MIGRACAO_CONCLUSAO = RAIZ / "4-DOCUMENTOS" / "OTIMIZADOR" / "FILA-PRODUCAO-V3" / "MIGRACAO-FILA-PRODUCAO-V3-CONCLUSAO-IDENTITY-V4.sql"
ROLLBACK_CONCLUSAO = RAIZ / "4-DOCUMENTOS" / "OTIMIZADOR" / "FILA-PRODUCAO-V3" / "ROLLBACK-FILA-PRODUCAO-V3-CONCLUSAO-IDENTITY-V4.sql"
...
class GatewayDaFila:
    ...
    def rpc(self, nome, corpo=None):
        ...
        if nome == "otimizador_producao_bloquear_linha_v3":
            self.bloqueios.append(corpo)
            return {"contrato": fila.CONTRATO, "linha_id": 7001, "estado": "bloqueado"}
        # O worker consulta o contrato compacto de controle depois de cada
        # unidade atômica; a paginação/status rico não participa da pausa.
...
class WorkerSemTecnico(WorkerControlado):
    def _calcular(self, item):
        resultado = super()._calcular(item)
        resultado["tecnico_id"] = None
        return resultado


class RecusaConfirmada(Exception):
    rejeicao_confirmada_sem_gravacao = True


class WorkerComConclusaoRecusada(WorkerControlado):
    def _concluir(self, _item, _resultado):
        raise RecusaConfirmada("o contrato recusou a gravação antes de salvar a linha")
...
class FilaProducaoV3Test(unittest.TestCase):
    ...
    def test_saida_sem_tecnico_e_bloqueada_antes_da_gravacao(self):
        gateway = GatewayDaFila()
        worker = WorkerSemTecnico(gateway, "00000000-0000-0000-0000-000000000123", esperar=0.01)
        final = worker.executar()
        self.assertEqual(final["estado_lote"], "pausado")
        self.assertEqual(gateway.resultados, [])
        self.assertEqual(len(gateway.bloqueios), 1)
        self.assertIn("tecnico_id canônico", gateway.bloqueios[0]["p_motivo"])

    def test_recusa_confirmada_da_gravacao_bloqueia_so_a_linha(self):
        gateway = GatewayDaFila()
        worker = WorkerComConclusaoRecusada(
            gateway, "00000000-0000-0000-0000-000000000123", esperar=0.01
        )
        final = worker.executar()
        self.assertEqual(final["estado_lote"], "pausado")
        self.assertEqual(gateway.resultados, [])
        self.assertEqual(len(gateway.bloqueios), 1)
        self.assertIn("recusado pelo contrato", gateway.bloqueios[0]["p_motivo"])
    ...
    def test_piloto_limitado_corta_antes_das_projecoes_sem_tocar_na_formula(self):
        migracao = MIGRACAO_PILOTO.read_text(encoding="utf-8")
        rollback = ROLLBACK_PILOTO.read_text(encoding="utf-8")
        self.assertIn("_otimizador_producao_candidatas_base_v3", migracao)
        self.assertIn("limit v_janela_consulta", migracao)
        self.assertIn("p_limite_cards+2", migracao)
    ...
    def test_conclusao_respeita_pk_identity_do_build(self):
        migracao = MIGRACAO_CONCLUSAO.read_text(encoding="utf-8")
        rollback = ROLLBACK_CONCLUSAO.read_text(encoding="utf-8")
        self.assertIn("insert into clube_novo.build_otimizador(\n    tecnico_id", migracao)
        self.assertNotIn("nextval('clube_novo.build_otimizador_id_seq'", migracao)
        self.assertIn("returning id into v_build_id", migracao)
        self.assertIn("nextval('clube_novo.build_otimizador_id_seq'", rollback)
        self.assertIn("pode_publicar',false", migracao)
