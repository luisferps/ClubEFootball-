...
function aplicarAcoes(acoes,confirmacao,estado){const a=acoes||{},c=confirmacao||{};const iniciar=$('#iniciar-fila'),recuperar=$('#recuperar-fila');iniciar.disabled=!(a.iniciar===true||a.retomar===true||a.criar===true||a.preparar===true);iniciar.textContent=a.iniciar===true?'Iniciar esteira':estado==='preparo_pausado'?'Retomar esteira':a.retomar===true?'Retomar':a.criar===true?'Iniciar fila integral':a.preparar===true?'Ligando esteira…':'Iniciar';recuperar.hidden=a.recuperar!==true;recuperar.disabled=!(a.recuperar===true&&c.recuperar_exige_confirmacao===true);$('#pausar-fila').disabled=a.pausar!==true;$('#pausar-fila').textContent=estado==='preparando'?'Pausar preparação':'Pausar';$('#parar-fila').disabled=!(a.parar===true&&c.parar_exige_confirmacao===true);}
function agendarAtualizacaoFila(estado){if(temporizadorFila!==null){window.clearTimeout(temporizadorFila);temporizadorFila=null;}const atraso=estado==='consulta_indisponivel'?5000:['preparando','rodando','pausando','encerrando'].includes(estado)?3000:null;if(atraso!==null)temporizadorFila=window.setTimeout(atualizarFila,atraso);}
...
function pintarFila(d){
  ...
  $('#fila-aviso').textContent=workerLocalAusente?(d.acoes&&d.acoes.recuperar===true?'A linha '+atual.linha_id+' ficou reservada sem worker local. Clique em Recuperar fila para devolvê-la a pendente e deixar a rodada pausada; as concluídas não serão tocadas.':'A fila registra a linha '+atual.linha_id+' como processando, mas este computador não tem worker local ativo. Nenhuma nova linha será calculada até a recuperação segura; não feche nem reinicie por tentativa.'):(d.mensagem||mensagemDaRodada(estado,t));
