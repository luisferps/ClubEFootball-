...
INTERFACE_VERSAO = "20260831-v22"
FILA_V3_INDISPONIVEL = (
    "Fila operacional V3 ainda não foi autorizada. O painel não consulta nem "
    "executa a rodada histórica; use apenas Teste unitário até existir contrato "
    "V3 selado em clube_novo."
)
RPC_PERMITIDAS = {
...
class ServicoOtimizador:
    ...
    def _status_fila(self):
        """A fila só pode existir quando o contrato V3 específico for autorizado."""
        return self._fila_v3_indisponivel()
    ...
    def _fila_v3_indisponivel():
        ...
        return {
            ...
            "mensagem": FILA_V3_INDISPONIVEL,
            "origem": "clube_novo somente; não há contrato de fila V3 autorizado",
            "publicacao": "TESTE / NÃO PUBLICADO", "pode_publicar": False,
        }
    ...
    def eventos_fila(self):
        indisponivel = self._fila_v3_indisponivel()
        return {"ok": True, "disponivel": False, "estado": indisponivel["estado"],
                "itens": [], "mensagem": FILA_V3_INDISPONIVEL,
                "origem": indisponivel["origem"]}

    def resultados_fila(self):
        indisponivel = self._fila_v3_indisponivel()
        return {"ok": True, "disponivel": False, "estado": indisponivel["estado"],
                "mensagem": FILA_V3_INDISPONIVEL, "itens": [],
    ...
    def _rodar_worker_selado(self):
        raise ErroDaInterface(FILA_V3_INDISPONIVEL, 409)
    ...
    def saude(self):
        ...
        return {
            ...
            "modo": "consulta_local_v3; fila_v3_aguardando_contrato",
