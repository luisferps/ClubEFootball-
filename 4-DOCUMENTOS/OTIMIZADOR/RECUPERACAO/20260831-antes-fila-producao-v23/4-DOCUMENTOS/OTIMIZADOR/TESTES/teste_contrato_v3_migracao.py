...
LANCADOR_FILA = MOTOR / "RODAR-COMPARACAO-LEGADO-50-CARDS.bat"
RODA_LOTE = MOTOR / "roda_lote_v6.py"
...
class ContratoV3MigracaoTest(unittest.TestCase):
    ...
    def test_relacao_funcao_posicao_por_fk_e_entrada_historica_esta_inerte(self):
        ...
        self.assertIn("frente de legado foi encerrada", fila)
        self.assertNotIn("fonte_unica", fila)
        self.assertNotIn("otimizador_", fila)
    ...
    def test_entradas_historicas_nao_possuem_caminho_executavel(self):
        lancador = LANCADOR_FILA.read_text(encoding="utf-8")
        fila = FILA.read_text(encoding="utf-8")
        roda_lote = RODA_LOTE.read_text(encoding="utf-8")
        self.assertIn("exit /b 1", lancador.lower())
        self.assertNotIn("python", lancador.lower())
        self.assertIn("raise SystemExit(MENSAGEM)", fila)
        inicio_main = roda_lote.index("def main():")
        inicio_import = roda_lote.index("    import multiprocessing", inicio_main)
        self.assertIn("raise SystemExit(", roda_lote[inicio_main:inicio_import])

    def test_auditoria_le_somente_o_contrato_novo_v3(self):
        ...
        self.assertIn("audita_cartas_v3", auditor)
        self.assertNotIn("cartas_do_motor", auditor)
