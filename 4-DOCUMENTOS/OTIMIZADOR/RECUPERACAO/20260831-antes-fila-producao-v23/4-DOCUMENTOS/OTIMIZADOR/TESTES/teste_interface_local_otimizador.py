...
class ServicoFalso:
    ...
    def _fila():
        return {
            ...
            "mensagem": "Fila operacional V3 ainda não foi autorizada.",
            "origem": "clube_novo somente; contrato de fila V3 ausente",
        }
    ...
    def resultados_fila(self):
        dados = self._fila()
        return {"ok": True, "disponivel": False, "estado": dados["estado"],
                "itens": [], "mensagem": dados["mensagem"], "publicacao": "TESTE / NÃO PUBLICADO"}

    @staticmethod
    def _bloqueia():
        raise mod.ErroDaInterface("Fila operacional V3 ainda não foi autorizada.", 409)
...
class InterfaceOtimizadorTest(unittest.TestCase):
    ...
    def test_fila_e_resultados_ficam_honestamente_indisponiveis_sem_contrato_v3(self):
        for rota in ("/api/fila/status", "/api/fila/eventos", "/api/resultados"):
            ...
            self.assertEqual(resposta["estado"], "aguardando_fila_v3")
    ...
    def test_rotulos_continuam_resolvidos_por_ids_canonicos(self):
        ...
        self.assertIn("aguardando_fila_v3", js)
    ...
    def test_servico_real_recusa_controles_sem_consultar_gateway(self):
        ...
        self.assertEqual(status["estado"], "aguardando_fila_v3")
        for chamada in (servico.iniciar_fila, servico.pausar_fila,
    ...
    def test_executavel_reconhece_a_interface_v22(self):
        ...
        self.assertIn('ExpectedVersion = "\\\"versao_interface\\\": \\"20260831-v22\\\""', launcher)
