...
class GatewayOtimizador:
    ...
    def rpc(
        ...
        # A rota privada, quando configurada, é preferida porque não depende do
        # pool público do PostgREST. Ela ainda chama somente contratos V17/V6
        # por ID e falha fechada se o banco não confirmar a leitura.
        erro_privado = None
        if self.banco:
            try:
...
class ServicoOtimizador:
    ...
    def _ler_pagina_operacional(self, lote_id, offset, limite, somente_finais, status):
        ...
        def consultar(grupo, deslocamento, quantidade):
            ...
            resposta = self.gateway.rpc("otimizador_producao_fila_operacional_v3", {
                "p_lote_id": lote_id,
                "p_offset": int(deslocamento),
                "p_limite": int(quantidade),
                "p_grupo": grupo,
            }) or {}
            if resposta.get("contrato") != "otimizador_fila_producao_v6":
                raise ErroDaInterface("leitura operacional da fila V6 inesperada", 503)
            return resposta
        ...
        try:
            ...
            resposta = {
                **status, "ok": True, "itens": itens, "linha_atual": linha_atual,
                ...
                "totais": self._totais_fila(status),
                "publicacao": "SEM PUBLICAÇÃO", "pode_publicar": False,
                "origem": "clube_novo -> contratos V6 por ID -> worker local V3 -> Bonificador",
            }
