...
if str(MOTOR_DIR) not in sys.path:
    # Também cobre os testes isolados que constroem o serviço sem __init__.
...
INTERFACE_VERSAO = "20260901-v52"
JANELA_CACHE_STATUS_SEGUNDOS = 2.0
INTERVALO_RECONEXAO_BANCO_SEGUNDOS = 30.0
FALHA_TRANSPORTE_RECUPERAVEL_V18 = (
    "nenhum contrato seguro respondeu (ponte privada local e Data API); "
    "nenhuma linha foi iniciada"
)
FALHA_RETOMADA_INTEGRAL_POS_PREPARO_V20 = "contrato recusou a consulta (400)"
FALHA_PRE_RESERVA_RECUPERAVEL_V19 = (
    "contrato recusou a consulta (400); ponte privada local também indisponível "
    "(contrato privado local indisponível (CheckViolation))"
)
...
RPC_PERMITIDAS = {
    ...
    "otimizador_rotulos_cartas_fila_v1",
    ...
    "otimizador_producao_eventos_paginados_v5",
...
class ErroDaInterface(Exception):
    """Erro apresentado ao painel local, com classificação segura de transporte.

    ``recuperavel`` não autoriza nem altera uma linha. Ele somente informa ao
    worker que uma queda de rede/contrato deve ser aguardada, em vez de marcar
    todo o lote como falho. Erros de selo, fórmula e contrato incompatível
    continuam fechando o lote normalmente.
    """
    ...
    def __init__(
        ...
        *,
        ...
        rejeicao_confirmada_sem_gravacao: bool = False,
    ):
        super().__init__(mensagem)
        self.status = status
        self.recuperavel = bool(recuperavel)
        # Só é verdadeiro quando o servidor confirmou que a chamada foi
        # recusada antes de alterar estado (por exemplo HTTP 401/403 ou 57014).
        # Timeout e queda de conexão podem ter deixado uma reserva gravada, por
        # isso nunca são repetidos cegamente pelo worker.
        self.repetivel = bool(repetivel)
        # O banco devolveu uma recusa síncrona antes de confirmar uma escrita.
        # O worker pode bloquear somente a linha reservada; timeout e rede nunca
        # recebem este marcador porque podem ter gravado parcialmente.
        self.rejeicao_confirmada_sem_gravacao = bool(rejeicao_confirmada_sem_gravacao)
...
def ler_config() -> tuple[str, str, str]:
    ...
    # A conexão privada é opcional e fica exclusivamente neste computador. Ela
    # não é uma leitura de tabela: chama a allowlist ``otimizador_portal_local_v1``
    # no banco quando a Data API externa estiver indisponível. O nome novo tem
    # precedência; a chave compartilhada existente mantém as cópias já
    # configuradas operando sem exigir que o operador edite um arquivo manualmente.
    banco = (valores.get("OTIMIZADOR_DATABASE_URL", "").strip()
             or valores.get("BONIFICADOR_DATABASE_URL", "").strip())
...
class GatewayOtimizador:
    ...
    def __init__(self):
        ...
        self._banco_lock = threading.RLock()
        self._conexao_privada = None
        self._portal_privado_ativo = None
        self.canal_ativo = "Data API segura"

    def _fechar_conexao_privada(self):
        conexao = self._conexao_privada
        self._conexao_privada = None
    ...
    def _rpc_privado(self, nome: str, corpo: dict | None = None):
        """Chama somente uma ponte allowlist por conexão local protegida.

        A UI nunca recebe essa URL ou credencial. As versões são tentadas em
        ordem compatível para que uma cópia nova do aplicativo continue abrindo
        contra um banco que ainda só tenha a ponte V3. Todas aceitam somente a
        allowlist canônica: não há SQL, tabela nem argumento livre.
        """
        if not self.banco:
            raise ErroDaInterface("conexão privada local não configurada", 503)
        try:
            import psycopg
        except ImportError as erro:
            raise ErroDaInterface(
                "componente local sem a dependência da conexão privada", 503
            ) from erro
        try:
            with self._banco_lock:
                if self._conexao_privada is None or self._conexao_privada.closed:
                    self._conexao_privada = psycopg.connect(
                        self.banco, connect_timeout=8, autocommit=True,
                ...
                for portal in (
                    ...
                    "otimizador_portal_local_v7",
                    "otimizador_portal_local_v6",
                    "otimizador_portal_local_v5",
                    "otimizador_portal_local_v4",
                    "otimizador_portal_local_v3",
                ):
                    try:
                        with self._conexao_privada.cursor() as cursor:
                            # ``portal`` é escolhido de uma tupla interna fixa;
                            # nunca vem do browser nem de um argumento externo.
                            cursor.execute(
                                f"select public.{portal}(%s, %s::jsonb)",
                                (nome, json.dumps(corpo or {}, ensure_ascii=False)),
                            )
                            resposta = cursor.fetchone()
                        self._portal_privado_ativo = portal
                        break
                    except Exception as erro_portal:
                        # Função ainda não instalada: tenta somente a ponte
                        # anterior conhecida. Qualquer outro erro é transporte
                        # ou permissão e não pode ser disfarçado como fallback.
                        if getattr(erro_portal, "sqlstate", None) == "42883":
                            ultimo_erro = erro_portal
                            continue
                        raise
                if resposta is None and ultimo_erro is not None:
                    raise ultimo_erro
            if not resposta:
                return None
            valor = resposta[0]
            ...
            with self._banco_lock:
                self._fechar_conexao_privada()
            raise ErroDaInterface(
                f"contrato privado local indisponível ({type(erro).__name__})", 503,
                recuperavel=True,
            ) from erro
    ...
    def _rpc_data_api(
        ...
        """Rota HTTPS original, preservada para instalações sem ponte privada."""
        ...
        try:
            ...
            if (ausente_ok and erro.code in {400, 404}
                ...
                return None
            if '"code":"57014"' in detalhe:
                raise ErroDaInterface(
                    "o banco excedeu o tempo da consulta de acompanhamento (57014); "
                    "a conexão será tentada novamente sem iniciar outra linha",
                    503, recuperavel=True, repetivel=True,
                ) from erro
            if erro.code in {502, 503, 504} and any(
                ...
                raise ErroDaInterface(
                    ...
                    "a conexão será tentada novamente sem iniciar outra linha",
                    503, recuperavel=True,
                ) from erro
            if erro.code in {401, 403}:
                raise ErroDaInterface(
                    f"a chave local foi recusada pelo contrato ({erro.code}); "
                    "o worker aguardará a correção da conexão sem alterar a fila",
                    503, recuperavel=True, repetivel=True,
                ) from erro
            if erro.code == 400 and nome in {
                "otimizador_producao_concluir_linha_v3",
                "otimizador_producao_concluir_linha_v6",
                "otimizador_producao_concluir_lote_local_v1",
            }:
                raise ErroDaInterface(
                    "o contrato recusou a gravação antes de salvar a linha",
                    400, rejeicao_confirmada_sem_gravacao=True,
                ) from erro
            ...
            raise ErroDaInterface(
                f"não foi possível consultar o contrato ({type(erro).__name__}); "
                "a conexão será tentada novamente sem iniciar outra linha",
                503, recuperavel=True,
            ) from erro
    ...
    def rpc(
        ...
        # A Data API com chave de serviço é o caminho padrão: é idêntica em
        # qualquer Windows e não depende de driver local. A ponte privada é
        # contingência somente para uma falha transitória de transporte; nunca
        # mascara uma recusa determinística do contrato.
        try:
            resposta = self._rpc_data_api(nome, corpo, ausente_ok, timeout_segundos)
            self.canal_ativo = "Data API segura"
            return resposta
        except ErroDaInterface as erro_data_api:
            if not self.banco or not getattr(erro_data_api, "recuperavel", False):
                raise
            try:
                resposta = self._rpc_privado(nome, corpo)
                self.canal_ativo = "contrato privado local " + (self._portal_privado_ativo or "")
                return resposta
            except ErroDaInterface as erro_privado:
                raise ErroDaInterface(
                    f"{erro_data_api}; ponte privada local também indisponível "
                    f"({erro_privado})",
                    503, recuperavel=getattr(erro_data_api, "recuperavel", False),
                    repetivel=getattr(erro_data_api, "repetivel", False),
                ) from erro_data_api
...
class ServicoOtimizador:
    ...
    def __init__(self, gateway=None):
        ...
        # A fotografia integral é baixada uma única vez, enquanto o lote está
        # pausado. Ela fica ao lado do aplicativo e pode ser reconstruída em
        # outro Windows; nunca contém config.txt nem uma credencial.
        self._pacote_thread = None
        self._pacote_lote_id = None
        self._pacote_cancelar = None
        self._pacote_continuar_apos_download = False
        self._pacote_estado = self._estado_local("pacote_nao_baixado")
        self._pacote_apos_preparo_lote_id = None
    ...
    def rotulos_fila(self):
        """Catálogo mínimo, específico da tabela operacional da fila.

        Não carrega a régua completa na abertura: funções e posições são
        apresentação por ID, não insumos do cálculo. Isso mantém a próxima
        linha visível mesmo se o catálogo de técnicos estiver pesado.
        """
        if self._rotulos_fila is None:
            pacote = self.gateway.rpc("otimizador_rotulos_fila_v1") or {}
            if pacote.get("contrato") != "otimizador_rotulos_fila_v1":
                raise ErroDaInterface("rótulos canônicos da fila indisponíveis", 503)
            self._rotulos_fila = pacote
        return self._rotulos_fila
    ...
    def _status_fila(self):
        """Lê o status V6; só consulta V7 no incidente V18 estritamente selado."""
        ...
        with self._status_fila_lock:
            ...
            try:
                # O RPC V6 declara ``p_lote_id uuid DEFAULT NULL``. PostgREST não
                ...
                bruto = self.gateway.rpc(
                    "otimizador_producao_status_v6", {"p_lote_id": None},
            ...
            if bruto is None:
                return self._fila_v5_aguardando_aplicacao()
            if bruto.get("contrato") != "otimizador_fila_producao_v6":
                raise ErroDaInterface("contrato resiliente da fila integral V6 inesperado", 503)
            # A V7 não é uma fonte alternativa. Ela só acrescenta o selo de
            # recuperação para a falha exata registrada antes de existir uma
            # reserva, mantendo o caminho normal em V6 inclusive em bancos que
            # ainda não tenham aplicado V18.
            if (
                bruto.get("estado_lote") == "falhou"
                and bruto.get("falha_lote") == FALHA_TRANSPORTE_RECUPERAVEL_V18
                and int(bruto.get("processando") or 0) == 0
                and int(bruto.get("pendentes") or 0) > 0
                and bruto.get("pode_publicar") is False
            ):
                v7 = self.gateway.rpc(
                    "otimizador_producao_status_v7", {"p_lote_id": bruto.get("lote_id")},
                    ausente_ok=True, timeout_segundos=5,
                )
                if v7 is not None:
                    if v7.get("contrato") != "otimizador_fila_producao_v6":
                        raise ErroDaInterface("contrato de recuperação V18 inesperado", 503)
                    bruto = v7
            resposta = dict(bruto)
            resposta["disponivel"] = True
            resposta["conexao_banco"] = True
            resposta["dados_em_cache"] = False
            self._oferecer_recuperacao_pre_reserva_v19(resposta)
    ...
    def status_fila_rapido(self):
        """Devolve o estado acionável sem aguardar a página de linhas.

        O status V6 já é o contrato resumido para a esteira. A página, os
        rótulos de função/posição e os nomes de carta são apenas apresentação
        e podem demorar mais que o estado do lote. Misturá-los fazia a tela
        parecer travada mesmo quando não havia nenhuma ação pendente. Esta
        rota não calcula, não reserva e não altera a fila.
        """
        try:
            ...
            acoes["recuperar"] = (
                self._recuperacao_reserva_orfa_disponivel(status)
                or self._recuperacao_falha_retomada_integral_v20_disponivel(status)
            )
            ...
            linha_atual = dict(correntes[0]) if correntes else None
            resposta = {
                **status,
                "ok": True,
                "acoes": acoes,
                ...
                "linhas_carregando": True,
                "paginacao": {
                    "total": max(0, int(status.get("linhas") or 0)),
                    "offset": 0,
                    "limite": 100,
                    "somente_finais": False,
                    "ordem": "proximas_primeiro_finais_por_ultimo",
                },
                "totais": self._totais_fila(status),
                "publicacao": "SEM PUBLICAÇÃO",
                "pode_publicar": False,
                "origem": "clube_novo -> fotografia local V52 por ID -> worker local -> envio em lotes",
            }
            return self._anexar_status_pacote_local(resposta)
        except ErroDaInterface as erro:
            if erro.status >= 500:
                self._registrar_indisponibilidade_banco(erro)
                return self._painel_banco_indisponivel(erro, 0, 100, False)
            raise

    def _anexar_status_pacote_local(self, status):
        """Mostra o download local como preparação, sem falsificar o banco.

        A tela atual já atualiza automaticamente o estado ``preparando``. O
        lote real continua ``pausado`` até o arquivo completo passar no hash;
        preservamos esse valor em ``estado_banco`` para diagnóstico.
        """
        lote_id = status.get("lote_id")
        if not lote_id:
            return status
        ativo, pacote = self._estado_pacote_local(lote_id)
        if not ativo:
            return status
        resposta = dict(status)
        acoes = dict(resposta.get("acoes") or {})
        acoes.update({"criar": False, "iniciar": False, "retomar": False, "preparar": False,
                       "pausar": True, "parar": True})
        resposta.update({
            "estado_banco": status.get("estado_lote") or status.get("estado"),
            "estado": "preparando",
            "estado_lote": "preparando",
            "acoes": acoes,
            "mensagem": pacote.get("detalhe") or "Baixando a fotografia local selada.",
            "pacote_local": {
                "estado": pacote.get("etapa") or "baixando_pacote_local",
                "feito": pacote.get("feito"),
                "total": pacote.get("total"),
                "sem_credencial": True,
            },
            "pode_publicar": False,
        })
    ...
    def _carregar_nomes_cartas(self, itens):
        ...
        if not faltantes:
            return
        pacote = self.gateway.rpc("otimizador_rotulos_cartas_fila_v1", {
            "p_card_ids": faltantes,
        }) or {}
        if pacote.get("contrato") != "otimizador_rotulos_cartas_fila_v1":
            raise ErroDaInterface("rótulos oficiais de carta indisponíveis", 503)
    ...
    def _linhas_com_rotulos(self, itens, detalhes=False):
        # A página V4 já traz estes rótulos da mesma view canônica. O catálogo
        # local só é consultado quando uma resposta antiga/incompleta realmente
        # não tem o rótulo; não há busca textual nem fonte paralela.
        precisa_rotulos = any(
            (x.get("funcao_id") is not None and not x.get("funcao_rotulo"))
            or (x.get("posicao_id") is not None and not x.get("posicao_rotulo"))
            for x in itens
        )
        if precisa_rotulos and not self._funcoes_por_id:
            rotulos = self.rotulos_fila()
            self._funcoes_por_id = {
                int(funcao["funcao_id"]): funcao.get("rotulo")
                for funcao in rotulos.get("funcoes") or []
            }
            self._posicoes_por_id = {int(x["posicao_id"]): x.get("rotulo")
                                     for x in rotulos.get("posicoes") or []}
        # A aba Resultados mostra os componentes da build campeã. Só nela (ou
        # se surgir um ímpeto condicional) buscamos o catálogo amplo; a fila
        # normal recebe a tabela imediatamente pelos rótulos mínimos acima.
        precisa_catalogo_amplo = detalhes or any(
            x.get("impeto_condicional_codigo") is not None for x in itens
        )
        if precisa_catalogo_amplo and not self._tecnicos_por_id:
            catalogos = self.catalogos_apresentacao()
            self._tecnicos_por_id = {int(x["tecnico_id"]): x.get("rotulo")
                                     for x in catalogos.get("tecnicos") or []}
        ...
        if any(x.get("card_id") is not None and not x.get("carta_nome") for x in itens):
        ...
        for linha in itens:
            ...
            carta_nome = item.get("carta_nome") or self._nomes_cartas.get(card_id)
            # IDs são identidade de contrato, não conteúdo para o operador.
            # A tela recebe somente o nome oficial; o card_id continua no
            # payload para reserva, vínculo e diagnóstico técnico local.
            item["carta_rotulo"] = carta_nome or "Carta sem nome no catálogo"
            item["funcao_rotulo"] = item.get("funcao_rotulo") or self._funcoes_por_id.get(
                int(funcao_id) if funcao_id is not None else None,
                "Função sem catálogo",
            )
            item["posicao_rotulo"] = item.get("posicao_rotulo") or self._posicoes_por_id.get(
                int(posicao_id) if posicao_id is not None else None,
                "Posição sem catálogo",
            )
            ...
            if codigo_impeto is not None:
                nome_impeto = self._impetos_por_id.get(
                    int(codigo_impeto), "Ímpeto sem catálogo")
            ...
            if precisa_catalogo_amplo and tecnico_id is not None:
                item["tecnico_rotulo"] = self._tecnicos_por_id.get(
                    int(tecnico_id), "Técnico sem catálogo"
                )
            if precisa_catalogo_amplo:
                item["habilidades_adicionais_rotulo"] = [
                    self._habilidades_por_id.get(int(skill_id), "Habilidade sem catálogo")
                    for skill_id in item.get("habilidades_adicionais") or []
                ]
                codigo_adicional = item.get("impeto_adicional_codigo")
                if codigo_adicional is not None:
                    item["impeto_adicional_rotulo"] = self._impetos_por_id.get(
                        int(codigo_adicional), "Ímpeto adicional sem catálogo"
                    )
    ...
    def _ler_pagina_operacional(self, lote_id, offset, limite, somente_finais, status):
        """Lê a fila V6 na ordem útil para operar, sem reordenar a fila real.

        Na aba Fila, a cabeça visual contém primeiro a linha processando e as
        pendentes em ordem_fila crescente. As linhas finais aparecem somente
        depois delas. Na aba Resultados, o recorte final continua mais recente
        primeiro. Nenhuma dessas escolhas muda reserva, cálculo ou estado.
        """
        offset = max(0, int(offset or 0))
        limite = int(limite or 0)
        if limite not in range(1, 201):
            raise ErroDaInterface("limite de paginação fora da faixa", 400)
        abertas = max(0, int(status.get("processando") or 0)) + max(
            0, int(status.get("pendentes") or 0)
        )
        finais = sum(
            max(0, int(status.get(chave) or 0))
            for chave in ("concluidas", "bloqueadas", "interrompidas", "falhas")
        )

        def consultar(grupo, deslocamento, quantidade):
            if quantidade <= 0:
                return {"itens": [], "total": 0}
            resposta = self.gateway.rpc("otimizador_producao_fila_operacional_v4", {
                "p_lote_id": lote_id,
                "p_offset": int(deslocamento),
                "p_limite": int(quantidade),
                "p_grupo": grupo,
            }) or {}
            if (resposta.get("contrato") != "otimizador_fila_producao_v7"
                    or resposta.get("fonte") != "clube_novo.otimizador_entrada_linha_v1"):
                raise ErroDaInterface("leitura operacional da fila V7 inesperada", 503)
            return resposta

        if somente_finais:
            plano = self._plano_pagina_mais_recente(finais, offset, limite)
            resposta = consultar("finais", plano["offset_canonico"], plano["quantidade"])
            itens = list(reversed((resposta.get("itens") or [])[:plano["quantidade"]]))
            total = max(0, int(resposta.get("total") or 0))
            return itens, {
                "total": total, "offset": plano["offset_visual"], "limite": limite,
                "somente_finais": True, "ordem": "mais_recentes_primeiro",
                "abertas": abertas, "finais": total,
            }

        itens = []
        if offset < abertas:
            resposta_abertas = consultar("abertas", offset, limite)
            itens.extend(resposta_abertas.get("itens") or [])
            faltam = max(0, limite - len(itens))
            if faltam:
                resposta_finais = consultar("finais", 0, faltam)
                itens.extend(resposta_finais.get("itens") or [])
        else:
            resposta_finais = consultar("finais", offset - abertas, limite)
            itens.extend(resposta_finais.get("itens") or [])

        return itens[:limite], {
            "total": abertas + finais, "offset": offset, "limite": limite,
            "somente_finais": False,
            "ordem": "proximas_primeiro_finais_por_ultimo",
            "abertas": abertas, "finais": finais,
        }
    ...
    def painel_fila(self, offset=0, limite=100, somente_finais=False):
        try:
            ...
            pagina = {"total": 0, "offset": int(offset), "limite": int(limite),
                      ...
                      "ordem": "proximas_primeiro_finais_por_ultimo"}
            if lote_id:
                bruto, pagina = self._ler_pagina_operacional(
                    lote_id, offset, limite, somente_finais, status
                )
                itens = self._linhas_com_rotulos(bruto, detalhes=bool(somente_finais))
            ...
            resposta = {
                **status, "ok": True, "itens": itens, "linha_atual": linha_atual,
                ...
                "totais": self._totais_fila(status),
                "publicacao": "SEM PUBLICAÇÃO", "pode_publicar": False,
                "origem": "clube_novo.otimizador_entrada_linha_v1 -> fotografia local V52 por ID -> worker local -> envio em lotes",
            }
    ...
    @staticmethod
    def _recuperacao_falha_pre_reserva_v19_disponivel(status):
        """Reconhece somente o incidente comprovado antes de qualquer reserva.

        Não é uma tentativa genérica de "destravar" a fila. A função do banco
        repete a mesma condição exata e confirma que não existe linha ativa.
        """
        try:
        ...
        return (
            ...
            and (status.get("estado_lote") or status.get("estado")) == "falhou"
            and status.get("falha_lote") == FALHA_PRE_RESERVA_RECUPERAVEL_V19
            and status.get("pode_publicar") is False
            and processando == 0
            and pendentes > 0
        )

    def _oferecer_recuperacao_pre_reserva_v19(self, status):
        """Transforma a recuperação já selada pelo banco em um único clique.

        A tela não decide o estado; ela apenas recebe o botão Iniciar quando a
        condição estrita abaixo é verdadeira. Ao clicar, a RPC V19 valida tudo
        novamente antes de mudar ``falhou`` para ``rodando``.
        """
        if not self._recuperacao_falha_pre_reserva_v19_disponivel(status):
            return status
        acoes = dict(status.get("acoes") or {})
        acoes["iniciar"] = True
        acoes["recuperar_pre_reserva"] = True
        status["acoes"] = acoes
        status["recuperacao_pre_reserva_v19"] = True
        status["mensagem"] = (
            "A falha ocorreu antes de qualquer linha ser reservada. Iniciar retoma "
            "somente as pendências, sem duplicar resultado nem publicar."
        )
        return status

    @staticmethod
    def _recuperacao_falha_transporte_disponivel(status):
        """Reconhece exclusivamente o incidente em que o worker não iniciou linha.

        Esta condição é repetida também dentro do banco. O painel só oferece a
        recuperação quando o contrato devolve o selo, mas jamais decide por
        texto de interface, por cache ou por uma linha local.
        """
        try:
            processando = int(status.get("processando") or 0)
            pendentes = int(status.get("pendentes") or 0)
        ...
        return (
            status.get("tipo_lote") == "integral"
            and (status.get("estado_lote") or status.get("estado")) == "falhou"
            and status.get("falha_lote") == FALHA_TRANSPORTE_RECUPERAVEL_V18
            and status.get("pode_publicar") is False
            and processando == 0
            and pendentes > 0
            and (status.get("acoes") or {}).get("retomar") is True
        )

    def _invalidar_cache_status_fila(self):
        """Nunca reutiliza o estado Falhou depois de uma recuperação confirmada."""
        if not hasattr(self, "_status_fila_lock"):
            return
        with self._status_fila_lock:
            self._status_fila_confirmado = None
    ...
    def _recuperacao_falha_retomada_integral_v20_disponivel(self, status):
        """Reconhece somente a reserva sem resultado criada pelo desvio V3.

        Não é um botão genérico para falhas. A função de banco V20 repete os
        mesmos selos e trava a linha; esta parte só evita exibir a ação quando
        este computador ainda tem um worker/preparador local vivo.
        """
        if self._worker_ativo() or self._preparador_ativo():
            return False
        try:
            ...
            preparo_total = int(preparo.get("total") or 0)
            preparo_concluido = int(preparo.get("concluido") or 0)
        except (TypeError, ValueError):
            return False
        correntes = status.get("corrente") or []
        return (
            status.get("tipo_lote") == "integral"
            and (status.get("estado_lote") or status.get("estado")) == "falhou"
            and status.get("falha_lote") == FALHA_RETOMADA_INTEGRAL_POS_PREPARO_V20
            and status.get("pode_publicar") is False
            and preparo_total > 0
    ...
    def _worker_encerrado(self, worker, resultado):
        with self._worker_lock:
            if self._worker_lote_id == worker.lote_id:
                resultado = resultado if isinstance(resultado, dict) else {}
                erro = resultado.get("erro")
                self._progresso_worker(
                    worker,
                    "falha_worker" if erro else "encerrado",
                    detalhe=erro or resultado.get("estado_lote") or resultado.get("estado"),
                )
                self._worker_thread = None
                self._worker_lote_id = None

    def _pacote_local_ativo(self):
        thread = getattr(self, "_pacote_thread", None)
        return bool(thread and thread.is_alive())

    @staticmethod
    def _texto_progresso_pacote(etapa, feito, total):
        nomes = {"cartas": "cartas", "linhas": "linhas da fila"}
        nome = nomes.get(str(etapa), "dados")
        return f"Baixando {nome} para uso local: {int(feito):,}/{int(total):,}.".replace(",", ".")

    def _abrir_pacote_local(self, lote_id, validar=False):
        """Abre apenas a fotografia deste lote, sem consultar tabelas diretamente."""
        from fila_local_v1 import FalhaPacoteLocal, PacoteLocalV1

        try:
            pacote = PacoteLocalV1(PacoteLocalV1.pasta_do_lote(MOTOR_DIR, str(lote_id)))
            if pacote.lote_id != str(lote_id):
                raise FalhaPacoteLocal("a fotografia local pertence a outro lote")
            if validar:
                pacote.validar_integridade()
            return pacote
        except (OSError, FalhaPacoteLocal) as erro:
            raise ErroDaInterface(
                "o pacote local selado ainda não está pronto; nenhuma linha foi iniciada",
                409,
            ) from erro

    def _progresso_pacote_local(self, lote_id, etapa, feito, total):
        with self._worker_lock:
            if self._pacote_lote_id != str(lote_id):
                return
            estado = self._estado_local(
                "baixando_pacote_local", lote_id,
                detalhe=self._texto_progresso_pacote(etapa, feito, total),
            )
            estado.update({"etapa_pacote": str(etapa), "feito": int(feito), "total": int(total)})
            self._pacote_estado = estado

    def _estado_pacote_local(self, lote_id):
        """Estado local só para acompanhar o download; nunca decide o lote."""
        trava = getattr(self, "_worker_lock", None)
        if trava is None:
            return False, {}
        with trava:
            ativo = self._pacote_local_ativo() and self._pacote_lote_id == str(lote_id)
            estado = self._copia_estado_local(getattr(self, "_pacote_estado", {}))
        return ativo, estado

    def _baixar_pacote_local(self, lote_id, continuar_apos_download):
        """Inicia download assíncrono enquanto o lote permanece pausado.

        O clique do operador é suficiente para a sequência toda: quando o
        arquivo verificado termina, este mesmo processo retoma o lote e inicia
        somente o worker que lê o pacote. Não existe caminho de volta para a
        leitura antiga por linha.
        """
        lote_id = str(lote_id)
        with self._worker_lock:
            if self._worker_ativo() or self._preparador_ativo():
                raise ErroDaInterface("não é seguro baixar uma nova fotografia enquanto há trabalho local", 409)
            if self._pacote_local_ativo():
                if self._pacote_lote_id != lote_id:
                    raise ErroDaInterface("já existe download de pacote para outro lote", 409)
                return False
            cancelar = threading.Event()
            self._pacote_cancelar = cancelar
            self._pacote_lote_id = lote_id
            self._pacote_continuar_apos_download = bool(continuar_apos_download)
            self._pacote_estado = self._estado_local(
                "baixando_pacote_local", lote_id,
                detalhe="Conferindo a fotografia selada antes do download local.",
            )
            ...
            def executar_download():
                ...
                try:
                    ...
                    pacote = PacoteLocalV1.criar_do_contrato(
                        self.gateway,
                        lote_id,
                        MOTOR_DIR,
                        ao_progresso=lambda etapa, feito, total: self._progresso_pacote_local(
                            lote_id, etapa, feito, total
                        ),
                        cancelar=cancelar.is_set,
                    )
                    pacote.validar_integridade()
                    if cancelar.is_set():
                        raise RuntimeError("download do pacote local cancelado antes de iniciar uma linha")
                    pronto = True
                    ...
                    with self._worker_lock:
                        continuar = (
                            pronto
                            and not cancelar.is_set()
                            and self._pacote_continuar_apos_download
                            and self._pacote_lote_id == lote_id
                        )
                        self._pacote_thread = None
                        self._pacote_lote_id = None
                        self._pacote_cancelar = None
                        self._pacote_continuar_apos_download = False
                        self._pacote_estado = self._estado_local(
                            "pacote_pronto" if pronto else "pacote_interrompido",
                            lote_id,
                            detalhe=(
                                "Pacote local conferido. Iniciando o cálculo local."
                                if continuar else (erro or "Download do pacote local cancelado.")
                            ),
                        )
                    if continuar:
                        try:
                            self._retomar_worker_do_pacote(lote_id)
                        except Exception as excecao:
                            with self._worker_lock:
                                self._pacote_estado = self._estado_local(
                                    "pacote_pronto_sem_inicio", lote_id, detalhe=str(excecao)
                                )

            thread = threading.Thread(
                target=executar_download,
                name="otimizador-pacote-local-v1",
                daemon=True,
            )
            self._pacote_thread = thread
            thread.start()
            return True

    def _retomar_worker_do_pacote(self, lote_id):
        """Só muda o lote depois de a fotografia integral passar no hash."""
        lote_id = str(lote_id)
        self._abrir_pacote_local(lote_id, validar=True)
        self._invalidar_cache_status_fila()
        status = self._status_fila()
        estado = status.get("estado_lote") or status.get("estado")
        if status.get("tipo_lote") != "integral" or status.get("pode_publicar") is not False:
            raise ErroDaInterface("o pacote local não corresponde a um lote integral sem publicação", 409)
        if int((status.get("preparo") or {}).get("pendentes") or 0) != 0:
            raise ErroDaInterface("a fotografia local não pode iniciar antes de a preparação terminar", 409)
        if int(status.get("processando") or 0) != 0:
            raise ErroDaInterface("há reserva ativa; o pacote local não disputará uma linha", 409)
        if estado == "pausado":
            ...
            if (resposta.get("contrato") != "otimizador_fila_producao_v3"
                    or (resposta.get("estado_lote") or resposta.get("estado")) != "rodando"
                    or resposta.get("pode_publicar") is not False):
                raise ErroDaInterface("o contrato não confirmou a retomada da fotografia local", 503)
        elif estado != "rodando":
            raise ErroDaInterface("o lote não está pausado nem apto para retomar pelo pacote local", 409)
        self._invalidar_cache_status_fila()
        self._iniciar_worker_producao(lote_id, esteira=True)
        return self._inicio_esteira_confirmado(lote_id)

    def _progresso_preparador(self, preparador, etapa, detalhe=None):
        """Mostra o estágio real do produtor sem alterar a fila ou o cálculo."""
        with self._worker_lock:
            if self._preparo_lote_id != preparador.lote_id:
                return
            self._preparo_estado = self._estado_local(
                etapa, preparador.lote_id, detalhe=detalhe,
            )

    def _preparador_encerrado(self, preparador, resultado):
        baixar_pacote = False
        with self._worker_lock:
            if self._preparo_lote_id == preparador.lote_id:
                resultado = resultado if isinstance(resultado, dict) else {}
                self._progresso_preparador(
                    preparador,
                    "falha_preparo" if resultado.get("erro") else "preparo_encerrado",
                    resultado.get("erro") or resultado.get("estado_lote") or resultado.get("estado"),
                )
                self._preparo_thread = None
                self._preparo_lote_id = None
                baixar_pacote = (
                    not resultado.get("erro")
                    and self._pacote_apos_preparo_lote_id == preparador.lote_id
                    and int((resultado.get("preparo") or {}).get("pendentes") or 0) == 0
                )
                if baixar_pacote:
                    self._pacote_apos_preparo_lote_id = None
        if baixar_pacote:
            threading.Thread(
                target=self._pausar_preparo_e_baixar_pacote,
                args=(preparador.lote_id,),
                name="otimizador-selar-pacote-local-v1",
                daemon=True,
            ).start()

    def _pausar_preparo_e_baixar_pacote(self, lote_id):
        """Fecha a preparação antes de exportar; ainda não calcula uma carta."""
        try:
            self._invalidar_cache_status_fila()
            status = self._status_fila()
            if (status.get("tipo_lote") != "integral"
                    or int((status.get("preparo") or {}).get("pendentes") or 0) != 0
                    or int(status.get("processando") or 0) != 0):
                raise ErroDaInterface("a preparação não ficou estável para criar o pacote local", 409)
            estado = status.get("estado_lote") or status.get("estado")
            if estado == "rodando":
                self.gateway.rpc("otimizador_producao_controlar_lote_v3", {
                    "p_lote_id": lote_id, "p_acao": "pausar", "p_confirmado": False,
                })
                self._invalidar_cache_status_fila()
                status = self._status_fila()
                estado = status.get("estado_lote") or status.get("estado")
            if estado == "pausando" and int(status.get("processando") or 0) == 0:
                self.gateway.rpc("otimizador_producao_controlar_lote_v3", {
                    "p_lote_id": lote_id, "p_acao": "confirmar_pausa", "p_confirmado": False,
                })
                self._invalidar_cache_status_fila()
                status = self._status_fila()
                estado = status.get("estado_lote") or status.get("estado")
            if estado != "pausado":
                raise ErroDaInterface("o banco não confirmou a pausa antes da fotografia local", 503)
            self._baixar_pacote_local(lote_id, continuar_apos_download=True)
        except Exception as erro:
            with self._worker_lock:
                self._pacote_estado = self._estado_local(
                    "pacote_pronto_sem_inicio", lote_id, detalhe=str(erro)
                )
    ...
    def _iniciar_worker_producao(self, lote_id, esteira=False):
        with self._worker_lock:
            ...
            if self._worker_ativo():
                ...
                return
            if esteira:
                # A fila integral nunca volta a buscar carta/régua por linha.
                # A leitura V52 só ocorre ao montar a fotografia pausada; daqui
                # em diante o motor recebe o arquivo local, e a rede recebe só
                # reservas mínimas e lotes de resultados já gravados no disco.
                from fila_local_v1 import WorkerFilaLocalV1

                pacote = self._abrir_pacote_local(lote_id, validar=True)
                worker = WorkerFilaLocalV1(
                    self.gateway, pacote, self._worker_encerrado,
                    ao_progresso=self._progresso_worker,
                )
                nome_thread = "otimizador-fila-local-v1"
            else:
                from fila_producao_v3 import WorkerFilaProducaoV3

                worker = WorkerFilaProducaoV3(
                    self.gateway, str(lote_id), self._worker_encerrado, esteira=False,
                    ao_progresso=self._progresso_worker,
                )
                nome_thread = "otimizador-fila-v3"
            thread = threading.Thread(target=worker.executar, name=nome_thread, daemon=True)
    ...
    def _iniciar_preparador_integral(self, lote_id, esteira=False):
        with self._worker_lock:
            ...
            preparador = PreparadorFilaIntegralV5(
                ...
                ao_progresso=self._progresso_preparador,
            )
            thread = threading.Thread(
            ...
            self._preparo_estado = self._estado_local("iniciando_preparo", lote_id)
            thread.start()
    ...
    def _iniciar_esteira_integral(self, lote_id):
        ...
        # V52 não permite que a esteira comece a calcular pela leitura antiga.
        # Primeiro só prepara as snapshots por ID. Quando terminar, o callback
        # pausa o lote, baixa a fotografia local e só então inicia o worker.
        self._pacote_apos_preparo_lote_id = str(lote_id)
    ...
    @staticmethod
    def _inicio_esteira_confirmado(lote_id):
        """Reconhece o clique sem depender da página pesada da fila.

        A atualização completa vem em seguida pela rota de acompanhamento V6.
        Assim, iniciar não fica preso aguardando paginação, eventos ou rótulos.
        """
        return {
            "ok": True,
            "lote_id": str(lote_id),
            ...
            "estado_lote": "rodando",
            "pode_publicar": False,
            "mensagem": "Esteira local iniciada; acompanhando a próxima linha pronta.",
        }
    ...
    def criar_fila(self):
        ...
        return self._inicio_esteira_confirmado(criada["lote_id"])
    ...
    def iniciar_fila(self):
        ...
        estado = status.get("estado_lote") or status.get("estado")
        lote_id = status.get("lote_id")
        # O lote integral é operado exclusivamente pela fotografia V52. Se o
        # arquivo ainda não existir nesta máquina, o próprio clique inicia o
        # download e, ao terminar, continua automaticamente sem exigir um
        # segundo botão e sem ligar o worker antigo.
        if status.get("tipo_lote") == "integral" and estado == "pausado":
            ativo, pacote_estado = self._estado_pacote_local(lote_id)
            if ativo:
                return {
                    "ok": True, "lote_id": str(lote_id), "estado": "preparando",
                    "estado_lote": "preparando", "pode_publicar": False,
                    "mensagem": pacote_estado.get("detalhe") or "Baixando a fotografia local selada.",
                }
            # Só baixa quando a fotografia ainda não existe. Não tratamos uma
            # recusa posterior (selo, estado do lote, reserva ou controle) como
            # se fosse arquivo ausente: isso esconderia o motivo real e poderia
            # iniciar um segundo fluxo indevidamente.
            from fila_local_v1 import PacoteLocalV1
            pasta_pacote = PacoteLocalV1.pasta_do_lote(MOTOR_DIR, str(lote_id))
            if not pasta_pacote.exists():
                self._baixar_pacote_local(lote_id, continuar_apos_download=True)
                return {
                    "ok": True, "lote_id": str(lote_id), "estado": "preparando",
                    "estado_lote": "preparando", "pode_publicar": False,
                    "mensagem": "Baixando a fotografia local selada; o cálculo continuará automaticamente quando ela estiver pronta.",
                }
            return self._retomar_worker_do_pacote(lote_id)
        if self._recuperacao_falha_pre_reserva_v19_disponivel(status):
            recuperado = self.gateway.rpc(
                "otimizador_producao_recuperar_falha_pre_reserva_v2",
                {"p_lote_id": lote_id},
            ) or {}
            if (recuperado.get("contrato") != "otimizador_fila_producao_v6"
                    or recuperado.get("estado_lote") != "rodando"
                    or recuperado.get("pode_publicar") is not False):
                raise ErroDaInterface(
                    "o contrato não confirmou a recuperação segura antes da reserva", 503
            )
            self._invalidar_cache_status_fila()
            return self._retomar_worker_do_pacote(lote_id)
        if self._recuperacao_falha_transporte_disponivel(status):
            recuperado = self.gateway.rpc(
                "otimizador_producao_recuperar_falha_transporte_v1",
                {"p_lote_id": lote_id},
            ) or {}
            if (recuperado.get("contrato") != "otimizador_fila_producao_v6"
                    ...
                    or recuperado.get("pode_publicar") is not False):
                raise ErroDaInterface(
                    "o contrato não confirmou a recuperação segura do lote", 503
            )
            self._invalidar_cache_status_fila()
            return self._retomar_worker_do_pacote(lote_id)
        if estado in {"preparando", "preparo_pausado"}:
            self._iniciar_esteira_integral(status.get("lote_id"))
            return self._inicio_esteira_confirmado(status.get("lote_id"))
        if estado == "rodando" and status.get("tipo_lote") == "integral":
            preparo_pendente = int((status.get("preparo") or {}).get("pendentes") or 0)
            if preparo_pendente == 0:
                # Uma abertura posterior a uma preparação já concluída não
                # pode usar o consumidor V3. Pausamos sem linha ativa, baixamos
                # a fotografia e retomamos automaticamente pelo worker local.
                if int(status.get("processando") or 0) != 0:
                    raise ErroDaInterface("há reserva ativa; aguarde a recuperação segura antes de trocar de computador", 409)
                self.gateway.rpc("otimizador_producao_controlar_lote_v3", {
                    "p_lote_id": lote_id, "p_acao": "pausar", "p_confirmado": False,
                })
                self._invalidar_cache_status_fila()
                ...
                return {
                    "ok": True, "lote_id": str(lote_id), "estado": "preparando",
                    "estado_lote": "preparando", "pode_publicar": False,
                    "mensagem": "Preparação concluída; baixando a fotografia local antes de retomar o cálculo.",
                }
        ...
        preparo_pendente = int((status.get("preparo") or {}).get("pendentes") or 0)
        if status.get("tipo_lote") == "integral":
            # Mesmo depois de a preparação terminar, a fila integral continua
            # obrigada a reservar pela fotografia V7. Usar o worker V3 aqui
            # criaria uma segunda rota de entrada e foi a causa da falha V20.
            if preparo_pendente > 0:
                self._pacote_apos_preparo_lote_id = str(lote_id)
                self._iniciar_preparador_integral(lote_id, esteira=True)
            else:
                # O lote foi retomado já com snapshots prontos. Pausar antes
                # do download impede que algum processo use a fonte antiga.
                self.gateway.rpc("otimizador_producao_controlar_lote_v3", {
                    "p_lote_id": lote_id, "p_acao": "pausar", "p_confirmado": False,
                })
                self._invalidar_cache_status_fila()
    ...
    def pausar_fila(self):
        # Durante o download não há reserva nem cálculo. Pausar nesse ponto só
        # cancela o arquivo temporário; o lote já está pausado no banco.
        trava = getattr(self, "_worker_lock", None)
        if trava is None:
            cancelador = None
        else:
            with trava:
                cancelador = self._pacote_cancelar if self._pacote_local_ativo() else None
        if cancelador is not None:
            ...
            return {
                ...
                "estado_lote": "pausado",
                "pode_publicar": False,
                "mensagem": "Download local cancelado com segurança; nenhuma linha foi iniciada.",
            }
        status = self._status_fila()
    ...
    def recuperar_reserva_orfa(self, confirmado):
        """Devolve a única reserva comprovadamente órfã ao lote pausado.

        V9 cobre pausa/encerramento solicitado. V20 cobre só a retomada
        integral pós-preparo que reservou uma linha pelo contrato V3 e falhou
        ...
        if self._recuperacao_falha_retomada_integral_v20_disponivel(status):
            resposta = self.gateway.rpc("otimizador_producao_recuperar_falha_retomada_integral_v1", {
                "p_lote_id": status.get("lote_id"),
                "p_linha_id": int(corrente["linha_id"]),
                "p_confirmado": True,
            }) or {}
        elif self._recuperacao_reserva_orfa_disponivel(status):
    ...
    def parar_fila(self, confirmado):
        ...
        if cancelador is not None:
            ...
            return {
                ...
                "estado_lote": "pausado",
                "pode_publicar": False,
                "mensagem": "Download local cancelado; como nenhuma linha começou, não há resultado a encerrar.",
            }
        status = self._status_fila()
    ...
    def saude(self):
        ...
        if trava is None:
            ...
            pacote = self._copia_estado_local(getattr(self, "_pacote_estado", {}))
            ...
            with trava:
                ...
                pacote_ativo = self._pacote_local_ativo()
                worker = self._copia_estado_local(getattr(self, "_worker_estado", {}))
                preparador = self._copia_estado_local(getattr(self, "_preparo_estado", {}))
                pacote = self._copia_estado_local(getattr(self, "_pacote_estado", {}))
                iniciado_em = getattr(self, "_servidor_iniciado_em", None)
        ...
        if worker_ativo:
            ...
            resumo = pacote.get("detalhe") or "Baixando a fotografia local"
        else:
        ...
        return {
            ...
            "pode_rodar": None,
            "modo": "consulta_local_v3; pacote_local_v2; esteira_integral_sem_publicacao",
            ...
            "pacote_local_ativo": pacote_ativo,
            "pacote_local": pacote,
        }
        ...
        def do_GET(self):
            ...
            try:
                ...
                if caminho.path == "/api/fila/status":
                    # Estado acionável primeiro: esta rota não espera a página,
                    # nomes de carta nem catálogos. Isso mantém Pausar/Parar e a
                    # indicação de worker responsivos durante uma lista grande.
                    return self.responder_json(200, servico.status_fila_rapido())
                if caminho.path == "/api/fila/linhas":
                    # Detalhes visuais são carregados depois do estado rápido.
                    # O contrato continua o mesmo e a leitura é só por IDs.
