...
* o pacote é baixado uma vez, enquanto o lote está pausado, por RPCs seladas;
...
class PacoteLocalV1:
    ...
    def estado_path(self) -> Path:
        return self.pasta / "estado-local.json"

    @property
    def pendentes_path(self) -> Path:
        return self.pasta / "spool" / "pendentes"

    @property
    def enviando_path(self) -> Path:
        return self.pasta / "spool" / "enviando"

    @property
    def reservas_path(self) -> Path:
        return self.pasta / "spool" / "reservas"

    @classmethod
    def pasta_do_lote(cls, raiz_operacional: Path, lote_id: str) -> Path:
        return Path(raiz_operacional) / "runtime" / "fila-local" / str(lote_id)
    ...
    def _validar_manifesto(self) -> None:
        ...
        if int(m.get("versao_pacote") or 0) != VERSAO_PACOTE_LOCAL_V2:
    ...
    def validar_integridade(self) -> None:
        ...
        for chave in ("cartas", "linhas"):
            ...
            caminho = self.pasta / str(meta.get("nome") or "")
            if not caminho.is_file() or _hash_bytes(caminho) != meta.get("sha256"):
                raise FalhaPacoteLocal(f"integridade inválida em {chave}.jsonl")
            with caminho.open("r", encoding="utf-8") as arquivo:
                total = sum(1 for _ in arquivo)
            if total != int(meta.get("total") or -1):
                raise FalhaPacoteLocal(f"contagem inválida em {chave}.jsonl")
    ...
    def cartas(self) -> dict[str, dict]:
        ...
        caminho = self.pasta / "cartas.jsonl"
        with caminho.open("r", encoding="utf-8") as arquivo:
    ...
    def iter_linhas(self) -> Iterable[dict]:
        caminho = self.pasta / "linhas.jsonl"
        with caminho.open("r", encoding="utf-8") as arquivo:
            for numero, texto in enumerate(arquivo, 1):
                try:
                    item = json.loads(texto)
    ...
    def arquivos_pendentes(self) -> list[Path]:
        ...
        for arquivo in sorted(self.enviando_path.glob("*.json")):
            destino = self.pendentes_path / arquivo.name
            if destino.exists():
                raise FalhaPacoteLocal("duas cópias locais do mesmo resultado aguardam envio")
            os.replace(arquivo, destino)
        return sorted(self.pendentes_path.glob("*.json"))
    ...
    def gravar_resultado(self, reserva: dict, resultado: dict) -> Path:
        linha_id = int(reserva["linha_id"])
        conteudo = {"contrato": CONTRATO_PACOTE_LOCAL_V1, "reserva": reserva, "resultado": resultado}
        destino = self.pendentes_path / f"{int(reserva['ordem_fila']):09d}-{linha_id}.json"
...
class EnviadorLotesLocalV1:
    ...
    def _mover_para_envio(self, arquivos: list[Path]) -> list[Path]:
        ...
        for arquivo in arquivos:
            destino = self.pacote.enviando_path / arquivo.name
            os.replace(arquivo, destino)
        ...
        with self._trava:
            while True:
                ...
                if not arquivos or (not forcar and len(arquivos) < TAMANHO_LOTE_ENVIO):
                    self.pacote.atualizar_estado(pendentes_envio=len(arquivos))
                    return enviados
                ...
                try:
                    ...
                    for arquivo in selecionados:
                        destino = self.pacote.pendentes_path / arquivo.name
...
class WorkerFilaLocalV1:
    ...
    def executar(self) -> dict:
        ...
        try:
            ...
            for linha in self.pacote.iter_linhas():
                ...
                if reserva.get("reservada") is not True:
                    # Outro computador pode ter concluído a mesma linha depois
                    # da fotografia local. Não recalculamos nem tratamos como
                    # erro: a exclusividade é do banco.
                ...
                self.pacote.gravar_resultado(reserva, resultado)
                self._reserva_aberta = False
                atual = self.pacote.estado()
                self.pacote.atualizar_estado(
                    estado="calculando",
                ...
                if (self.limite_linhas is not None
                        and self._linhas_calculadas_nesta_execucao >= self.limite_linhas):
                    # Piloto explícito: pede pausa depois de a linha atual,
                    # força a persistência desta saída e confirma a pausa antes
                    # de o laço poder tentar reservar outra linha.
                    ...
                    final = self._confirmar_pausa_ou_encerramento(pausa)
                    break
