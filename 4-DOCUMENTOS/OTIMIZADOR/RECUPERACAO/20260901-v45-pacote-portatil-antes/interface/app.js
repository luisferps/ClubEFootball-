...
function pintarFila(d){
  ...
  if(!d.disponivel){linhasFilaCarregando=false;linhasFilaAtuais=[];linhaFocoId=null;$('#ir-linha-atual').disabled=true;renderizarLinhasFila();$('#eventos-fila').textContent='Fila indisponível: '+texto(d.origem);return;}
  linhasFilaCarregando=d.linhas_carregando===true;
...
async function carregarApoiosFila(forcar=false){
  if(carregandoApoiosFila||(!forcar&&Date.now()<proximaLeituraApoiosFila))return;
  carregandoApoiosFila=true;
  try{
    const pedidos=[api('/api/fila/eventos?offset=0&limite=100')];
    if(abaAtiva('resultados'))pedidos.push(api('/api/resultados?offset='+(paginaResultados*TAMANHO_PAGINA)+'&limite='+TAMANHO_PAGINA));
    const respostas=await Promise.allSettled(pedidos),eventos=respostas[0],resultados=respostas[1];
    if(eventos&&eventos.status==='fulfilled')pintarEventos((eventos.value||{}).itens||[]);
    else $('#eventos-fila').textContent='Não foi possível atualizar os eventos: '+texto(eventos&&eventos.reason&&eventos.reason.message)+'.';
    if(resultados){
      if(resultados.status==='fulfilled')pintarResultados(resultados.value);
      else $('#resultados-aviso').textContent='Não foi possível atualizar Resultados: '+texto(resultados.reason&&resultados.reason.message)+'. A lista será tentada novamente.';
    }
  }catch(e){
    $('#eventos-fila').textContent='Não foi possível atualizar os eventos: '+texto(e&&e.message)+'.';
    if(abaAtiva('resultados'))$('#resultados-aviso').textContent='Não foi possível atualizar Resultados: '+texto(e&&e.message)+'. A lista será tentada novamente.';
  }finally{
    carregandoApoiosFila=false;
    // Eventos e Resultados não podem disputar o contrato a cada refresh de
    // três segundos. O estado acionável continua imediato; os apoios são
    // atualizados a cada 15 s ou ao abrir explicitamente Resultados.
...
async function carregarDetalhesFila(){
  ...
  try{
    ...
    if(d.conexao_banco===false)return;
    ...
    if(linhasFilaAtuais.length===0){
      $('#resumo-linhas').textContent=mensagem;
      linhaVazia('#linhas-fila',mensagem,10);
    }
...
async function atualizarFila(){
  ...
  try{
    ...
    pintarFila({...s,linhas_carregando:s.disponivel===true});
...
async function carregar(){await atualizarSaudeLocal();atualizarFila();if(temporizadorSaude===null)temporizadorSaude=window.setInterval(atualizarSaudeLocal,2000);}
