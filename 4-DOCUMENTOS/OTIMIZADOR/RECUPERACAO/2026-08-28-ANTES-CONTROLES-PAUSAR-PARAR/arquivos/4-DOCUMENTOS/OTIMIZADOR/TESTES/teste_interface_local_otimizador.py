...
class ServicoFalso:
    ...
    def painel_fila(self):
        return {
            ...
            "acoes": {"criar": False, "iniciar": True, "parar": False, "retomar": True, "console": True},
            "totais": {"cartas_selecionadas": 100, "linhas_geradas": 896, "pendentes": 895,
                       "em_processamento": 0, "concluidas": 1, "bloqueadas": 0, "falhas": 0},
            "linha_atual": None,
...
class InterfaceOtimizadorTest(unittest.TestCase):
    ...
    def test_fila_real_e_controles_somente_pelo_selo(self):
        ...
        self.assertFalse(resposta["acoes"]["parar"])
        ...
        for rota in ("/api/fila/iniciar", "/api/fila/parar", "/api/fila/retomar", "/api/fila/console"):
            status, corpo = self.requisicao("POST", rota)
            self.assertEqual(status, 200)
            self.assertTrue(json.loads(corpo)["ok"])
        status, corpo = self.requisicao("POST", "/api/fila/criar")
    ...
    def test_fronteira_tem_apenas_contratos_v1_de_leitura(self):
        self.assertEqual(mod.RPC_PERMITIDAS, {
            ...
            "otimizador_controlar_lote_teste_v1",
        })
