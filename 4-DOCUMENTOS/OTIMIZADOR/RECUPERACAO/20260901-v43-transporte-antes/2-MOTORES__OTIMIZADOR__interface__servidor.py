...
class GatewayOtimizador:
    ...
    def _rpc_privado(self, nome: str, corpo: dict | None = None):
        """Chama somente a ponte V17 por conexão local protegida.

        A UI nunca recebe essa URL ou credencial. A função V17 no banco é uma
        allowlist fechada das mesmas RPCs canônicas expostas pelo gateway; ela
        não aceita SQL, nomes de tabela nem argumentos fora do contrato.
        """
        if not self.banco:
        ...
        try:
            with self._banco_lock:
                ...
                with self._conexao_privada.cursor() as cursor:
                    cursor.execute(
                        "select public.otimizador_portal_local_v4(%s, %s::jsonb)",
    ...
    def rpc(
        ...
        if self.banco:
            try:
                ...
                self.canal_ativo = "contrato privado local V17"
                return resposta
...
class ServicoOtimizador:
    ...
    def _status_fila(self):
        """Lê só o contrato V6; ausência da migração é honesta e fechada."""
