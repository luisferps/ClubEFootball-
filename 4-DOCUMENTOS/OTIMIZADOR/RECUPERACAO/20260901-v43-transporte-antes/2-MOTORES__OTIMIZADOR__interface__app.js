...
async function carregarApoiosFila(forcar=false){
  ...
  try{
    ...
    pintarEventos((respostas[0]||{}).itens||[]);
    if(respostas[1])pintarResultados(respostas[1]);
  }catch(_){
    // Eventos e resultados são apoio visual. O estado acionável da esteira já
    // foi confirmado pela rota rápida e não deve ser substituído por erro de
    // uma leitura secundária.
  }finally{
...
async function carregarDetalhesFila(){
  ...
  try{
    ...
    // O usuário continua vendo o estado real e os controles. A próxima
    // atualização tenta carregar esta página novamente, sem simular linhas.
    if(linhasFilaAtuais.length===0){
      linhasFilaCarregando=false;
      $('#resumo-linhas').textContent='A lista está demorando; o estado da esteira continua atualizado acima.';
      linhaVazia('#linhas-fila','Aguardando a resposta da lista. Nenhuma linha foi alterada.',10);
    }
