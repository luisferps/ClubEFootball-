...
class InterfaceOtimizadorTest(unittest.TestCase):
    ...
    def test_fila_vazia_e_portas_preparadas_nao_fabricam_execucao(self):
        for rota in ("/api/fila/status", "/api/fila/eventos", "/api/resultados"):
            ...
            self.assertEqual(resposta["execucao"]["estado"], "aguardando_contrato_de_integracao")
            self.assertEqual(resposta["totais"]["linhas_geradas"], 0)
            self.assertEqual(resposta["itens"], [])
        for rota in ("/api/fila/criar", "/api/fila/iniciar", "/api/fila/parar", "/api/fila/retomar", "/api/fila/console"):
