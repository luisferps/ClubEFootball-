...
FILA_AGUARDANDO_CONTRATO = {
    "ok": True,
    "disponivel": False,
    "estado": "aguardando_contrato_de_integracao",
    "execucao": {
        "estado": "aguardando_contrato_de_integracao",
        "pausa_segura": "indisponível enquanto não houver worker e selo de lote reais",
    },
    "mensagem": "A fila de teste ainda não foi disponibilizada pela Integração dos motores.",
    "totais": {"cartas_selecionadas": 0, "linhas_geradas": 0, "pendentes": 0,
               "em_processamento": 0, "concluidas": 0, "bloqueadas": 0, "falhas": 0},
    "linha_atual": None,
    "itens": [],
    "origem": "nenhuma: contrato de fila não aplicado; nenhuma linha foi inventada",
...
def criar_servidor(servico=None, porta=8767):
    ...
    class Manipulador(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if caminho.path in {"/api/fila/criar", "/api/fila/iniciar", "/api/fila/parar", "/api/fila/retomar", "/api/fila/console"}:
                return self.responder_json(HTTPStatus.CONFLICT, {
                    "ok": False, "disponivel": False,
                    "erro": "Ação de fila recusada: o contrato de teste ainda não foi aplicado.",
                    "estado": FILA_AGUARDANDO_CONTRATO["estado"],
                })
        ...
        def do_GET(self):
            ...
            try:
                ...
                if caminho.path in {"/api/fila/status", "/api/fila/eventos", "/api/resultados"}:
                    dados = dict(FILA_AGUARDANDO_CONTRATO)
                    dados["tipo"] = "eventos" if caminho.path.endswith("eventos") else "linhas"
