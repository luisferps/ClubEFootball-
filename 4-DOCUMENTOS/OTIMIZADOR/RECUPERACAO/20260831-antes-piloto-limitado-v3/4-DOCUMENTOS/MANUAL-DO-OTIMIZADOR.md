...
O único passo operacional que resta é uma autorização explícita para aplicar a
migração V3 no Supabase e fazer o readback pós-DDL. Só então se cria a fila e se
faz um piloto controlado; não se inicia lote nem Bonificador por inferência.

## 15. Fila produtiva V3 aplicada, sem lote (31/08/2026)
...
O próximo passo exige autorização operacional separada para **Criar e iniciar** a
fila completa. Esse clique cria todas as linhas aptas e inicia o worker; não foi
executado nesta aplicação. Ímpetos condicionais, Bonificador e publicação continuam
desligados/independentes.
