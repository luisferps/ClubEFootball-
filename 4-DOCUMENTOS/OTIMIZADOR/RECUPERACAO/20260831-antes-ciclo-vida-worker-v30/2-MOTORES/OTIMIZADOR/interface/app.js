...
async function carregar(){try{const x=await Promise.all([api('/api/saude'),api('/api/catalogos')]),s=x[0],c=x[1],painelConectado=s.ok===true;$('#estado').textContent=painelConectado?'Painel conectado':'Motor indisponível';$('#estado').title=(s.contrato||'')+(s.versao_interface?' · interface '+s.versao_interface:'');$('#estado').className='selo '+(painelConectado?'ok':'erro');$('#funcao').innerHTML=c.funcoes.map(x=>'<option value="'+x.funcao_id+'">'+seguro(x.nome)+' (#'+x.funcao_id+')</option>').join('');$('#tecnico').innerHTML=c.tecnicos.map(x=>'<option value="'+x.tecnico_id+'">'+seguro(x.nome)+' (#'+x.tecnico_id+' · '+seguro(x.proficiencia)+')</option>').join('');await atualizarFila();}catch(e){$('#estado').textContent='Motor indisponível';$('#estado').className='selo erro';$('#fila-aviso').textContent=e.message;}}
