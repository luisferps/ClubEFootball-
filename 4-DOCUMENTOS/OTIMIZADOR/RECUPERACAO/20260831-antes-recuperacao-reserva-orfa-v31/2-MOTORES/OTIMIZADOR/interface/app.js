...
function aplicarAcoes(acoes,confirmacao,estado){const a=acoes||{},c=confirmacao||{};const iniciar=$('#iniciar-fila');iniciar.disabled=!(a.iniciar===true||a.retomar===true||a.criar===true||a.preparar===true);iniciar.textContent=a.iniciar===true?'Iniciar esteira':estado==='preparo_pausado'?'Retomar esteira':a.retomar===true?'Retomar':a.criar===true?'Iniciar fila integral':a.preparar===true?'Ligando esteira…':'Iniciar';$('#pausar-fila').disabled=a.pausar!==true;$('#pausar-fila').textContent=estado==='preparando'?'Pausar preparação':'Pausar';$('#parar-fila').disabled=!(a.parar===true&&c.parar_exige_confirmacao===true);}
...
async function atualizarFila(){if(atualizandoFila)return;atualizandoFila=true;const botao=$('#atualizar-fila'),textoOriginal=botao.textContent;botao.disabled=true;botao.textContent='Atualizando…';try{await atualizarSaudeLocal();const filaUrl='/api/fila/status?offset='+(paginaFila*TAMANHO_PAGINA)+'&limite='+TAMANHO_PAGINA;const resultadosUrl='/api/resultados?offset='+(paginaResultados*TAMANHO_PAGINA)+'&limite='+TAMANHO_PAGINA;const s=await api(filaUrl);pintarFila(s);const [e,r]=await Promise.all([api('/api/fila/eventos?offset=0&limite=100'),api(resultadosUrl)]);pintarResultados(r);pintarEventos(e.itens||[]);}catch(e){const local=resumoSaudeLocal(estadoLocal);$('#fila-aviso').textContent=e.message+(estadoLocal?' — '+local:'');if(estadoLocal&&estadoLocal.worker_ativo===true)ultimoEstadoFila='rodando';}finally{atualizandoFila=false;botao.disabled=false;botao.textContent=textoOriginal;agendarAtualizacaoFila(ultimoEstadoFila);}}
async function acaoFila(acao,corpo){const botao=acao==='iniciar'?$('#iniciar-fila'):acao==='pausar'?$('#pausar-fila'):acao==='parar'?$('#parar-fila'):null;const original=botao&&botao.textContent;if(botao){botao.disabled=true;botao.textContent=acao==='iniciar'?'Ligando esteira…':acao==='pausar'?'Pausando…':'Parando…';}$('#fila-aviso').textContent='Enviando comando de '+acao+' à esteira integral V6…';try{const r=await fetch('/api/fila/'+acao,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify(corpo||{})}),d=await r.json();if(!r.ok)throw new Error(d.erro||'Ação de fila indisponível');await atualizarFila();}catch(e){$('#fila-aviso').textContent=e.message;}finally{if(botao){botao.disabled=false;botao.textContent=original;}}}
...
async function carregar(){await atualizarSaudeLocal();try{const c=await api('/api/catalogos');$('#funcao').innerHTML=c.funcoes.map(x=>'<option value="'+x.funcao_id+'">'+seguro(x.nome)+' (#'+x.funcao_id+')</option>').join('');$('#tecnico').innerHTML=c.tecnicos.map(x=>'<option value="'+x.tecnico_id+'">'+seguro(x.nome)+' (#'+x.tecnico_id+' · '+seguro(x.proficiencia)+')</option>').join('');}catch(e){$('#fila-aviso').textContent=e.message;}finally{await atualizarFila();if(temporizadorSaude===null)temporizadorSaude=window.setInterval(atualizarSaudeLocal,2000);}}
