...
class GatewayOtimizador:
    ...
    def _rpc_privado(self, nome: str, corpo: dict | None = None):
        """Chama somente a ponte V13 por conexão local protegida.

        A UI nunca recebe essa URL ou credencial. A função V16 no banco é uma
        ...
        try:
            with self._banco_lock:
                ...
                with self._conexao_privada.cursor() as cursor:
                    cursor.execute(
                        "select public.otimizador_portal_local_v3(%s, %s::jsonb)",
    ...
    def rpc(
        ...
        # pool público do PostgREST. Ela ainda chama somente contratos V16/V6
        # por ID e falha fechada se o banco não confirmar a leitura.
        erro_privado = None
        if self.banco:
            try:
                resposta = self._rpc_privado(nome, corpo)
                self.canal_ativo = "contrato privado local V16"
                return resposta
