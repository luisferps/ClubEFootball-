# -*- coding: utf-8 -*-
"""Auditoria somente leitura: contrato antigo versus contrato v1 do Otimizador.
...
FONTE_NOVA = 'public.otimizador_cartas_v1 -> clube_novo normalizado por IDs'
...
def compara(antigas, novas):
    ...
    return {
        'contrato_auditado': 'otimizador_entradas_v1', 'somente_leitura': True,
...
def compara_em_lotes(url, chave, ids, tamanho=200):
    ...
    for inicio in range(0, len(ids), tamanho):
        ...
        novas = rpc(url, chave, 'otimizador_cartas_v1', {'p_ids': pedaco}) or []
...
def main(argv=None):
    ...
    if args.todos:
        ...
        fila = rpc(url, chave, 'otimizador_proxima_fila_v1', {'p_limite': 1000000}) or []
        ids = sorted({str(x['card_id']).split('@')[0] for x in fila})
        rel = compara_em_lotes(url, chave, ids, max(1, args.lote))
        rel['universo'] = 'cards distintos referenciados pela fila canônica'
        rel['linhas_fila'] = len(fila)
    else:
        ids = [str(x).split('@')[0] for x in args.ids]
        antigas = rpc(url, chave, 'cartas_do_motor', {'p_ids': ids}) or []
        novas = rpc(url, chave, 'otimizador_cartas_v1', {'p_ids': ids}) or []
