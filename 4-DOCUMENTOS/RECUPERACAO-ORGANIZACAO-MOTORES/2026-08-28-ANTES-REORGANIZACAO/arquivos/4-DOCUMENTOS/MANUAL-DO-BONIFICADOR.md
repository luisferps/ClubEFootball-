...
|---|---|
| executável | `2-MOTORES/motor_bonus.py` |
| launcher | nenhum `.bat` ou chamador encontrado; execução era manual |
...
Como a UI não muda, não há amostra visual a homologar nesta etapa. A validação de tela
é explicitamente `N/A`; o consumidor `casa_tela` permanece intacto.
