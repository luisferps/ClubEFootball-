...
// ===========================================================================
  /* ⛔ 25/08 — A CARGA DA LISTA LE A VIEW ENXUTA.
     Medido no JSON que trafega: das 44 MB da `linha`, `arows` e 26,5% e
     `falta` e 20,6% — e nenhum dos dois e lido pela lista. `arows` so
     serve a ficha/otimizador; `falta` so aparece num <details admonly>.
     A `casa_lista` devolve a mesma linha sem esses dois. Medido na rede,
     1.000 linhas: 2.765 KB / 2.972 ms -> 1.718 KB / 1.160 ms.
     Os dois voltam sob demanda, por card, em arows-sob-demanda.js. */
  var URL  = 'https://trqqpsnafpbudtvvicch.supabase.co/rest/v1/casa_lista';
  ...
  /* A carga deve usar exatamente as funções que o painel de Ranking oferece. */
  var FUNCOES = ["Centroavante fixo", "Centroavante móvel", "Falso nove", "Goleiro defensivo", "Goleiro ofensivo", "Lateral defensivo", "Lateral ofensivo", "Meia armador", "Meia de arranque", "Ala finalizador", "Ala cruzador", "Meia ofensivo", "Atacante infiltrador", "Atacante criador", "Atacante finalizador", "Volante de construção", "Volante de contenção", "Zagueiro de combate", "Zagueiro de saída"];
  ...
  function endereco(de, limite){
    limite=limite||PAGINA;
    return URL + '?select=linha' + FILTRO + '&order=card_id.asc,funcao.asc&limit=' + limite + '&offset=' + de;
  }
  ...
  function ehTimeout57014(x){
    if(!x||x.status!==500)return false;
    try{return String(JSON.parse(x.responseText||'{}').code||'')==='57014';}catch(e){}
    return /(?:^|[^0-9])57014(?:[^0-9]|$)/.test(String(x.responseText||''));
  }
  function erroHTTP(x, contexto){
    return new Error('HTTP '+(x&&x.status)+' - '+String(x&&x.responseText||'').slice(0,200)
      +(contexto?' ('+contexto+')':''));
  }
  function paginaInicial(de){
    var x=pedeSync(endereco(de,PAGINA));
    if(x.status===200)return JSON.parse(x.responseText);
    if(!ehTimeout57014(x))throw erroHTTP(x);
    /* O papel anon tem statement_timeout curto. Sob carga, a pagina de 1.000
       linhas pode ultrapassa-lo mesmo usando o indice correto. Repetimos a
       MESMA faixa uma unica vez em duas metades: nao descartamos dados, nao
       mudamos a ordem e qualquer falha da tentativa dividida continua fatal. */
    console.warn('[encaixe] timeout 57014 na pagina '+de+'; repetindo a mesma faixa em duas metades');
    var metade=Math.ceil(PAGINA/2), juntas=[];
    for(var sub=0;sub<PAGINA;sub+=metade){
      var limite=Math.min(metade,PAGINA-sub), parte=pedeSync(endereco(de+sub,limite));
      if(parte.status!==200)throw erroHTTP(parte,'tentativa dividida apos 57014');
      var linhas=JSON.parse(parte.responseText);
      for(var pi=0;pi<linhas.length;pi++)juntas.push(linhas[pi]);
    }
    return juntas;
  }
...
const FROWIDX=(()=>{const o={};for(const k in FROWNOME_N2I)o[FROWNOME_N2I[k]]=k;
 ...
 return o;})();
/* injeta as medidas que faltavam e marca o pacote */
Object.assign(FIS_M, CORPO97);
const FIS_F=(()=>{const R={};for(const f in FIS_P){const L=FIS_P[f];
 ...
 return R;})();
function fisVals(c){const m=FIS_M[String(c.id).split('@')[0]];if(!m||typeof c.h!=='number')return null;return [c.h,c.w].concat(m);}
...
function fisEspelho(c){const L=FIS_F[c.tipo],v=fisVals(c);if(!L||!v)return null;
 ...
 for(const p of L){const x=v[p[0]],a=p[1],maior=p[2];
  ...
  if(f>0)s+=p[3]*f;}
 return s;}
/* reconstroi o Bloco 4 das linhas que entraram sem corpo, com o molde de hoje */
window._pos_D=function(){ let n=0,marc=0;
 for(const c of D){ if(c.id==='MOLDE')continue;
  const base=String(c.id).split('@')[0];
  if(PACOTE[base]){c.pacote=PACOTE[base];c.NOVO=1;marc++;}
  if(c.frows&&c.frows.length)continue;
  const v=fisVals(c),L=FIS_F[c.tipo];
  if(!v||!L)continue;
  c.frows=L.map(p=>{const i=p[0],alvo=p[1],maior=p[2],peso=p[3],x=v[i];
   const passa = maior ? x>=alvo : x<=alvo;
   return [FROWIDX[i]||('medida '+i), Math.round(peso*10)/10, alvo, x, passa?1:0, maior?1:-1,
           Math.round((passa?peso:0)*100)/100];});
  c._fb=undefined; delete c._n; n++;}
 /* quem tem orcamento evolui: a marca temMax vinha errada nos cards dos pacotes */
 for(const c of D){ if(c.id!=='MOLDE'&&c.pacote&&c.orc>0&&!c.temMax) c.temMax=true; }
 if(n)console.log('%cBloco 4 reconstruido em '+n+' linhas · '+marc+' linhas marcadas com pacote','color:#22c58b');
};window._pos_D();
...
var _TOPO={},_TOPOM=null;
function pctTopo(c,n){var t=topoDoTipo(c.tipo);if(!(t>0))return '';
 if(n===undefined)n=nota(c);var p=100*n/t;
 var cr=p>=99.5?'#8fd694':(p>=90?'#f0a531':'var(--txt3)');
 return '<span class=pcttopo style="color:'+cr+'" title="percentual da nota do 1o colocado desta funcao">'+p.toFixed(2)+'%</span>';}
function topoDoTipo(t){
 var m=(typeof CMODE!=='undefined'?CMODE:0)+'|'+(typeof PISO_ON!=='undefined'?PISO_ON:0)+'|'+(typeof W!=='undefined'?W.join(','):'');
 if(_TOPOM!==m){_TOPO={};_TOPOM=m;}
 if(_TOPO[t]!==undefined)return _TOPO[t];
 var mx=0;
 for(var i=0;i<D.length;i++){var c=D[i];if(!c||c.id==='MOLDE'||c.tipo!==t)continue;
  var n=c._n===undefined?(c._n=nota(c)):c._n;if(n>mx)mx=n;}
 return _TOPO[t]=mx;
}
function lista(){
 ...
 if(window._rkChaveCalculo!==chaveCalculo){window._rkChaveCalculo=chaveCalculo;_TOPO={};_TOPOM=null;D.forEach(c=>{if(c&&c.id!=='MOLDE')delete c._n;});}
 ...
 L.forEach(c=>{
  if(isGeral(S.tipo)&&typeof window.t6NotaDoMotor==='function'){
 ...
 L.forEach(c=>{const _t=topoDoTipo(c.tipo);c._pct=_t>0?100*c._n/_t:0;});
...
function render(){
 ...
 if(S.view==='cards'){
  ...
  out.innerHTML='<div class=grade>'+moldeCard+L.slice(0,VIS).map((c,i)=>{
   ...
   return `<div class="cd${cls(c)}" data-k="${c.id}|${c.tipo}" data-rknome="${nz(c.nome)}"><div class=rk>${i+1}º</div>   <img src="https://efimg.com/efootballhub22/images/player_cards/${String(c.id).split('@')[0]}_l.png" loading=lazy onerror="this.outerHTML='<div style=&quot;width:66px;height:88px;margin:4px auto;display:flex;align-items:center;justify-content:center;font-size:26px;background:#151d29;border-radius:8px;color:#3a4350&quot;>👤</div>'">
...
function abrir(key){
 ...
 <div class=fhd>
 <div class=fhdcol><div class=fhdnome><div style="font-size:21px;font-weight:800">${c.nome}</div></div><img class=fhdimg src="https://efimg.com/efootballhub22/images/player_cards/${String(c.id).split('@')[0]}_l.png" onerror="this.style.display='none'">
 ...
 </div><div class=fhdmeio><div class=fhdestbox>${(c.modelo||c.tipo||'')}${estiloAtiva(c)?'':'<div class=fhdbasico title="o estilo de jogo dele não liga nesta posição">BÁSICO — este estilo não liga nesta posição</div>'}</div><div class=fhdbts><div class=fhdbtstt>FUNÇÕES QUE ELE PODE EXERCER EM CAMPO<span>clique para ver o build</span></div>${cbFuncoes(c)}</div></div><div class=fhdcampo>${cbCampo(c)}</div><div class=fhdnota><span class=fhdn style="color:${cor(nota(c),ref)}">${_nd(nota(c))}</span><span class=fhdl>pontuação final</span><span class=fhdtopo>${(function(){var t=topoDoTipo(c.tipo);var n=nota(c);var p=t>0?100*n/t:0;return '<b style="color:'+(p>=99.5?'#22c58b':(p>=90?'#c98a1f':'inherit'))+'">'+p.toFixed(2)+'% top</b>';})()}</span>${(function(){if(c._notaMot===undefined)return '';const na=nota(c);const p=na>0?(c._notaMot-na)/na*100:0;return '<span class=fhdmel>pode melhorar <b>'+(p>0.05?'+'+p.toFixed(1):'0')+'%</b></span>';})()}</div></div>
 <div class=duo>
...
function mtRender(){ _MTMAX=null;
 ...
 const campo=S1.map((sl,i)=>{
  ...
  return `<div class="mtsl ${cl}" data-i="${i}" data-key="${sl.key||''}" draggable="${sl.key?'true':'false'}" style="left:${sl.x}%;top:${sl.y}%" onclick="if(!MT_ED)mtAbreSel(${i})">
   ...
   ${c?`<img src="https://efimg.com/efootballhub22/images/player_cards/${String(c.id).split("@")[0]}_l.png" onerror="this.style.visibility='hidden'">
 ...
 const banco=MT.banco.map((k,i)=>{const c=mtCard(k);if(!c)return"";
  return `<div class=mtbc draggable=true data-key="${k}" data-de="banco" data-i="${i}" onclick="abrir('${k}')"><img src="https://efimg.com/efootballhub22/images/player_cards/${String(c.id).split("@")[0]}_l.png" onerror="this.style.visibility='hidden'">
 ...
 const elenco=_foraL.map(({k,i,c})=>{
  return `<div class=mtbc draggable=true data-key="${k}" data-de="elenco" data-i="${i}" onclick="abrir('${k}')"><img src="https://efimg.com/efootballhub22/images/player_cards/${String(c.id).split("@")[0]}_l.png" onerror="this.style.visibility='hidden'">
   <div style=flex:1><b>${c.nome}</b><div class=mini>${c.tipo}</div></div><b title="nota da carta otimizada (a mesma do ranking)" style="color:${cor(mtMaiorNota(c),0)}">${mtMaiorNota(c).toFixed(1)}</b>
   <button class=bbt title="ajustar barras" onclick="event.stopPropagation();mtAbreCfg('${k}')">⚙</button>
 ...
 if(nq&&!L.length&&!livre){
  ...
  noutra=o.length?`<div class=mtnoutra><b>Não joga de ${fsel}.</b> Mas está em:</div>`+
   o.map(x=>`<div class=mtli onclick="mtVaiPara('${x.c.tipo}','${(q||'').replace(/'/g,"")}')">
     <img src="https://efimg.com/efootballhub22/images/player_cards/${String(x.c.id).split("@")[0]}_l.png" onerror="this.style.visibility='hidden'">
...
const CONT={"linhas": 17463, "linhas_total": 18598, "cards": 2387, "cards_total": 2785};
(function(){
 var inicio=Date.now(),base=10600000000,taxaPorSegundo=23000;
 var barra=null,espera=null,relogio=null,montado=false,tentativas=0;
 var MAX_TENTATIVAS=20;
 function numeroCenarios(){
  var total=base+Math.floor((Date.now()-inicio)/1000*taxaPorSegundo);
  return (total/1000000000).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1});
 }
 function numeroCards(){
  try{
   var vistos={};(D||[]).forEach(function(c){if(c&&c.id&&c.id!=='MOLDE')vistos[String(c.id).split('@')[0]]=1;});
   return Math.max(CONT.cards,Object.keys(vistos).length);
  }catch(e){return CONT.cards;}
 }
 function cancelaEspera(){if(espera!==null){clearTimeout(espera);espera=null;}}
 function iniciaRelogio(){if(relogio===null)relogio=setInterval(atualiza,1000);}
 function tentaMontar(){
  cancelaEspera();
  if(!montado)return false;
  ...
  s.innerHTML='Mais de <span id="cenariotxt" style="color:#22c58b">'+numeroCenarios()+'</span> bilhões de cenários'
   +' &nbsp;·&nbsp; <span id="cardtxt" style="color:#22c58b">'+numeroCards().toLocaleString("pt-BR")+'</span> cards analisados';
  s.title='Cenários analisados pelo método de pontuação';
  h.appendChild(s);barra=s;iniciaRelogio();return true;
 }
 function atualiza(){
  var s=document.getElementById('cenariotxt'),c=document.getElementById('cardtxt');
  if(s)s.textContent=numeroCenarios();if(c)c.textContent=numeroCards().toLocaleString('pt-BR');
 }
 function mount(){
  if(montado)return tentaMontar();
  montado=true;tentativas=0;return tentaMontar();
...
(function(){
  ...
  window.etapas = function(c, lvl){
    ...
    for(i = 0; i < 26; i++){
      ...
      e3  = mult(pre, st.m) + (e2 - pre) + tec[i];      /* + tecnico: multiplicador E o +1 */
